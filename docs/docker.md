...
> to the same `nastech-home` volume simultaneously. This can cause overlapping lock
...
> avoids this by running a single `NasTech-Agent` process that serves both the
...
That's it for a real personal Docker install. Your existing `~/.nastech`
...
them manually from the Tasks panel. In Docker, scheduled jobs require the NasTech gateway daemon
to tick while you are away. If System Settings shows `Gateway not configured`,
use `docker-compose.two-container.yml`,
`docker-compose.three-container.yml`, or run `nastech gateway` separately before
...
your real `~/.nastech` unless you intentionally want to test real state. Use an
isolated NasTech home and follow
[`docs/onboarding-agent-checklist.md`](onboarding-agent-checklist.md) instead.

> **Linux note**: run Compose as the user who owns the NasTech home. The command
> `sudo docker compose up -d` can make Compose expand `${HOME}` as `/root`, so
> the default `${HOME}/.nastech` bind mount becomes `/root/.nastech` instead of
> your user's real NasTech directory. Prefer adding your user to the `docker group`
...
The three-container layout adds the dashboard but is otherwise the same shape. If you must stay single-container, you can run `nastech gateway` inside the container as a long-lived background process, but the compose split is sturdier.
...
Since v0.14, `NasTech-Agent` can serve the gateway API and the built-in dashboard
from the same process by setting `NASTECH_DASHBOARD_HOST` and
`NASTECH_DASHBOARD_PORT`. Running agent and dashboard in one container means a
single writer to `nastech-home`, eliminating the concurrent-init write conflicts
that occur when `NasTech-Agent` and `nastech-dashboard` both start from the same
...
services:
  NasTech-Agent:
    ...
    ports:
      ...
      - "127.0.0.1:9119:9119"
    volumes:
      - nastech-home:/home/nastech/.nastech
      - NasTech-Agent-src:/opt/nastech
    environment:
      - NASTECH_HOME=/home/nastech/.nastech
      - NASTECH_UID=${UID:-1000}
      - NASTECH_GID=${GID:-1000}
      - NASTECH_DASHBOARD_HOST=0.0.0.0
      - NASTECH_DASHBOARD_PORT=9119
...
Check `nastech gateway run --help` for the exact flag names for your agent release —
the env-var equivalents shown above (`NASTECH_DASHBOARD_HOST`, `NASTECH_DASHBOARD_PORT`)
...
`nastech-dashboard` to `NasTech-Agent` with a `condition: service_healthy` healthcheck
...
If you see behavior issues after a mixed-version upgrade, capture both WebUI and NasTech-Agent versions and the compose layout in the issue.
...
**Symptom**: You set `NASTECH_HOME_MODE=0640` (or some other group-readable mode) on your host `.env` file, container starts, then errors out:
```
...
> ⚠️ **Multi-container warning**: `NASTECH_HOME_MODE` has DIFFERENT semantics in the agent image vs. the WebUI:
> - **WebUI**: credential FILE mode threshold (`0640` allows group bits on `.env`)
> - **Agent**: `NASTECH_HOME` *directory* mode (default `0700`)
>
> `0640` on a directory has no owner-execute bit, so the agent can't traverse its own home → bricked. For multi-container setups, use `NASTECH_HOME_MODE=0750` (group-traversable) or `0701` (x-only). The compose files have per-service comments that match each side's semantics.
...
- **Combined image** ([sunnysktsang/nastech-suite](https://github.com/sunnysktsang/nastech-suite)) — community fork that ships agent+webui+dashboard in one container

### 6. "config.yaml not loaded"

**Symptom**: You have a `config.yaml` in your host `~/.nastech/`, but the WebUI shows "no model configured" or doesn't pick up your custom providers.
...
### 7. "On Podman: can't share .nastech between containers"
...
**Fix**: Either upgrade to Podman 4+ (which fixes this), or use the [single-container setup](#5-minute-quickstart-single-container), or use the [community all-in-one image](https://github.com/sunnysktsang/nastech-suite).
...
docker volume rm <project>_NasTech-Agent-src
...
Replace `<project>` with your Compose project name (the parent directory by default; check with `docker volume ls`). The `nastech-home` volume (config, sessions, state) is left untouched — only `NasTech-Agent-src` (the agent's installed Python source) is recreated.

> The single-container setup (`docker-compose.yml`) does not use `NasTech-Agent-src` and is not affected by this upgrade pattern — pulling a newer WebUI image and `docker compose up -d --force-recreate` is sufficient.
...
- The gateway API (port 8642) is bound by the agent service only; the WebUI cannot bind it. Other containers reach the gateway via the `nastech-net` Docker network.
...
- **Filesystem boundary.** Both services share `nastech-home` (config, sessions, state), and the WebUI mounts the agent's installed source from `NasTech-Agent-src`. The WebUI mount is read-only (since v0.51.84), but the agent service still has write access, and both services share the home volume.
...
- **Trust boundary on the agent source.** The WebUI installs Python dependencies from the shared `NasTech-Agent-src` volume at startup. The read-only mount means a compromised WebUI cannot rewrite the agent source, but it does run code from that volume.
...
If you really need to bind-mount an existing host `~/.nastech` (e.g. you're keeping config in dotfiles, sharing with a non-Docker `nastech` install, etc.):
...
volumes:
  nastech-home:
    ...
    driver_opts:
      type: none
      o: bind
      device: /home/youruser/.nastech
  NasTech-Agent-src:
    driver: local
    driver_opts:
      type: none
      o: bind
      device: /opt/NasTech-Agent-source
```
...
- #858 — two-container `/opt/nastech` path confusion
