...
## "AIAgent not available -- check that NasTech-Agent is on sys.path"
...
**Why it happens.** The WebUI imports the agent class at chat time via `from run_agent import AIAgent`. That import only succeeds if the running Python's `sys.path` contains either the NasTech-Agent checkout or a pip-installed copy of the agent. Three common failure modes:
...
```bash
# If you have ~/NasTech-Agent (the default location):
ls -la ~/NasTech-Agent
readlink ~/NasTech-Agent          # if it's a symlink, where does it resolve?
ls ~/NasTech-Agent/agent/__init__.py 2>&1
```
...
```bash
cd /path/to/NasTech-Agent          # the directory holding pyproject.toml + the agent/ module
...
2. Check whether the run-journal contains real events:
   ```bash
   ls -la ~/.nastech/webui/sessions/_run_journal/<sid>/<stream_id>.jsonl
   head -2 ~/.nastech/webui/sessions/_run_journal/<sid>/<stream_id>.jsonl
   ```
