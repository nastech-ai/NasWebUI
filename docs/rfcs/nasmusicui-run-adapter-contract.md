# NasTech Run Adapter Contract and Migration Gates
...
- future NasTech runtime APIs cannot be adopted cleanly because WebUI lacks a
...
- Identify future backend mapping: existing NasTech runtime API, missing NasTech
...
- Do not depend on NasTech Agent shipping a WebUI-specific runtime connector before
...
future WebUI-managed runner, or a future NasTech `/v1/runs` backend.
...
| queue / continue | append follow-up work according to NasTech semantics | runner/runtime control plane |
...
|---|---|---|---|---|---|
| `STREAMS` / `STREAMS_LOCK` | `api.state_sync` process memory | adapter API surface + presentation fan-out | WebUI runner or future NasTech run observation API | keep live path; mirror events into journal | Must stop being authoritative for active run existence. |
...
| cached `AIAgent` objects / `AGENT_INSTANCES` | `api/config.py` process memory | runner process | runner-owned NasTech integration | unchanged | Moving this is deferred until after journal proof. |
...
| per-request `NASTECH_HOME` env mutation lock | `api/streaming.py` / config helpers | runner process | runner/profile execution context | unchanged | Long-term runner must isolate profile env without process-global mutation. |
...
| command capability metadata | WebUI command registry + NasTech command assumptions | adapter API surface | runtime command/capability metadata | unchanged | Unknown command support should not be guessed by WebUI. |
...
| Multi-profile isolation | profile-specific runs write/read the correct NasTech home and memory | protects #2134-family isolation concerns | runner/profile slice |
| Queue/continue | follow-up input during live/resumable work obeys NasTech semantics | prevents parallel continuation model | control migration slice |
...
- future NasTech CLI/Python/local API or `/v1/runs` backends can be evaluated
  behind the adapter.
...
- no dependency on NasTech Agent shipping `/v1/runs` before WebUI can validate the
...
- carry the runtime API gap matrix forward: missing NasTech-owned capabilities
...
- no permanent WebUI-owned active-run discovery cache when the missing capability
  belongs in a runner or future NasTech Runtime API;
...
NasTech runtime families such as `token.delta`, `tool.started`, and `done` before
...
- no permanent WebUI-owned active-run discovery cache that duplicates runner or
  future NasTech Runtime API responsibility.
...
- no server-side queue scheduler just for adapter symmetry;
- no broad WebUI product-surface migration;
- no claim that this is the canonical NasTech Agent Runtime API; if NasTech Agent
...
- If NasTech Agent later ships a durable `/v1/runs` API, which adapter fields map
