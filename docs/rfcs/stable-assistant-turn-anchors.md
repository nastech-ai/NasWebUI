...
- Insulate the presentation layer from the in-progress RuntimeAdapter substrate
  ...
  NasTech runtime changes the anchor's input source, not the UI.

## Non-goals

- Do not replace the Live-to-Final product model from #3400.
- Do not implement Transparent Stream in this RFC.
- Do not implement a RuntimeAdapter, runner process, sidecar, or NasTech runtime
  API.
...
`nastech-run-adapter-contract.md` draws the runtime ownership boundary:
...
emits now and a runner or NasTech runtime will emit later. `run_id` is the durable
...
`nastech-run-adapter-contract.md`) as its identity source, not a new scheme. That
envelope — `event_id = "run_id:seq"`, monotonic `seq`, `run_id` — is already
emitted by the WebUI run journal today and will be emitted by a runner or NasTech
...
from the WebUI run path to a runner or NasTech runtime changes the anchor's input
