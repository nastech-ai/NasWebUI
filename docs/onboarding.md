...
Do not delete `~/.nastech` just to see the wizard again. That directory can hold
your real NasTech config, credentials, memory, skills, profiles, sessions, and
cron state.

For a clean local trial, use an isolated NasTech home and WebUI state directory:
...
- NasTech Agent importability: whether WebUI can import and run `AIAgent`.
...
through the wizard or run `nastech model` in the same machine environment that
will run WebUI.
...
For API-key providers, the wizard writes the key to the active NasTech `.env`
...
the onboarding flow when your NasTech config selects the corresponding provider.
If the wizard points you back to `nastech model`, use that CLI flow first, then
refresh WebUI.
...
The workspace is the filesystem location NasTech should use for new sessions.
...
- Active NasTech `config.yaml`: provider, default model, and Base URL when
  relevant.
- Active NasTech `.env`: provider API keys when you entered one.
