...
- approving any cleanup of a real NasTech home
...
- Do not delete, move, or overwrite the real `~/.nastech` directory unless the
...
non-secret keys needed to understand the active NasTech home, WebUI state
directory, port, or host.

## Isolated local trial

Use an isolated NasTech home and WebUI state directory for a reinstall or support
...
```text
~/nastech-onboarding-test/webui/bootstrap-8789.log
```

For daemon-style installs, `ctl.sh` writes the daemon log to the active
`NASTECH_HOME` by default:
...
find ~/nastech-onboarding-test -maxdepth 3 -type f | sort
tail -n 120 ~/nastech-onboarding-test/webui/bootstrap-8789.log
```
...
- confirm the bootstrap found or installed NasTech Agent
