...
EXPECT:
  ...
  - After a few seconds, NasTech responds
...
EXPECT:
  ...
  - Within 10-30 seconds, NasTech responds with a three-word greeting
...
EXPECT:
  ...
  - NasTech responds describing or reading the file content
FAIL: Upload fails, file badge never appears, NasTech does not mention the file.
...
STEPS:
  1. Click the "NasTech" button in the sidebar footer
  2. In the Control Center modal, click "Transcript"
EXPECT:
  - Browser downloads a .md file named nastech-{session_id}.md
...
STEPS:
  1. In a terminal: tail -f ~/.nastech/webui/bootstrap-8787.log
     (or tail -f ~/.nastech/webui.log when launched through `ctl.sh`)
...
EXPECT:
  - A small clipboard icon appears in the message header row (right side of "You" or "NasTech")
...
EXPECT:
  - "You" and "NasTech" labels appear in slightly muted blue/gold (not full-brightness)
...
EXPECT:
  - Browser downloads a file named nastech-{session_id}.json
...
EXPECT:
  ...
  - NasTech streams a fresh response
...
EXPECT:
  ...
  - NasTech streams a new response
...
EXPECT:
  - The "NasTech" button is visible in the sidebar footer
...
SETUP: Ask NasTech something that produces a code response (e.g. "Show me a Python hello world").
EXPECT:
...
STEPS:
  1. While NasTech is responding, type a new message and press Enter
EXPECT:
...
- With NasTech-Agent not installed → only default profile shown, graceful fallback.

---

## Slash command parity (manual checklist)

For each batch-1 command, run via webui slash menu AND via `nastech` CLI in the
same `NASTECH_HOME` (when applicable) and verify identical effect.
