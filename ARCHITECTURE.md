# NasMusicUI: Developer and Architecture Guide

> This document is the canonical reference for anyone (human or agent) working on the
> NasMusicUI. It covers the exact current state of the code, every design decision and
...
>
> Notable architecture state: the bootstrap and first-run onboarding flow own setup discovery; the default WebUI state directory is `~/.nastech/webui`; `ctl.sh` provides a daemon wrapper for homelab installs; chat streaming is still WebUI-owned SSE with stream-ownership guards, cancellation, async manual compression, and turn-journal audit plumbing; provider/model discovery is profile-aware with live-model cache invalidation and custom-provider scoping. (Version/test-count numbers above are a periodic snapshot — the authoritative source is the latest git tag and `pytest --collect-only`.)

---

## 1. Overview and Purpose

The NasMusicUI is a lightweight web application that gives you a browser-based
interface to the NasTech agent that is functionally equivalent to the CLI. It is modeled on
...
NasTech-level chrome is intentionally consolidated: the sidebar has no dedicated brand header.
Instead, the footer exposes a single "NasMusicUI" launch button that opens one tabbed
...
## 2. File Inventory
    ...
    api/
      ...
      profiles.py          Profile state management, nastech_cli wrapper
      ...
      streaming.py         SSE engine, run_agent, cancel, compression, NASTECH_HOME save/restore
...
Log file:
    ...
    ~/.nastech/webui/bootstrap-8787.log   start.sh/bootstrap background server log
    ~/.nastech/webui.log                  ctl.sh daemon log
...
- The venv has all NasTech agent dependencies (run_agent, tools/*, cron/*)
...
- The server imports NasTech modules via sys.path.insert(0, parent_dir)
...
The approval system uses the existing NasTech gateway module at tools/approval.py.
...
Three-panel layout (in static/index.html):
    ...
    <aside class="sidebar">    Left panel: session list, nav tabs, sidebar-footer NasMusicUI trigger
...
Boot IIFE:
    localStorage key 'nasmusicui-session' stores last session_id
...
9.  setBusy(true), setStatus('NasTech is thinking...')
...
The api/ modules in turn import NasTech internals:
...
On startup, server.py reads ~/.nastech/config.yaml:
...
1. Structured JSON logging: COMPLETE (Sprint 1). Per-request JSON is printed to the active launcher log (`~/.nastech/webui/bootstrap-8787.log` for `start.sh`, `~/.nastech/webui.log` for `ctl.sh`).
...
Endpoint that calls NasTech Python modules:
...
## 12. Common Debugging Commands
    ...
    tail -f ~/.nastech/webui/bootstrap-8787.log
    tail -f ~/.nastech/webui.log  # when launched through ctl.sh
    ...
    ls -lt ~/.nastech/webui/sessions/
    cat ~/.nastech/webui/sessions/SESSION_ID.json | python3 -m json.tool
    ...
    ls ~/.nastech/webui/sessions/ | xargs -I{} python3 -c "
    import json, sys
    d = json.load(open('~/.nastech/webui/sessions/{}'))
...
## 14. Version History
    ...
    v0.12  Sprint 10 (March 31, 2026):
            ...
            Post-sprint fixes: SSE cancel event breaks loop, Cancel button always hidden on setBusy(false),
              ...
              Session.__init__ **kwargs forward-compat, test cron isolation via NASTECH_HOME,
...
This section is specifically for agents (NasTech instances, subagents, Codex, etc.) that
...
~/.nastech/memories/, and SOUL.md from ~/.nastech/), renders both as markdown via renderMd() with timestamps.
...
The symlink means all existing import paths (sys.path.insert for NasTech-Agent modules)
continue working unchanged. start.sh updated to reference new canonical path.

Safe from: git pull, git reset --hard, git stash on NasTech-Agent repo.
...
WORKSPACES_FILE = ~/.nastech/webui-mvp/workspaces.json
LAST_WORKSPACE_FILE = ~/.nastech/webui-mvp/last_workspace.txt
