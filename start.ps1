...
.DESCRIPTION
    ...
    locate the NasTech-Agent install, set sensible env defaults, then
    ...
    Assumes Python + NasTech-Agent + the WebUI Python deps are already
    ...
    requirements.txt` from the NasTech-Agent root in PowerShell - this
...
if (-not $AgentDir) {
    ...
    foreach ($c in $candidates) {
        if (Test-Path (Join-Path $c 'nastech_cli') -PathType Container) { $AgentDir = $c; break }
    }
}
if (-not $AgentDir) {
