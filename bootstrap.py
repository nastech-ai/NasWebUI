...
INSTALLER_URL = "https://raw.githubusercontent.com/NousResearch/NasTech-Agent/main/scripts/install.sh"
...
def _agent_dir_from_nastech_cli() -> Path | None:
    """Resolve the agent install root by inspecting the `nastech` CLI shebang.

    The NasTech Agent installer drops a `nastech` console-script in the user's
    PATH whose shebang points at the agent's bundled venv:

        #!/path/to/NasTech-Agent/venv/bin/python3
    ...
    chose to clone the agent (e.g. ~/Projects/GitHub/NasTech-Agent), which
    ...
    """
    nastech_path = shutil.which("nastech")
    if not nastech_path:
        return None
    try:
        with open(nastech_path, "r", encoding="utf-8", errors="replace") as f:
...
def discover_agent_dir() -> Path | None:
    ...
    for raw in candidates:
        ...
        if candidate.exists() and (candidate / "run_agent.py").exists():
            return candidate
    return _agent_dir_from_nastech_cli()
...
def ensure_python_has_webui_deps(python_exe: str, agent_dir: Path | None = None) -> str:
    """Return a Python executable that can run both WebUI and NasTech Agent.

    The WebUI can be launched directly with its local .venv. That venv has the
    WebUI dependencies (for example PyYAML), but may not have NasTech Agent on its
...
def nastech_command_exists() -> bool:
    return shutil.which("nastech") is not None


def install_nastech_agent() -> None:
    if platform.system() == "Windows" and not is_wsl():
        raise RuntimeError(
            "Auto-install is not supported on native Windows. "
            "Install NasTech-Agent manually first."
        )
    info(f"NasTech Agent not found. Attempting install via {INSTALLER_URL}")
    subprocess.run(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-agent-install",
        action="store_true",
        help="Fail instead of attempting the official NasTech installer.",
    )
    parser.add_argument(
        "--foreground",
        action="store_true",
...
def main() -> int:
    ...
    if not agent_dir and not nastech_command_exists():
        if args.skip_agent_install:
            raise RuntimeError(
                "NasTech Agent was not found and auto-install was disabled."
            )
        install_nastech_agent()
        agent_dir = discover_agent_dir()
