#!/usr/bin/env python3
"""Audit WebUI dependencies on the NasTech-Agent source tree.
...
def _import_kind(module_name: str) -> str:
    ...
    if module_name == "nastech_state" or module_name.startswith("nastech_state."):
        return "state_import"
    if module_name == "nastech_constants" or module_name.startswith("nastech_constants."):
        return "constants_import"
    if module_name.startswith("nastech_cli.runtime_provider"):
    ...
    if module_name.startswith(("nastech_cli.models", "agent.account_usage")):
        return "provider_model_catalog_import"
    if module_name.startswith(("nastech_cli.auth", "nastech_cli.config", "agent.credential_pool")):
        return "auth_config_credential_import"
    if module_name.startswith(("agent.skill_utils", "nastech_cli.plugins", "nastech_cli.profiles", "nastech_cli.goals")):
...
def build_report(root: Path) -> dict[str, object]:
    ...
    docker_findings = _iter_text_matches(
        ...
        (
            ("agent_source_volume", r"NasTech-Agent-src"),
            ("agent_source_path", r"/opt/nastech(?:\b|[-/])?"),
        ),
    )
    ...
    classes = (
        DependencyClass(
            ...
            current_dependency=(
                ...
                "the NasTech-Agent-src volume and /opt/nastech."
            ),
            replacement_surface=(
                "Remove the WebUI source mount after startup install and runtime "
                "imports move to NasTech-Agent endpoints or a versioned client package."
            ),
        ...
        DependencyClass(
            ...
            replacement_surface=(
                "Replace source-tree pip installs with a packaged NasTech-Agent client "
        ...
        DependencyClass(
            ...
            current_dependency=(
                "Browser chat, approvals, tools, and scheduled work import NasTech Agent "
            ...
            replacement_surface=(
                ...
                "NasTech Agent APIs or a versioned client package before removing source mounts."
            ),
            findings=_findings_by_kind(
                runtime_findings,
        ...
        DependencyClass(
            ...
            current_dependency=(
                "WebUI imports nastech_state.SessionDB and related state helpers directly "
                "to read or write agent session state."
            ),
            replacement_surface=(
                "Move cross-container state reads/writes behind NasTech-Agent session/state "
        ...
        DependencyClass(
            ...
            current_dependency=(
                "WebUI imports nastech_cli.runtime_provider and agent adapter helpers "
                "to resolve providers and normalize gateway calls."
            ),
            replacement_surface=(
                "Route provider resolution and gateway invocation through NasTech-Agent "
        ...
        DependencyClass(
            ...
            current_dependency=(
                "WebUI imports nastech_cli and agent helpers for config/auth status, "
