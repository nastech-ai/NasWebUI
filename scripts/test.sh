...
venv_guidance() {
  ...
  if [[ -n "$base_py" ]]; then
    ...
    echo "Could not create a working .venv for NasWebUI tests." >&2
  fi
  echo "Install the matching Python venv/ensurepip package (e.g. python3.x-venv on Debian/Ubuntu)" >&2
  echo "or set NASWEBUI_TEST_PYTHON to a supported Python 3.11, 3.12, or 3.13 interpreter." >&2
}
...
create_or_rebuild_venv() {
  ...
  if [[ -L "$VENV_DIR" ]]; then
    ...
    echo "Remove the .venv symlink (or set NASWEBUI_TEST_PYTHON to a supported Python 3.11-3.13) and rerun." >&2
    return 2
  fi
...
select_python() {
  local requested="${NASWEBUI_TEST_PYTHON:-}"
  ...
  if [[ -n "$requested" ]]; then
    ...
    if [[ -z "$requested_path" || ! -x "$requested_path" ]]; then
      echo "NASWEBUI_TEST_PYTHON does not point to an executable: $requested" >&2
      return 2
    fi
    if ! is_supported_python "$requested_path"; then
      echo "Unsupported Python for NasWebUI tests: $requested_path ($(python_version "$requested_path"))" >&2
    ...
    if [[ -z "$base_py" ]]; then
      echo "No supported Python found for NasWebUI tests." >&2
...
if missing="$(missing_dev_deps "$PYTHON_BIN" 2>/dev/null)"; then
  ...
  echo "Installing missing NasWebUI test dependencies in $PYTHON_BIN ($(python_version "$PYTHON_BIN"))." >&2
  if [[ -n "${missing:-}" ]]; then
