...
chown_home_nasmusicuiwebui() {
  ...
  # NasTech-Agent-src volume read-only on the WebUI side because the WebUI only
...
if [ -f /app/venv/.deps_installed ]; then
  ...
  if [ -n "$_agent_src" ]; then
    ...
    # `egg_info` build step, however, touches `nastech_agent.egg-info/` inside the
    ...
    _stage_src="/tmp/NasTech-Agent-build"
    rm -rf "$_stage_src"
    mkdir -p "$_stage_src"
    if command -v rsync >/dev/null 2>&1; then
      rsync -a \
        ...
        || error_exit "Failed to stage NasTech-Agent source to writable build dir"
    else
      ...
      cp -a "$_agent_src"/. "$_stage_src"/ \
        || error_exit "Failed to copy NasTech-Agent source to writable build dir"
    ...
    uv pip install "$_stage_src[all]" --trusted-host pypi.org --trusted-host files.pythonhosted.org \
      || error_exit "Failed to install NasTech-Agent's requirements"
    rm -rf "$_stage_src"
  else
    echo ""
    echo "!! WARNING: NasTech-Agent source not found."
