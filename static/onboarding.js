...
function _renderOnboardingProviderOAuthField(provider){
  ...
  return `<div class="onboarding-oauth-card onboarding-oauth-pending" style="margin-top:12px">
    ...
    <div style="flex:1">
      ...
      <p style="margin-top:6px;color:var(--muted);font-size:13px"><strong>Claude Code subscription credentials are not the same as an Anthropic API key.</strong> Use this path only when you want NasTech to use Claude Code credentials already available on the server, or start a short polling flow while you complete <code>claude setup-token</code> on the host.</p>
...
async function _pollCodexOAuth(){
  ...
  try{
    ...
    _setCodexOAuthButton(true);
    if(status==='success'){
      _renderCodexOAuthTerminal('success','Credentials saved to the NasTech credential pool. Refreshing provider status…');
...
async function _pollAnthropicOAuth(){
  ...
  try{
    ...
    _setAnthropicOAuthButton(true);
    if(status==='success'){
      _renderAnthropicOAuthTerminal('success','NasTech is now linked to Claude Code credentials. Refreshing provider status…');
...
async function startAnthropicOAuth(){
  ...
  flowDiv.style.display='block';
  flowDiv.innerHTML=`<div class="onboarding-oauth-card onboarding-oauth-pending"><div class="onboarding-oauth-icon">⏳</div><div><strong>Checking Claude Code credentials…</strong><p>NasTech is checking for existing Claude Code OAuth credentials on this server.</p></div></div>`;
  try{
    ...
    flowDiv.innerHTML=`
      <div class="onboarding-oauth-card onboarding-oauth-pending">
        ...
        <div style="flex:1">
          ...
          <p style="margin-top:6px">${esc(action_required||"Run 'claude setup-token' on the server, then return here. NasTech will detect the credential automatically.")}</p>
