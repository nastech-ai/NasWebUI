...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function _streamFadeNextText(targetText){
    ...
    // appear to accelerate. NasTech has a playout buffer, so track incoming word
    ...
    source.addEventListener('cancel',e=>{
      ...
      (async()=>{
        try{
          ...
          if(S.session&&S.session.session_id===activeSid){
            clearLiveToolCards();if(!assistantText)removeThinking();
            const cancelAgentName=(assistantDisplayName()+'').trim()||'NasTech';
...
function transcript(){
  const lines=[`# NasTech session ${S.session?.session_id||''}`,``,
...
function _stashClarifyDraft(reason) {
  ...
  const key = `nastech-clarify-draft-${sid}-${_clarifySignature || "unknown"}`;
  try {
