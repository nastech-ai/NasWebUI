...
function assistantDisplayName(){
  ...
  return window._botName||'NasTech';
}
...
// Handles iOS PWA standalone mode and keeps subpath mounts like /nastech/ from
...
const MEDIA_PLAYBACK_STORAGE_KEY='nastech-media-playback-rate';
...
async function _fetchLiveModels(provider, sel){
  ...
  try{
    ...
    if(added>0){
      if(typeof syncModelChip==='function') syncModelChip();
      console.debug('[nastech] Live models loaded for',provider+':',added,'new models added');
    }
  }catch(e){
    console.debug('[nastech] Live model fetch failed for',provider,e.message);
  }finally{
 ...
 * currently configured in NasTech. Returns a warning string if mismatched,
...
function _checkProviderMismatch(modelId){
  ...
  if(norm(modelProvider)!==norm(ap)){
    return (window.t?window.t('provider_mismatch_warning',modelId,ap):
      `"${modelId}" may not work with your configured provider (${ap}). Send anyway or run .nastech model\` to switch.`);
  }
  return null;
}
...
function renderMd(raw){
  ...
  s=s.replace(/\x00D(\d+)\x00/g,(_,i)=>{
    ...
    if(/^https?:\/\//i.test(ref)){
      ...
      // joins cleanly — this preserves any subpath mount (e.g. /nastech/).
      let src=ref;
...
function _buildBrowserUtterance(text, btn){
  ...
  const savedVoice=localStorage.getItem('nastech-tts-voice');
  const voices=speechSynthesis.getVoices();
  if(savedVoice&&voices.length){
    const match=voices.find(v=>v.name===savedVoice);
    if(match) utter.voice=match;
  }
  const savedRate=parseFloat(localStorage.getItem('nastech-tts-rate'));
  if(!isNaN(savedRate)) utter.rate=Math.min(2,Math.max(0.5,savedRate));
  const savedPitch=parseFloat(localStorage.getItem('nastech-tts-pitch'));
...
function _createVoiceNotePlayer(btn){
  const seg=btn?btn.closest('.assistant-segment,.msg-row'):null;
  if(!seg) return null;
  const old=seg.querySelector('.nt-voice-note');
  if(old) old.remove();
  const hts=[6,12,8,16,10,14,5,11,15,7,13,9,4,12,8];
  const bars=hts.map(function(h){return '<span class="nt-voice-note__bar" style="--nt-bh:'+h+'px"></span>';}).join('');
  const el=document.createElement('div');
  el.className='nt-voice-note';
  el.setAttribute('role','region');
  el.setAttribute('aria-label','Voice note');
  el.innerHTML=
    '<button class="nt-voice-note__btn" aria-label="Play">'+
    '<svg class="nt-vnb-play" width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>'+
    '<svg class="nt-vnb-pause" width="14" height="14" viewBox="0 0 24 24" fill="currentColor" style="display:none"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>'+
    '</button>'+
    '<div class="nt-voice-note__wave">'+bars+'</div>'+
    '<span class="nt-voice-note__time">0:00</span>';
  const foot=seg.querySelector('.msg-foot');
  seg.insertBefore(el,foot||null);
  return el;
}

function _vnSetState(vn,playing){
  if(!vn) return;
  if(playing){
    vn.classList.add('nt-voice-note--playing');
    const p=vn.querySelector('.nt-vnb-play'),pa=vn.querySelector('.nt-vnb-pause');
    if(p) p.style.display='none';
    if(pa) pa.style.display='';
  } else {
    vn.classList.remove('nt-voice-note--playing');
    const p=vn.querySelector('.nt-vnb-play'),pa=vn.querySelector('.nt-vnb-pause');
...
function _playEdgeTtsChunked(text, btn){
  ...
  const vn=_createVoiceNotePlayer(btn);
  _vnSetState(vn,true);
  ...
  const _playOne=function(idx){
    if(idx>=chunks.length){
      ...
      if(btn) btn.dataset.speaking='0';
      _vnSetState(vn,false);
      return;
    }
    const chunk=chunks[idx];
    const voice=localStorage.getItem('nastech-tts-voice')||'en-GB-SoniaNeural';
    const savedRate=parseFloat(localStorage.getItem('nastech-tts-rate'));
    const savedPitch=parseFloat(localStorage.getItem('nastech-tts-pitch'));
    let rate='', pitch='';
    ...
    .then(function(blob){
      ...
      const timeEl=vn?vn.querySelector('.nt-voice-note__time'):null;
      audio.ontimeupdate=function(){
        if(timeEl) timeEl.textContent=_fmtVoiceTime(audio.currentTime);
      };
      ...
      audio.play().catch(function(e){
        ...
        if(btn) btn.dataset.speaking='0';
        _vnSetState(vn,false);
        if(typeof showToast==='function') showToast('Voice: '+(e&&e.message||e));
      });
    })
    .catch(function(e){
      _ttsSpeaking=false;_playingEdgeAudio=null;
      if(btn) btn.dataset.speaking='0';
      _vnSetState(vn,false);
      if(typeof showToast==='function') showToast('Voice failed: '+(e&&e.message||e));
    });
  };

  if(vn){
    const vnBtn=vn.querySelector('.nt-voice-note__btn');
    if(vnBtn) vnBtn.addEventListener('click',function(){
      if(_ttsSpeaking&&_playingEdgeAudio){
        _playingEdgeAudio.pause();
        _ttsSpeaking=false;
        _vnSetState(vn,false);
        if(btn) btn.dataset.speaking='0';
      } else if(!_ttsSpeaking&&_playingEdgeAudio){
        _playingEdgeAudio.play();
        _ttsSpeaking=true;
        _vnSetState(vn,true);
        if(btn) btn.dataset.speaking='1';
      } else {
...
function speakMessage(btn){
  ...
  const engine=localStorage.getItem('nastech-tts-engine')||'edge';
  if(engine==='edge'){
...
function autoReadLastAssistant(){
  ...
  const pref=localStorage.getItem('nastech-tts-auto-read');
  if(pref!=='true') return;
...
// ── NasTech agent/gateway heartbeat alert (#716) ──
...
function _showAgentHealthAlert(payload){
  ...
  if(!banner) return;
  if(title) title.textContent='NasTech agent is not responding';
...
const WHATS_NEW_SUMMARY_STORAGE_KEY='nastech-whats-new-generated-summaries';
...
function dismissUpdate(){
  ...
  sessionStorage.setItem('nastech-update-dismissed','1');
}
...
async function applyUpdates(){
  ...
  try{
    ...
    sessionStorage.removeItem('nastech-update-checked');
    sessionStorage.removeItem('nastech-update-dismissed');
...
function syncTopbar(){
  if(!S.session){
    ...
    if(typeof _syncNasTechPanelSessionActions==='function') _syncNasTechPanelSessionActions();
    else {
  ...
  // active profile (the nastech_profile cookie, set only by /api/profile/switch).
...
const _activityDisclosureStoragePrefix='nastech-activity-disclosure:';
...
function renderMessages(options){
  ...
  if(!S.busy && (hasMessageToolMetadata||!S.toolCalls||!S.toolCalls.length)){
    ...
    S.messages.forEach((m,rawIdx)=>{
      if(!m) return;
      // OpenAI / NasTech CLI format: role=tool with tool_call_id
      if(m.role==='tool'){
...
function toggleWorkspaceHiddenFiles(value){
  ...
  try{localStorage.setItem('nastech-workspace-show-hidden-files',S.showHiddenWorkspaceFiles?'1':'0');}catch(_){}
  _syncWorkspaceHiddenToggle();
  renderFileTree();
}
try{S.showHiddenWorkspaceFiles=localStorage.getItem('nastech-workspace-show-hidden-files')==='1';}catch(_){}
...
async function uploadPendingFiles(){
  ...
  (function(){
    ...
    // Play a short tone: freq (Hz), duration (ms), type, gain
    function _tone(freq, dur, type, gain){
      ...
      try{
        ...
        vol.gain.linearRampToValueAtTime(gain||0.06, ctx.currentTime + 0.004);
        vol.gain.exponentialRampToValueAtTime(0.0001, ctx.currentTime + dur/1000);
        osc.start(ctx.currentTime);
        osc.stop(ctx.currentTime + dur/1000 + 0.01);
      }catch(e){}
    }

    // Sound profiles for different element types
    function _playForElement(el){
      if(!el) return;
      // Nav tab / rail button → soft mid tick
      if(el.classList.contains('nav-tab') || el.classList.contains('rail-btn')){
        _tone(520, 55, 'triangle', 0.055);
        return;
      }
      // Send button → clean high ping
      if(el.classList.contains('send-btn') || el.id === 'btnSend'){
        _tone(880, 80, 'sine', 0.07);
        return;
      }
      // New chat button → ascending two-tone
      if(el.classList.contains('new-chat-btn') || el.id === 'btnNewChat'){
        _tone(440, 45, 'sine', 0.06);
        setTimeout(function(){ _tone(660, 60, 'sine', 0.05); }, 40);
        return;
      }
      // Suggestion chips → soft low tick
      if(el.classList.contains('suggestion')){
        _tone(380, 50, 'triangle', 0.05);
        return;
      }
      // Settings / modal open buttons → neutral click
      if(el.dataset && el.dataset.panel === 'settings'){
        _tone(340, 45, 'triangle', 0.045);
        return;
      }
      // Hamburger menu toggle → double soft click
      if(el.id === 'btnHamburger' || el.classList.contains('app-titlebar-hamburger')){
        _tone(420, 40, 'triangle', 0.05);
        return;
      }
      // Generic buttons inside panels → very soft tick
      if(el.closest && el.closest('.panel-view, .settings-main, .composer-wrap')){
        _tone(460, 40, 'triangle', 0.035);
        return;
      }
      // Fallback: do nothing — no sound for unregistered elements
    }

    // Event delegation: only fire on intentional interactive elements
    document.addEventListener('click', function(e){
      if(!_enabled) return;
      var el = e.target;
      if(!el) return;
      // Walk up max 2 levels to find the interactive element
      for(var i = 0; i < 3; i++){
        if(!el || el === document.body) break;
        var tag = el.tagName && el.tagName.toLowerCase();
        var isBtn = tag === 'button' || tag === 'a';
        var hasRole = el.getAttribute && (el.getAttribute('role') === 'button');
        if(isBtn || hasRole){
          _playForElement(el);
          return;
        }
  ...
  /* ═══ Auto-Sidebar: know where to stop and expand ═══
     Prevents the sidebar from opening when the viewport is too narrow
     or when content areas need the full width. Auto-collapses on resize. */
  (function(){
    var _SIDEBAR_BREAKPOINT = 641; // px — matches media query
    var _SIDEBAR_MIN_CONTENT = 340; // px — minimum content area width to keep sidebar open

    function _shouldCollapseSidebar(){
      return window.innerWidth < _SIDEBAR_BREAKPOINT;
    }

    function _autoManageSidebar(){
      var sidebar = document.querySelector('.sidebar');
      var layout = document.querySelector('.layout');
      if(!sidebar) return;
      if(_shouldCollapseSidebar()){
        // On mobile: ensure sidebar is properly closed unless explicitly opened
        if(!sidebar.classList.contains('mobile-open')){
          sidebar.style.willChange = 'left';
        }
      } else {
        // On desktop: clear any mobile-specific inline styles
    ...
    window.addEventListener('resize', function(){
      ...
      _resizeTimer = setTimeout(_autoManageSidebar, 80);
    }, {passive: true});

    // Run on init after DOM is ready
    if(document.readyState === 'loading'){
      document.addEventListener('DOMContentLoaded', _autoManageSidebar);
    } else {
      _autoManageSidebar();
    }
  })();
