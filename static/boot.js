...
function _syncWorkspacePanelInlineWidth(){
  ...
  const saved = localStorage.getItem('nastech-panel-w');
  if(!saved) return;
...
(function(){
  ...
  // Prefer NasTech server-side STT (MediaRecorder -> /api/transcribe) only
  ...
  let _rawAudioMode = localStorage.getItem('nastech-raw-audio-mode') === 'true';
  ...
  if(rawAudioCheckbox){
    ...
    rawAudioCheckbox.addEventListener('change', function(){
      ...
      localStorage.setItem('nastech-raw-audio-mode', _rawAudioMode ? 'true' : 'false');
      _updateMicTooltip();
    });
  }
  _updateMicTooltip();
...
(function(){
  ...
  function _voiceModePrefEnabled(){
    try{ return localStorage.getItem('nastech-voice-mode-button')==='true'; }
    catch(_){ return false; }
  }
...
$('btnDownload').onclick=()=>{
  ...
  a.download=`nastech-${S.session.session_id}.md`;a.click();URL.revokeObjectURL(a.href);
};
$('btnExportJSON').onclick=()=>{
  if(!S.session)return;
  const url=`/api/session/export?session_id=${encodeURIComponent(S.session.session_id)}`;
  const a=document.createElement('a');a.href=url;
  a.download=`nastech-${S.session.session_id}.json`;a.click();
};
...
$('modelSelect').onchange=async()=>{
  ...
  // Warn if selected model belongs to a different provider than what NasTech is configured for
...
(function(){
  ...
  window._initResizePanels = function(){
    ...
    initResize('sidebarResize',    sidebar,    'right', SIDEBAR_MIN, SIDEBAR_MAX, 'nastech-sidebar-w');
    initResize('rightpanelResize', rightpanel, 'left',  PANEL_MIN,   PANEL_MAX,   'nastech-panel-w');
  };
})();
...
//   3. Native WKWebView wrappers (e.g. nastech-swift-mac) that read this attribute as
...
(async()=>{
  ...
  try{
    ...
    if(typeof setLocale==='function'){
      const _lang=typeof resolvePreferredLocale==='function'
        ? resolvePreferredLocale(s.language, localStorage.getItem('nastech-lang'))
        : (s.language || localStorage.getItem('nastech-lang') || 'en');
      setLocale(_lang);
      if(typeof applyLocaleToDOM==='function')applyLocaleToDOM();
    }
    // TTS: apply enabled state on boot so buttons show/hide correctly (#499)
    if(typeof _applyTtsEnabled==='function') _applyTtsEnabled(localStorage.getItem('nastech-tts-enabled')==='true');
  }catch(e){
    ...
    if(typeof setLocale==='function'){
      const _lang=typeof resolvePreferredLocale==='function'
        ? resolvePreferredLocale(null, localStorage.getItem('nastech-lang'))
        : (localStorage.getItem('nastech-lang') || 'en');
      setLocale(_lang);
      if(typeof applyLocaleToDOM==='function')applyLocaleToDOM();
    }
    if(typeof _applyTtsEnabled==='function') _applyTtsEnabled(localStorage.getItem('nastech-tts-enabled')==='true');
  ...
  if(_testUpdates||(_bootSettings.check_for_updates!==false&&!sessionStorage.getItem('nastech-update-checked')&&!sessionStorage.getItem('nastech-update-dismissed'))){
    const _checkUrl='api/updates/check'+(_testUpdates?'?simulate=1':'');
    api(_checkUrl,{method:_testUpdates?'GET':'POST',body:_testUpdates?undefined:JSON.stringify({force:false})}).then(d=>{if(!_testUpdates)sessionStorage.setItem('nastech-update-checked','1');if((d.webui&&d.webui.behind>0)||(d.agent&&d.agent.behind>0))_showUpdateBanner(d);}).catch(()=>{});
  }
  ...
  const pwaLaunchAction=(window.NasTechPWA&&typeof window.NasTechPWA.launchAction==='function')
    ? window.NasTechPWA.launchAction()
    : null;
  ...
  console.error('[nastech] boot failed', e);
