...
function _cronScriptCardHtml(job){
  ...
  return `<div class="detail-card cron-script-card">
        ...
        ${workdirRow}
        <div class="detail-hint cron-script-card-hint">${esc(t('cron_script_path_hint') || 'Resolved under ~/.nastech/scripts/ unless an absolute path. Edit the script file on the server to change behavior.')}</div>
      </div>`;
}
...
function _renderCronForm({ name, schedule, prompt, deliver, profile, toast_notifications=true, no_agent=false, script='', model='', provider='', isEdit }){
  ...
  const scriptBlock = isNoAgent ? `
        <div class="detail-form-row">
          ...
          <div class="detail-form-hint">${esc(t('cron_script_path_hint') || 'Resolved under ~/.nastech/scripts/ unless an absolute path. Edit the script file on the server to change behavior.')}</div>
        </div>` : '';
...
function _kanbanRenderMarkdown(source){
  ...
  while (i < lines.length) {
    ...
    if (/^```/.test(trimmed)) {
      ...
      out.push(lang
        ? `<pre class="nastech-kanban-code"><code class="language-${_kanbanRenderMarkdownInline(lang)}">${codeHtml}</code></pre>`
        : `<pre class="nastech-kanban-code"><code>${codeHtml}</code></pre>`);
      continue;
    }
    ...
    if (taskMatch) {
      ...
      const items = [];
      items.push(`<li class="nastech-kanban-task${checked ? ' checked' : ''}"><input type="checkbox"${checked ? ' checked' : ''} disabled> ${_kanbanRenderMarkdownInline(text)}</li>`);
      i++;
      ...
      while (i < lines.length) {
        ...
        if (nextTask) {
          const c = nextTask[1] !== ' ';
          items.push(`<li class="nastech-kanban-task${c ? ' checked' : ''}"><input type="checkbox"${c ? ' checked' : ''} disabled> ${_kanbanRenderMarkdownInline(nextTask[2])}</li>`);
          i++;
        } else if (nextLi) {
  ...
  return `<div class="nastech-kanban-md">${out.join('\n')}</div>`;
}
...
async function runKanbanDispatcher(){
  ...
  // (one `nastech -p <assignee>` per claimed row, up to max=8 per call).
...
//
// The assignee field auto-completes against the union of (a) live NasTech
...
async function _kanbanPopulateAssigneeSelect(currentValue){
  ...
  let html = '';
  if (profiles.length) {
    html += `<optgroup label="${esc(t('kanban_assignee_profiles_label') || 'NasTech profiles')}">`;
...
const KANBAN_BOARD_LS_KEY = 'nastech-kanban-active-board';
...
function _renderLlmWikiStatus(d) {
  ...
  const rawDocsUrl = status.docs_url || 'https://NasTech-Agent.nousresearch.com/docs/user-guide/skills/bundled/research/research-llm-wiki';
  ...
  const toggleNote = status.toggle_available
    ? 'Toggle available from configured NasTech Agent setting.'
...
function _syncNasTechPanelSessionActions(){
...
function _rememberPreferencesSaved(payload){
  if(!payload) return;
  if(payload.send_key!==undefined) localStorage.setItem('nastech-pref-send_key',payload.send_key);
  if(payload.language!==undefined) localStorage.setItem('nastech-pref-language',payload.language);
}
...
async function loadSettingsPanel(){
  try{
    ...
    const resolvedLanguage=(typeof resolvePreferredLocale==='function')
      ? resolvePreferredLocale(settings.language, localStorage.getItem('nastech-lang'))
      : (settings.language || localStorage.getItem('nastech-lang') || 'en');
    ...
    if(ttsEnabledCb){ttsEnabledCb.checked=localStorage.getItem('nastech-tts-enabled')==='true';ttsEnabledCb.onchange=function(){localStorage.setItem('nastech-tts-enabled',this.checked?'true':'false');_applyTtsEnabled(this.checked);};}
    const ttsAutoReadCb=$('settingsTtsAutoRead');
    if(ttsAutoReadCb){ttsAutoReadCb.checked=localStorage.getItem('nastech-tts-auto-read')==='true';ttsAutoReadCb.onchange=function(){localStorage.setItem('nastech-tts-auto-read',this.checked?'true':'false');};}
    ...
    if(voiceModeCb){
      voiceModeCb.checked=localStorage.getItem('nastech-voice-mode-button')==='true';
      voiceModeCb.onchange=function(){
        localStorage.setItem('nastech-voice-mode-button',this.checked?'true':'false');
    ...
    if(ttsRateSlider){
      const savedRate=localStorage.getItem('nastech-tts-rate');
      ttsRateSlider.value=savedRate||'1';
      if(ttsRateValue) ttsRateValue.textContent=parseFloat(ttsRateSlider.value).toFixed(1)+'x';
      ttsRateSlider.oninput=function(){if(ttsRateValue)ttsRateValue.textContent=parseFloat(this.value).toFixed(1)+'x';localStorage.setItem('nastech-tts-rate',this.value);};
    }
    const ttsPitchSlider=$('settingsTtsPitch');
    const ttsPitchValue=$('settingsTtsPitchValue');
    if(ttsPitchSlider){
      const savedPitch=localStorage.getItem('nastech-tts-pitch');
      ttsPitchSlider.value=savedPitch||'1';
      if(ttsPitchValue) ttsPitchValue.textContent=parseFloat(ttsPitchSlider.value).toFixed(1);
      ttsPitchSlider.oninput=function(){if(ttsPitchValue)ttsPitchValue.textContent=parseFloat(this.value).toFixed(1);localStorage.setItem('nastech-tts-pitch',this.value);};
    }
    const notifCb=$('settingsNotificationsEnabled');
    ...
    const botNameField=$('settingsBotName');
    if(botNameField){
      botNameField.value=settings.bot_name||'NasTech';
    ...
    _syncNasTechPanelSessionActions();
...
async function _loadPluginPage(path, label) {
  ...
  // Security note: plugins are locally-installed (~/.nastech/plugins/), similar
...
function _providerQuotaPoolShouldDefaultOpen(pool){
  try{
    const saved=localStorage.getItem('nastech-provider-quota-pool-open');
...
function _buildProviderQuotaCard(status){
  ...
  if(poolDetails){
    poolDetails.addEventListener('toggle',()=>{
      try{localStorage.setItem('nastech-provider-quota-pool-open',poolDetails.open?'1':'0');}catch(e){}
    });
  }
  return card;
}
...
function _buildProviderCard(p){
  ...
  // key_source can be 'oauth' (nastech auth), 'config_yaml' (token in config.yaml), or 'none'.
  ...
  if(isOauth){
    ...
    if(p.key_source==='config_yaml'){
      hint.textContent=t('providers_oauth_config_yaml_hint')||'Token configured via config.yaml. To update, edit the providers section in your config.yaml or run nastech auth.';
      ...
      hint.textContent=t('providers_oauth_not_configured_hint')||'Not authenticated. Run nastech auth in the terminal to configure this provider.';
  ...
  if(p.configurable){
    ...
    hint.textContent=p.is_custom
      ? 'Custom provider loaded from config.yaml / nastech model. Edit it from the CLI or config file.'
...
function _applySavedSettingsUi(saved, body, opts){
  ...
  window._botName=body.bot_name||'NasTech';
  if(typeof applyBotName==='function') applyBotName();
...
// Keep in sync with nastech_cli/main.py _AUX_TASKS and nastech_cli/web_server.py _AUX_TASK_SLOTS.
...
window.addEventListener('nastech:cron_created', () => {
