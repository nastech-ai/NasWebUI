...
const CACHE_NAME = 'nastech-shell-__WEBUI_VERSION__';
...
self.addEventListener('fetch', (event) => {
  ...
  // The WebUI may be mounted under a subpath such as /nastech/, so API
  // requests can look like /nastech/api/sessions rather than /api/sessions.
  if (
    ...
    event.respondWith(
      fetch(new Request(event.request, { cache: 'no-store' })).then((response) => {
        ...
        return caches.match('./').then((cached) => cached || new Response(
          ...
          '<p>NasTech requires a server connection. Please check your network and try again.</p>' +
          '</body></html>',
