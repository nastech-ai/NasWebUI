...
# CI passes: --build-arg NASMUSICUI_VERSION=$(git describe --tags --always)
# Local builds that omit the arg get "unknown" as the fallback.
ARG NASMUSICUI_VERSION=unknown
RUN echo "__version__ = '${NASMUSICUI_VERSION}'" > /apptoo/api/_version.py

# Default to binding all interfaces (required for container networking)
ENV NASMUSICUI_HOST=0.0.0.0
ENV NASMUSICUI_PORT=8787
