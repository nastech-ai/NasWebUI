"""NasTech Kanban bridge for the WebUI.

This module exposes a full CRUD API under ``/api/kanban/*`` while keeping
NasTech Agent's ``nastech_cli.kanban_db`` as the only source of truth.
...
def _kb():
    """Lazily import nastech_cli.kanban_db to avoid circular imports at module load."""
...
def _config_payload(*, board=None):
    """Return kanban configuration: column names, known assignees, and lane/display settings from nastech_cli.config."""
    kb = _kb()
    try:
        with _conn(board=board) as conn:
            try:
...
def handle_kanban_get(handler, parsed) -> bool | None:
    ...
    try:
        ...
        # nastech_cli not installed (webui-only deploy). Return a clean 503
