...
WORKSPACE_GIT_DESTRUCTIVE_ENV = "NASMUSICUI_WORKSPACE_GIT_DESTRUCTIVE"
_GIT_ENV_SCRUB_KEYS = (
    "GIT_DIR",
    "GIT_WORK_TREE",
    "GIT_CONFIG_GLOBAL",
...
def _stash_subject_parts(subject: str) -> tuple[str, str] | None:
    ...
    if not branch or not message.startswith(_NASTECH_BRANCH_SWITCH_STASH_PREFIX):
        return None
...
def git_stash_and_checkout(
    ...
    stash_name = f"{_NASTECH_BRANCH_SWITCH_STASH_PREFIX} to {target_label}".strip()
    restored: dict = {}
    with _git_mutation_lock(ctx):
        _validate_checkout_request_locked(ctx, ref, mode, new_branch)
        stashed = False
