...
NASTECH_HOME directory is used for config, skills, memory, cron, and API keys.
Profile switches update os.environ['NASTECH_HOME'] and monkey-patch module-level
cached paths in NasTech-Agent modules (skills_tool, skill_manager_tool,
cron/jobs) that snapshot NASTECH_HOME at import time.
"""
import json
import logging
import os
...
# ── Constants (match nastech_cli.profiles upstream) ─────────────────────────
...
# reads its own profile from the nastech_profile cookie instead of the
...
def snapshot_skill_home_modules() -> dict[str, dict[str, object]]:
    ...
    for module_name in _SKILL_HOME_MODULES:
        ...
        snapshot[module_name] = {
            "module_present": True,
            "has_NASTECH_HOME": hasattr(module, "NASTECH_HOME"),
            "NASTECH_HOME": getattr(module, "NASTECH_HOME", None),
...
def patch_skill_home_modules(home: Path) -> None:
    """Patch imported skill modules that cache NASTECH_HOME at import time."""
    for module_name in _SKILL_HOME_MODULES:
        module = sys.modules.get(module_name)
        if module is None:
            continue
        try:
            module.NASTECH_HOME = home
...
def restore_skill_home_modules(snapshot: dict[str, dict[str, object]]) -> None:
    ...
    for module_name, values in snapshot.items():
        ...
        if module is None:
            continue
        for attr in ("NASTECH_HOME", "SKILLS_DIR"):
...
def _unwrap_profile_home_to_base(home: Path) -> Path:
    """Return the base NasTech home when *home* is already a named profile dir."""
    if home.parent.name == 'profiles':
        return home.parent.parent
    return home


def _resolve_base_nastech_home() -> Path:
    """Return the BASE ~/.nastech directory — the root that contains profiles/.

    This is intentionally distinct from NASTECH_HOME, which tracks the *active
    profile's* home and changes on every profile switch.  The base dir must
    always point to the top-level .nastech regardless of which profile is active.

    Resolution order:
      1. NASTECH_BASE_HOME env var (set explicitly, highest priority)
      2. NASTECH_HOME env var — but only if it does NOT look like a profile subdir
         (i.e. its parent is not named 'profiles').  This handles test isolation
         where NASTECH_HOME is set to an isolated test state dir.
      3. ~/.nastech (always-correct default)

    The bug this prevents: if NASTECH_HOME has already been mutated to
    /home/user/.nastech/profiles/webui (by init_profile_state at startup),
    reading it here would make _DEFAULT_NASTECH_HOME point to that subdir,
    causing switch_profile('webui') to look for
    /home/user/.nastech/profiles/webui/profiles/webui — which doesn't exist.

    NASTECH_BASE_HOME normally points at the base home already, but isolated
    ...
    base_override = os.getenv('NASTECH_BASE_HOME', '').strip()
    if base_override:
        return _unwrap_profile_home_to_base(Path(base_override).expanduser())

    nastech_home = os.getenv('NASTECH_HOME', '').strip()
    if nastech_home:
        p = Path(nastech_home).expanduser()
        # If NASTECH_HOME points to a profiles/ subdir, walk up two levels to the base
        return _unwrap_profile_home_to_base(p)

    # Platform default. On Windows this includes the #2905 migration-safety
    # fallback (prefer the populated legacy %USERPROFILE%.nastech over an
    # empty %LOCALAPPDATA%\nastech). Import the shared path helper directly
    ...
    from api.paths import _platform_default_nastech_home

    return _platform_default_nastech_home()
...
def _read_active_profile_file() -> str:
    ...
    ap_file = _DEFAULT_NASTECH_HOME / 'active_profile'
    if ap_file.exists():
        try:
...
#
# NasTech Agent allows the root/default profile (~/.nastech itself) to have a
# display name other than the legacy literal 'default'.  When that happens,
# WebUI must NOT resolve the display name as ~/.nastech/profiles/<name> — that
# directory doesn't exist, and every site that does `if name == 'default':`
# will fall through to the wrong filesystem path.
#
# `_is_root_profile(name)` answers "does this name resolve to ~/.nastech?" and
# is the canonical replacement for scattered `if name == 'default':` checks
# in switch_profile, get_active_nastech_home, _validate_profile_name, etc.
#
# Cost note: list_profiles_api() shells out via nastech_cli (non-trivial), so
...
def _is_root_profile(name: str) -> bool:
    """True if *name* resolves to the NasTech Agent root profile (~/.nastech).
    ...
    # avoid holding it across a nastech_cli subprocess call.
    try:
...
def get_active_profile_name() -> str:
    ...
    Priority:
      1. Thread-local (set per-request from nastech_profile cookie) — issue #798
...
def set_request_profile(name: str) -> None:
    ...
    Called by server.py at the start of each request when a nastech_profile
...
def _resolve_profile_home_for_name(name: str) -> Path:
    """Resolve a logical profile name to its NasTech home path.

    Root/default aliases resolve to _DEFAULT_NASTECH_HOME.  Valid named profiles
    resolve to _DEFAULT_NASTECH_HOME/profiles/<name> even when the directory has
    ...
    if not name or _is_root_profile(name):
        return _DEFAULT_NASTECH_HOME
    if not _PROFILE_ID_RE.fullmatch(name):
        return _DEFAULT_NASTECH_HOME
    return _resolve_named_profile_home(name)


def get_active_nastech_home() -> Path:
    """Return the NASTECH_HOME path for the currently active profile.
...
# `cron.jobs` reads NASTECH_HOME from os.environ (process-global) at function-
...
# This context manager swaps NASTECH_HOME (and the cached module-level constants
...
# all webui access to NASTECH_HOME goes through one thread at a time. Any
...
# (own-profile) NASTECH_HOME, never a half-swapped state.
...
def _home_for_scheduled_cron_job(job: dict) -> Path:
    ...
    Jobs pinned to a named profile execute under that profile's NASTECH_HOME, so
    ...
    if not raw:
        return get_active_nastech_home()
...
def install_cron_scheduler_profile_isolation() -> None:
    ...
    NASTECH_HOME isolation as the manual /api/crons/run path.
    """
    try:
...
class cron_profile_context_for_home:
    """Context manager that pins NASTECH_HOME to an explicit profile home path.
    ...
    def __enter__(self):
        ...
        try:
            self._prev_env = os.environ.get('NASTECH_HOME')
            os.environ['NASTECH_HOME'] = str(self._home)
            ...
            try:
                ...
                self._prev_cj = (_cj.NASTECH_DIR, _cj.CRON_DIR, _cj.JOBS_FILE, _cj.OUTPUT_DIR)
                _cj.NASTECH_DIR = self._home
            ...
            # cron.scheduler snapshots _nastech_home at import time and run_job()
            ...
            try:
                import cron.scheduler as _cs
                self._prev_cs = (
                    getattr(_cs, '_nastech_home', None),
                    getattr(_cs, '_LOCK_DIR', None),
                    getattr(_cs, '_LOCK_FILE', None),
                )
                _cs._nastech_home = self._home
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        try:
            if self._prev_env is None:
                ...
                os.environ['NASTECH_HOME'] = self._prev_env
            if self._prev_cj is not None:
                try:
                    import cron.jobs as _cj
                    _cj.NASTECH_DIR, _cj.CRON_DIR, _cj.JOBS_FILE, _cj.OUTPUT_DIR = self._prev_cj
                except (ImportError, AttributeError):
                    pass
            if getattr(self, '_prev_cs', None) is not None:
                try:
                    import cron.scheduler as _cs
                    _cs._nastech_home, _cs._LOCK_DIR, _cs._LOCK_FILE = self._prev_cs
                except (ImportError, AttributeError):
                    pass
        finally:
            _pop_cron_profile_context_depth()
...
class cron_profile_context:
    """Context manager that pins NASTECH_HOME to the TLS-active profile.
...
def get_nastech_home_for_profile(name: str) -> Path:
    """Return the NASTECH_HOME Path for *name* without mutating any process state.
    ...
    Falls back to _DEFAULT_NASTECH_HOME (same as 'default') when *name* is None,
...
def get_profile_runtime_env(home: Path) -> dict[str, str]:
    ...
    environment variables (matching ``nastech -p <profile>``), so streaming must
...
def profile_env_for_background_worker(
    ...
    try:
        ...
        profile_home_path = Path(get_nastech_home_for_profile(profile))
    ...
    thread_env["NASTECH_HOME"] = str(profile_home_path)
    ...
    # worker body because several production NasTech readers still call
    ...
    old_nastech_home = None
    had_nastech_home = False
    ...
    try:
        ...
        with _ENV_LOCK:
            ...
            had_nastech_home = "NASTECH_HOME" in os.environ
            old_nastech_home = os.environ.get("NASTECH_HOME")
            skill_home_snapshot = snapshot_skill_home_modules()
            os.environ.update(runtime_env)
            os.environ["NASTECH_HOME"] = str(profile_home_path)
            try:
        ...
        with _ENV_LOCK:
            ...
            if had_nastech_home:
                os.environ["NASTECH_HOME"] = old_nastech_home or ""
            else:
                os.environ.pop("NASTECH_HOME", None)
...
def profile_env_for_active_request(
    ...
    on a named profile sets a ``nastech_profile`` cookie, which ``server.py``
    ...
    ``os.environ`` / ``NASTECH_HOME`` — ``/api/providers`` (``get_auth_status``
...
def _set_nastech_home(home: Path):
    """Set NASTECH_HOME env var and monkey-patch cached module-level paths."""
    os.environ['NASTECH_HOME'] = str(home)
...
def switch_profile(name: str, *, process_wide: bool = True) -> dict:
    ...
    # Process-wide profile switches mutate NASTECH_HOME, module-level path caches,
...
def _build_profile_rows_fast() -> list | None:
    ...
    ``nastech_cli.profiles.list_profiles()`` calls ``find_alias_for_profile()``
    ...
    if default_home.is_dir():
        ...
        # the directory is literally ".nastech" — match that exactly.
...
def list_profiles_api() -> list:
    ...
    if rows is None:
        ...
        # or the default-only dict if nastech_cli isn't importable at all.
        logger.debug(
...
def _default_profile_dict() -> dict:
    """Fallback profile dict when nastech_cli is not importable."""
    enabled_count, compatible_count = _get_profile_skills_stats(_DEFAULT_NASTECH_HOME)
    return {
        'name': 'default',
        'path': str(_DEFAULT_NASTECH_HOME),
        'is_default': True,
        'is_active': True,
        'gateway_running': False,
        'model': None,
        'provider': None,
        'has_env': (_DEFAULT_NASTECH_HOME / '.env').exists(),
        'visible': True,
        'skill_count': enabled_count,
        'enabled_skills': enabled_count,
...
def _validate_profile_name(name: str):
    """Validate profile name format (matches nastech_cli.profiles upstream)."""
    if name == 'default':
...
def _profiles_root() -> Path:
    ...
    return (_DEFAULT_NASTECH_HOME / 'profiles').resolve()
...
def _resolve_named_profile_home(name: str) -> Path:
    ...
    final filesystem path and enforces containment under ~/.nastech/profiles.
    """
...
def _create_profile_fallback(name: str, clone_from: str = None,
    ...
    """Create a profile directory without nastech_cli (Docker/standalone fallback)."""
    profile_dir = _DEFAULT_NASTECH_HOME / 'profiles' / name
    ...
    if clone_config and clone_from:
        if _is_root_profile(clone_from):
            source_dir = _DEFAULT_NASTECH_HOME
        else:
            source_dir = _DEFAULT_NASTECH_HOME / 'profiles' / clone_from
...
# the key must be written to the profile's .env file so that NasTech Agent's
...
def _write_api_key_to_dotenv(
    ...
    ``NASTECH_API_KEY`` that the user can rename later.
    """
    env_var = _resolve_env_var_for_provider(model_provider)
    if not env_var:
        env_var = "NASTECH_API_KEY"
        logger.info(
...
def create_profile_api(name: str, clone_from: str = None,
    ...
    # nastech_cli and the webui runtime do not always agree on the exact root,
    ...
    profile_path = _DEFAULT_NASTECH_HOME / 'profiles' / name
    ...
    # When nastech_cli is not importable, list_profiles_api() also falls back
