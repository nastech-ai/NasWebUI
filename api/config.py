...
HOME = _paths.HOME
_nastech_home_has_webui_state = _paths._nastech_home_has_webui_state
_platform_default_nastech_home = _paths._platform_default_nastech_home
...
# ── NasTech agent directory discovery ─────────────────────────────────────────
def _discover_agent_dir() -> Path:
    ...
    Priority:
      ...
      5. Common install paths            -- ~/.nastech/NasTech-Agent (again as fallback)
      6. HOME / NasTech-Agent             -- ~/NasTech-Agent (simple flat layout)
    """
    candidates = []
    ...
    nastech_home = os.getenv("NASTECH_HOME", str(_DEFAULT_NASTECH_HOME))
    candidates.append(Path(nastech_home).expanduser() / "NasTech-Agent")

    # 3. Sibling: <repo-root>/../NasTech-Agent
    candidates.append(REPO_ROOT.parent / "NasTech-Agent")

    # 4. Parent is the agent repo itself (repo cloned inside NasTech-Agent/)
    if (REPO_ROOT.parent / "run_agent.py").exists():
        candidates.append(REPO_ROOT.parent)

    # 5. ~/.nastech/NasTech-Agent (explicit common path)
    candidates.append(_DEFAULT_NASTECH_HOME / "NasTech-Agent")

    # 6. ~/NasTech-Agent
    candidates.append(HOME / "NasTech-Agent")

    # 7. XDG_DATA_HOME / NasTech-Agent  (e.g. ~/.local/share/NasTech-Agent)
    xdg_data = Path(os.getenv("XDG_DATA_HOME", str(HOME / ".local" / "share")))
    candidates.append(xdg_data.expanduser() / "NasTech-Agent")

    # 8. System-wide install paths (e.g. /opt/NasTech-Agent, /usr/local/NasTech-Agent)
    for sys_prefix in ("/opt", "/usr/local", "/usr/local/share"):
        candidates.append(Path(sys_prefix) / "NasTech-Agent")
...
# ── Inject agent dir into sys.path so NasTech modules are importable ──────────

# When users (or CI builds) run `pip install --target .` or
# `pip install -t .` inside the NasTech-Agent checkout, third-party
# package directories (openai/, pydantic/, requests/, etc.) end up
# alongside real NasTech source files.  Putting _AGENT_DIR at the
...
# and NasTech-specific modules (run_agent, nastech/, etc.) still
...
def _get_config_path() -> Path:
    ...
    env_override = os.getenv("NASTECH_CONFIG_PATH")
    if env_override:
        return Path(env_override).expanduser()
    try:
        from api.profiles import get_active_nastech_home

        return get_active_nastech_home() / "config.yaml"
    except ImportError:
        return _DEFAULT_NASTECH_HOME / "config.yaml"
...
def get_config_for_profile_home(profile_home: "Path | str | None") -> dict:
    ...
    the per-request thread-local profile context (set from the ``nastech_profile``
...
def _save_yaml_config_file(config_path: Path, config_data: dict) -> None:
    try:
        ...
        raise RuntimeError("PyYAML is required to write NasTech config.yaml") from exc
...
def print_startup_config() -> None:
    ...
    if not _NASTECH_FOUND:
        print(
            f"{err}  Could not find the NasTech agent directory.\n"
            ...
            "        git clone <NasTech-Agent-repo> ../NasTech-Agent\n",
            flush=True,
        )


def verify_nastech_imports() -> tuple:
    """
    Attempt to import the key NasTech modules.
...
_LEGACY_CLI_TOOLSET_ALIASES = {
    # Older NasTech configs used "nastech" as the CLI composite toolset. Modern
    # NasTech Agent exposes that split as these two registered composites; keep
    # WebUI sessions usable when pointed at an older shared config.yaml.
    "nastech": ("nastech-cli", "nastech-api-server"),
}
...
_FALLBACK_MODELS = [
    ...
    # are visible even when the tool-support filter in nastech_cli strips
...
# Provider display names for known NasTech provider IDs
_PROVIDER_DISPLAY = {
    "nous": "Nous Portal",
...
#
# This table is authoritative for the WebUI.  When ``nastech_cli.models``
...
# not on ``sys.path`` (CI, installs without NasTech-Agent cloned
# alongside the WebUI).
_PROVIDER_ALIASES = {
    "glm": "zai",
    "z-ai": "zai",
    ...
    # ``nastech_cli.auth.PROVIDER_REGISTRY``. Routing it through ``custom``
...
def _resolve_provider_alias(name: str) -> str:
    ...
    additional aliases the agent provides (when nastech_cli is on
    ...
    try:
        from nastech_cli.models import _PROVIDER_ALIASES as _agent_aliases
...
def _canonicalise_provider_id(name: object) -> str:
    ...
    (the nastech_cli alias target which the WebUI doesn't index by).
    ...
    # at non-canonical strings (legacy, nastech_cli-specific) from leaking
...
def _provider_is_known_or_configured(
    ...
    """True when ``provider_id`` is a provider NasTech recognizes (static registry)
    ...
    This distinguishes a provider NasTech knows how to route (e.g. ``ollama-cloud``,
...
_PROVIDER_MODELS = {
    ...
    # 'gemini' is the nastech_cli provider ID for Google AI Studio
    ...
    # nastech_cli.models.provider_model_ids("bedrock") when available (#2720).
    "bedrock": [
...
_LOCAL_SERVER_PROVIDERS = {
    "lmstudio",     # canonical (in nastech_cli.models.CANONICAL_PROVIDERS)
...
def get_effective_default_model(config_data: dict | None = None) -> str:
    """Resolve the effective NasTech default model from config, then env overrides."""
    ...
    env_model = (
        os.getenv("NASTECH_MODEL") or os.getenv("OPENAI_MODEL") or os.getenv("LLM_MODEL")
    )
    if env_model:
...
# Mirrors nastech_constants.parse_reasoning_effort so WebUI can validate without
...
def _heuristic_reasoning_efforts(model_id: str, provider_id: str) -> list[str]:
    """Fallback when nastech_cli is unavailable."""
...
def _models_dev_reasoning_efforts(model_id: str, provider_id: str) -> list[str] | None:
    """Return reasoning efforts from NasTech Agent model metadata when known.
...
def set_nastech_default_model(model_id: str) -> dict:
    """Persist the NasTech default model in config.yaml and reload runtime config."""
    ...
    with _cfg_lock:
        ...
        # prefix is a WebUI-internal routing hint that the NasTech-Agent CLI does
        # not understand — if we wrote `@nous:anthropic/claude-opus-4.6` to
        # config.yaml, a user who ran `nastech` in the terminal right after
...
# Canonical auxiliary task slots. Keep in sync with nastech_cli/config.py
# DEFAULT_CONFIG["auxiliary"] and nastech_cli/web_server.py _AUX_TASK_SLOTS.
...
def _get_auth_store_path() -> Path:
    """Return the auth.json path for the active NasTech profile."""
    try:
        from api.profiles import get_active_nastech_home as _gah

        return _gah() / "auth.json"
    except ImportError:
        return _DEFAULT_NASTECH_HOME / "auth.json"
...
def _read_live_provider_model_ids(provider_id: str) -> list[str]:
    ...
    groups should ask ``nastech_cli.models.provider_model_ids()`` first (#1240).
    Provider aliases are tried as a secondary lookup because WebUI keeps a few
    display-facing IDs (for example ``google`` / ``x-ai``) that NasTech CLI may
    ...
    if not pid:
        return []
    try:
        from nastech_cli.models import provider_model_ids as _provider_model_ids
    except Exception:
        return []
    ...
    for candidate in candidates:
        try:
            ...
            logger.debug("Failed to load %s models from nastech_cli", candidate)
            continue
        result: list[str] = []
        for mid in live_ids:
...
def _models_from_live_provider_ids(provider_id: str, live_ids: list[str]) -> list[dict]:
    """Convert NasTech CLI model ids into WebUI picker model entries."""
...
def get_available_models(*, prefer_cache: bool = False) -> dict:
    ...
    Discovery order:
      ...
      2. Check for known API keys in env or ~/.nastech/.env
    ...
    def _build_available_models_uncached() -> dict:
        ...
        _nastech_auth_used = False
        try:
            from nastech_cli.models import list_available_providers as _lap
            from nastech_cli.auth import get_auth_status as _gas
            ...
            # with `get_auth_status(<id>).logged_in` on some nastech_cli versions
            ...
            logger.debug("Failed to detect auth providers from nastech")

        if not _nastech_auth_used:
            try:
                from api.profiles import get_active_nastech_home as _gah2

                nastech_env_path = _gah2() / ".env"
            except ImportError:
                nastech_env_path = _DEFAULT_NASTECH_HOME / ".env"
            env_keys = {}
            if nastech_env_path.exists():
                try:
                    for line in nastech_env_path.read_text(encoding="utf-8").splitlines():
                        line = line.strip()
                        if line and not line.startswith("#") and "=" in line:
                            k, v = line.split("=", 1)
                            env_keys[k.strip()] = v.strip().strip('"').strip("'")
                except Exception:
                    logger.debug("Failed to parse nastech env file")
            all_env = {**env_keys}
            for k in (
                "ANTHROPIC_API_KEY",
                "OPENAI_API_KEY",
            ...
            if all_env.get("OPENAI_API_KEY"):
                # NasTech-Agent registers its OPENAI_API_KEY/OPENAI_BASE_URL provider
            ...
            # LM Studio: detect via LM_API_KEY + LM_BASE_URL in ~/.nastech/.env
                ...
                try:
                    ...
                    if addr.is_private or addr.is_loopback or addr.is_link_local:
                        if "ollama" in host or "127.0.0.1" in host or "localhost" in host:
                            ...
                            # ``nastech_cli.auth.PROVIDER_REGISTRY`` — see #1384.
                            provider = "custom"
                except ValueError:
                    pass
        ...
        if isinstance(_custom_providers_cfg, list):
            ...
            for _cp in _custom_providers_cfg:
                ...
                if _slug and _cp_base_url:
                    ...
                    # discovery (same as NasTech-Agent model_switch.py Section 4
        ...
        if detected_providers:
            for pid in sorted(detected_providers):
                ...
                if pid == "openrouter":
                    ...
                    #   (1) curated tool-supporting catalog via nastech_cli.models.fetch_openrouter_models()
                    ...
                    try:
                        ...
                        logger.warning("Failed to load OpenRouter curated catalog from nastech_cli")
                    ...
                    try:
                        ...
                        logger.warning("Failed to load Ollama Cloud models from nastech_cli")
                    ...
                    try:
                        ...
                        logger.warning("Failed to load OpenAI Codex models from nastech_cli")
                    ...
                    # live-fetch through nastech_cli.models.provider_model_ids()
                    ...
                    try:
                        ...
                        logger.warning("Failed to load Nous Portal models from nastech_cli")
                        live_ids = []
                    ...
                    # CI environments where nastech_cli isn't importable —
                    ...
                    try:
                        ...
                        logger.debug("nastech_cli LM Studio lookup unavailable; using urlopen fallback")
                    ...
                    # Otherwise, ask NasTech CLI first so WebUI tracks the same
    ...
    with _available_models_cache_lock:
        ...
        # NASTECH_HOME and the disk-cache path/fingerprint from the profile TLS;
...
# subprocess via NASTECH_SESSION_KEY) back to the WebUI session_id that owns it,
...
try:
    ...
    pass  # nastech_cli not available -- default profile only
