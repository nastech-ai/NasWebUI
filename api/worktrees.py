"""Helpers for WebUI-managed NasTech Agent git worktrees."""
...
def _setup_agent_worktree(repo_root: str) -> dict:
    try:
        import api.config  # noqa: F401  # ensure NasTech Agent dir is on sys.path
        from cli import _setup_worktree
    except Exception as exc:
        raise RuntimeError("NasTech Agent worktree helper is unavailable") from exc
    output = StringIO()
    with redirect_stdout(output), redirect_stderr(output):
        info = _setup_worktree(repo_root)
    emitted = output.getvalue().strip()
    if emitted:
        logger.debug("NasTech Agent worktree helper output: %s", emitted)
    if not info:
        raise RuntimeError("NasTech Agent failed to create a git worktree")
    return info
...
def create_worktree_for_workspace(workspace: str | Path) -> dict:
    ...
    if not path or not branch:
        raise RuntimeError("NasTech Agent returned incomplete worktree metadata")
    return {
