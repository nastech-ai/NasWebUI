...
def _get_state_db(profile: Optional[str] = None):
    ...
    ``NASTECH_HOME`` — silently routing the write to the wrong DB
    would defeat the point of the explicit-profile path (#2762).

    When ``profile`` is None it falls back to the TLS-based
    ``get_active_nastech_home()`` lookup for backward compatibility,
    with a final ``NASTECH_HOME`` fallback only on that path. TLS may be
    ...
    Returns None if nastech_state is not importable, the explicit
    ...
    if profile is not None:
        ...
        # silently fall back to NASTECH_HOME or the caller's "write to
        ...
        # quietly returns ``_DEFAULT_NASTECH_HOME``, which is the exact
        ...
        try:
            ...
            if not (_is_root_profile(profile) or _PROFILE_ID_RE.fullmatch(profile)):
                ...
                return None
            nastech_home = Path(_resolve_profile_home_for_name(profile)).expanduser().resolve()
        except Exception:
            logger.warning(
        ...
        try:
            ...
            logger.debug("Failed to resolve nastech home, using default")
            nastech_home = Path(os.getenv('NASTECH_HOME', str(Path.home() / '.nastech')))
