"""WebUI bridge for NasTech persistent session goals."""
...
try:  # Exposed as a module attribute so tests can monkeypatch it directly.
    from nastech_cli.goals import (  # type: ignore
...
except Exception:  # pragma: no cover - depends on installed NasTech-Agent
...
def _default_max_turns() -> int:
    """Return the configured /goal turn budget, defaulting to NasTech' 20 turns."""
    try:
...
def _profile_db(profile_home: str | Path):
    """Return a SessionDB pinned to *profile_home*, without reading NASTECH_HOME.

    The upstream NasTech GoalManager persists through nastech_cli.goals.load_goal(),
    which resolves SessionDB from process-global NASTECH_HOME. WebUI sessions are
    ...
    if cached is not None:
        return cached
    try:
        from nastech_state import SessionDB  # type: ignore
...
class _ProfileGoalManager:
    ...
    def __init__(self, session_id: str, *, profile_home: str | Path, default_max_turns: int = 20):
        if GoalState is None:
            raise RuntimeError("NasTech goal state unavailable")
...
def restore_goal_state(session_id: str, snapshot: Any, *, profile_home: str | Path | None = None) -> None:
    ...
    if isinstance(mgr, _ProfileGoalManager):
        ...
        return
    try:
        from nastech_cli.goals import save_goal  # type: ignore
