...
def _active_skills_dir() -> Path:
    """Return the skills directory for the request's active NasTech profile.
    ...
    ``get_active_nastech_home()`` for every request instead of reading that
    process-global constant.
    """
    try:
        from api.profiles import get_active_nastech_home

        return Path(get_active_nastech_home()) / "skills"
    except Exception:
        try:
            ...
            return Path(os.getenv("NASTECH_HOME", str(Path.home() / ".nastech"))).expanduser() / "skills"
...
def _active_profile_config_path() -> Path:
    ...
    NasTech home, not process-global NASTECH_HOME or NASTECH_CONFIG_PATH values
    ...
    try:
        ...
        return Path(get_active_nastech_home()) / "config.yaml"
    except Exception:
        return _get_config_path()
...
def _get_disabled_skill_names_for_profile() -> set:
    ...
    the process-global ``NASTECH_HOME``, this uses ``_get_config_path()`` which
...
def _gateway_session_metadata_path():
    try:
        from api.profiles import get_active_nastech_home
        nastech_home = Path(get_active_nastech_home()).expanduser().resolve()
    except Exception:
        nastech_home = Path(os.getenv("NASTECH_HOME", str(Path.home() / ".nastech"))).expanduser().resolve()
    return nastech_home / "sessions" / "sessions.json"
...
def _profile_home_for_cron_job(job: dict):
    ...
    """
    from api.profiles import get_active_nastech_home, get_nastech_home_for_profile
    ...
    if raw not in _available_cron_profile_names():
        ...
        return get_active_nastech_home()
    return get_nastech_home_for_profile(raw)
...
def _run_cron_job_in_profile_subprocess(job, execution_profile_home):
    ...
    cron.scheduler/cron.jobs still rely on process-global NASTECH_HOME and module
    ...
    try:
        ...
        def _persist_success():
            ...
            try:
                ...
                # not expose the newer delivery_error parameter. Real NasTech
...
# Used as fallback when nastech_cli.provider_model_ids() is unavailable or
...
# specific model filtering happens downstream in nastech_cli.
#
...
def _read_profile_model_config(
    ...
    if _clean_session_model_provider(requested_provider) or not getattr(session, "profile", None):
        return None, None
    try:
        from api.profiles import get_nastech_home_for_profile
        _profile_home = get_nastech_home_for_profile(session.profile)
...
def _resolve_context_length_for_session_model(
    ...
    try:
        ...
        try:
            ...
            # Older NasTech-Agent builds: legacy 2-arg form.
...
def _should_hide_stale_messaging_session(
    ...
    NasTech Gateway keeps the external conversation identity in sessions.json.
...
def _handle_logs(handler, parsed) -> bool:
    """Return a bounded tail window for an active-profile NasTech log file."""
    ...
    try:
        ...
        nastech_home = Path(get_active_nastech_home()).expanduser()
    except Exception:
        nastech_home = Path(os.environ.get("NASTECH_HOME") or (Path.home() / ".nastech")).expanduser()
...
_LLM_WIKI_DOCS_URL = "https://NasTech-Agent.nousresearch.com/docs/user-guide/skills/bundled/research/research-llm-wiki"
_LLM_WIKI_PAGE_DIRS = ("entities", "concepts", "comparisons", "queries")


def _llm_wiki_active_nastech_home() -> Path:
    try:
        from api.profiles import get_active_nastech_home
        return Path(get_active_nastech_home()).expanduser()
    except Exception:
        return Path(os.getenv("NASTECH_HOME", str(Path.home() / ".nastech"))).expanduser()


def _llm_wiki_env_file_path(nastech_home: Path) -> str | None:
...
def _llm_wiki_resolve_path() -> tuple[Path, str, bool]:
    nastech_home = _llm_wiki_active_nastech_home()
    raw = os.getenv("WIKI_PATH") or _llm_wiki_env_file_path(nastech_home)
...
def _build_llm_wiki_status() -> dict:
    ...
    try:
        ...
        base = {
            ...
            "toggle_reason": "NasTech Agent exposes WIKI_PATH/wiki.path for location, but no stable on/off config flag is currently available.",
...
def _handle_insights(handler, parsed) -> bool:
    ...
    # ── Also include CLI sessions from NasTech state.db ─────────────────────
    try:
...
def _deep_health_checks(stream_check: dict | None = None) -> tuple[dict, bool]:
    ...
    shared stream map, sidebar/session path, project state, and NasTech state.db
...
def _get_plugin_manager_for_visibility():
    """Return NasTech Agent's plugin manager for read-only WebUI visibility."""
...
def _plugin_visibility_payload(manager=None) -> dict:
    ...
    The NasTech Agent manager stores manifests and callback objects internally.
    ...
    # NasTech Agent lifecycle-hook plugins
...
<head>
  ...
  <title>NasTech is restarting</title>
</head>
<body style=\"margin:0;padding:2rem;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#111827;color:#e5e7eb;\">
  <main style=\"max-width:40rem;margin:10vh auto;line-height:1.5;\">
    <h1 style=\"font-size:1.5rem;margin:0 0 0.75rem;\">NasTech is restarting…</h1>
...
def _saved_prompts_path() -> "Path":
    try:
        from api.profiles import get_active_nastech_home
        return Path(get_active_nastech_home()).expanduser() / "webui" / "saved_prompts.json"
    except Exception:
        return Path(os.getenv("NASTECH_HOME", str(Path.home() / ".nastech"))).expanduser() / "webui" / "saved_prompts.json"
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/login":
        ...
        _bn = _html.escape(_settings.get("bot_name") or "NasTech")
    ...
    if parsed.path == "/api/session":
        import time as _time
        _t0 = _time.monotonic()
        _debug_slow = os.environ.get("NASTECH_DEBUG_SLOW", "")
        query = parse_qs(parsed.query)
        sid = query.get("session_id", [""])[0]
        if not sid:
    ...
    if parsed.path == "/api/personalities":
        ...
        # (matches NasTech-Agent CLI behavior, not filesystem SOUL.md approach)
            ...
            return j(
                handler,
                {
                    ...
                    "agent": {
                        ...
                        "latest_sha": "bbb0002",
                        "branch": "master",
                        "repo_url": "https://github.com/NousResearch/NasTech-Agent",
                        "compare_url": "https://github.com/NousResearch/NasTech-Agent/compare/aaa0001...bbb0002",
                    },
                    "checked_at": 0,
                },
            )
    ...
    # ── Cron API (GET) ──
    # All cron handlers touch cron.jobs which resolves NASTECH_HOME from
    ...
    if parsed.path == "/api/profile/active":
        ...
        return j(
            handler,
            {
                ...
                "path": str(get_active_nastech_home()),
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/default-model":
        try:
            return j(handler, set_nastech_default_model(body.get("model")))
        except ValueError as e:
            return bad(handler, str(e))
    ...
    if parsed.path == "/api/model/set":
        ...
        if scope == "main":
            try:
                return j(handler, set_nastech_default_model(model))
            except ValueError as exc:
                return bad(handler, str(exc), status=400)
    ...
    if parsed.path == "/api/personality/set":
        ...
        # (matches NasTech-Agent CLI behavior)
        prompt = ""
        if name:
            ...
            # Resolve prompt using the same logic as NasTech-Agent cli.py
            if isinstance(value, dict):
    ...
    if parsed.path == "/api/settings":
        ...
        if "bot_name" in body:
            body["bot_name"] = (str(body["bot_name"]) or "").strip() or "NasTech"
        ...
        def _llm_update_summary(system_prompt: str, user_prompt: str) -> str:
            ...
            with profiles_api.profile_env_for_background_worker(
                ...
                try:
                    ...
                    # discover in the NasTech Agent setup/config UI.
...
def _handle_session_export(handler, parsed):
    ...
    handler.send_header("Content-Type", "application/json; charset=utf-8")
    handler.send_header(
        "Content-Disposition", f'attachment; filename="nastech-{sid}.json"'
    )
...
def _handle_media(handler, parsed):
    ...
    Security:
    - Path must resolve to an allowed root (nastech home, /tmp, common dirs)
    ...
    _NASTECH_HOME = Path(_os.getenv("NASTECH_HOME", str(_HOME / ".nastech"))).expanduser()
    ...
    # Allowed roots: nastech home, /tmp, and active workspace.
    ...
    # ── #3234: hard-deny NasTech's own state + secret/config files ────────────
    # The allowlist above grants the whole NasTech home (and base ~/.nastech), so
    ...
    # NasTech root. The deny rules target NasTech's OWN internal state, which lives
    ...
    # the internal state subdirectories across every NasTech root the allowlist
    # accepts (active-profile NASTECH_HOME, base ~/.nastech, the api.profiles
    ...
    try:
        from api.profiles import _DEFAULT_NASTECH_HOME as _BASE_HH
    ...
    # State-subdir deny set: each DENY_SUBDIR directly under any NasTech root
    ...
    # location ($HOME, a NasTech root itself, an ANCESTOR of a NasTech root, a
    ...
    # NasTech root (e.g. STATE_DIR/workspace) is still a legit project workspace
...
def _handle_live_models(handler, parsed):
    ...
    try:
        ...
        # works even when nastech_cli is not on sys.path.
        from api.config import _resolve_provider_alias
        provider = _resolve_provider_alias(provider)
        ...
        try:
            ...
            _agent_dir = _os.path.join(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))),
                                       "..", "..", ".nastech", "NasTech-Agent")
            _agent_dir = _os.path.normpath(_agent_dir)
            if _agent_dir not in _sys.path:
                _sys.path.insert(0, _agent_dir)
            from nastech_cli.models import provider_model_ids as _pmi
            ids = _pmi(provider)
        ...
        if not ids:
            ...
            # returns [] because they aren't real nastech_cli endpoints.
...
def _handle_goal_command(handler, body):
    ...
    try:
        ...
        profile_home = get_nastech_home_for_profile(getattr(s, "profile", None))
    except Exception:
        profile_home = None
...
def _normalize_chat_attachments(raw_attachments):
    ...
    to NasTech as native multimodal inputs for the current turn.
    """
    normalized = []
...
def _handle_chat_sync(handler, body):
    ...
    with _ENV_LOCK:
        ...
        os.environ["NASTECH_SESSION_KEY"] = s.session_id
    try:
        from run_agent import AIAgent
        ...
        with _ENV_LOCK:
            ...
            if old_exec_ask is None:
                ...
                os.environ["NASTECH_EXEC_ASK"] = old_exec_ask
            if old_session_key is None:
                os.environ.pop("NASTECH_SESSION_KEY", None)
            else:
                os.environ["NASTECH_SESSION_KEY"] = old_session_key
    with _get_session_agent_lock(s.session_id):
...
def _handle_cron_run(handler, body):
    ...
    #
    # Resolve directly without a try/except: get_active_nastech_home() does
    ...
    # run unpinned against the process-global NASTECH_HOME — so we'd
    # rather let any unexpected exception 500 the request than corrupt
    # cross-profile state.
    from api.profiles import get_active_nastech_home

    _profile_home = get_active_nastech_home()
...
def _handle_session_compress(handler, body):
    ...
    try:
        ...
        from api.oauth import resolve_runtime_provider_with_anthropic_env_lock
        import nastech_cli.runtime_provider as _runtime_provider
...
def _handle_handoff_summary(handler, body):
    ...
    def _agent_text_completion(agent, system_prompt, user_text, max_tokens=700):
        """Use the current NasTech Agent transport without mutating conversation history."""
        api_messages = [
            {"role": "system", "content": system_prompt},
...
def _handle_skill_toggle(handler, body):
    ...
    ``NASTECH_SESSION_PLATFORM`` is set).
    """
    try:
    ...
    with _cfg_lock:
        ...
        # platform-specific list first when NASTECH_SESSION_PLATFORM=webui).
        platform_disabled = skills_cfg.get("platform_disabled")
        if isinstance(platform_disabled, dict) and "webui" in platform_disabled:
...
def _parse_mcp_enabled(value) -> bool:
    """Parse NasTech MCP ``enabled`` values without raising on bad config."""
    if value is None:
        return True
...
def _mcp_runtime_status_by_name() -> dict[str, dict]:
    ...
    configuration; it does not probe or spawn MCP subprocesses. If NasTech Agent
