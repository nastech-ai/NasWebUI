...
def _profile_state_dir() -> Path:
    ...
    """
    try:
        from api.profiles import get_active_profile_name, get_active_nastech_home
        name = get_active_profile_name()
        if name and name != 'default':
            d = get_active_nastech_home() / 'webui_state'
...
def _profile_default_workspace() -> str:
    ...
    Checks keys in priority order:
      ...
      3. 'terminal.cwd'      — NasTech-Agent terminal working dir (most common)
...
def _clean_workspace_list(workspaces: list) -> list:
    ...
    - Remove entries whose paths live inside another profile's directory
      (e.g. ~/.nastech/profiles/X/... should not appear on a different profile).
    - Rename any entry whose name is literally 'default' to 'Home' (avoids
      confusion with the 'default' profile name).
    Returns the cleaned list (may be empty).
    """
    nastech_profiles = (Path.home() / '.nastech' / 'profiles').resolve()
    result = []
    for w in workspaces:
        path = w.get('path', '')
        name = w.get('name', '')
        ...
        # created under ~/.nastech/profiles/webui/webui-mvp-test/).
        try:
            p.relative_to(nastech_profiles)
            # p is under ~/.nastech/profiles/ — only skip if it's under a DIFFERENT profile
            try:
                from api.profiles import get_active_nastech_home
                own_profile_dir = get_active_nastech_home().resolve()
...
def _workspace_access_error(candidate: Path, *, missing_label: str = "Path does not exist") -> str | None:
    ...
    try:
        ...
        return (
            ...
            f"Folders permission to the NasTech/WebUI app or server process, then try again."
        )
