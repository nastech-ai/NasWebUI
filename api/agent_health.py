"""NasTech agent/gateway heartbeat payload helpers (#716, #1879).

The WebUI process is not always paired with a long-running NasTech gateway. Some
setups use WebUI only, while self-hosted messaging deployments run a separate
NasTech gateway daemon that records runtime metadata in the NasTech Agent home.
...
where the WebUI runs separately from ``NasTech-Agent`` and only a NasTech data
...
``pid: "service:NasTech-Agent"`` compose workaround, we accept a recent
...
def _gateway_root_pid_path() -> Path | None:
    """Return the root NasTech gateway PID path.

    Gateway runtime files are root-level singletons.  A profile-scoped WebUI
    process may have NASTECH_HOME=<root>/profiles/<name>, but gateway.pid,
    ...
    try:
        ...
        try:
            ...
            profile_pid = Path(get_active_nastech_home()) / _GATEWAY_PID_FILE
...
# ``gateway_not_configured`` even though ``NASTECH_API_URL`` points at a perfectly
...
def build_agent_health_payload() -> dict[str, Any]:
    """Return `{alive, checked_at, details}` for the NasTech gateway/agent.
    ...
    # Multi-container deployments (#3281): when NASTECH_API_URL is set the
