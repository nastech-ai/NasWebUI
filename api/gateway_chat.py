"""Default-off NasTech Gateway bridge for browser-originated chat turns."""
...
def webui_chat_backend_mode(config_data=None, environ: dict[str, str] | None = None) -> str:
    ...
    values opt browser chat into the NasTech API server bridge; generic truthy
...
def _gateway_tool_progress_event(payload: dict) -> tuple[str, dict] | None:
    """Translate NasTech Gateway tool-progress SSE payloads to WebUI events."""
...
def _run_gateway_chat_streaming(
    ...
    *,
    ...
    """Bridge a WebUI chat turn through NasTech Gateway's API server.
    ...
    try:
        ...
        if api_key:
            ...
            headers["X-NasTech-Session-Key"] = f"webui:{session_id}"
        ...
        if not assistant_text:
            put_gateway_event("apperror", {
                ...
                "hint": "Check that NasTech Gateway API server is running and reachable.",
            })
            return
