"""Shared helpers for reading NasTech Agent sessions from state.db."""
import logging
import sqlite3
...
def normalize_agent_session_source(raw_source: str | None) -> dict:
    """Return stable source metadata for NasTech Agent session rows.
...
def _is_continuation_session(parent: dict | None, child: dict | None) -> bool:
    ...
    by ``nastech -c`` also records a new child session; for sidebar projection it
...
def read_importable_agent_session_rows(
    ...
    NasTech Agent can create rows in ``state.db.sessions`` before a session has
    ...
    sidebar. This mirrors NasTech Agent CLI's session-list behaviour: interactive
    ...
    with closing(sqlite3.connect(str(db_path))) as conn:
        ...
        # Older NasTech Agent versions may not have source tracking. Without a
        ...
        if 'source' not in session_cols:
            log.warning(
                ...
                "(older NasTech-Agent?). Agent sessions unavailable. "
                "Upgrade NasTech-Agent to fix this.",
                db_path,
            )
            return []
        ...
        # A normally-migrated NasTech-Agent state.db has it, but a db that lost its
        # migrations (older NasTech-Agent, or a hand-rebuilt/reimported db) does
        ...
        if limit is not None:
            ...
            # aggregating every historical NasTech state.db session before
...
def read_session_lineage_metadata(db_path: Path, session_ids: list[str] | set[str]) -> dict[str, dict]:
    ...
    WebUI sessions are persisted as JSON files, but NasTech Agent also mirrors
