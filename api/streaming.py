"""
NasMusicUI -- SSE streaming engine and agent thread runner.
...
def _resolve_custom_provider_runtime_overrides(
    ...
    NasTech Agent treats named custom providers as routing hints around an
...
def _prewarm_skill_tool_modules():
    ...
    in the NasTech-Agent package which may not be on ``sys.path`` at
...
# Lazy import to avoid circular deps -- NasTech-Agent is on sys.path via api/config.py
try:
...
- Match the normal NasTech messaging style, but do not let long tool-running WebUI turns appear silent.
...
def _apply_prefill_context_budget(context: dict, config_data: dict) -> dict:
    ...
    file_raw = os.getenv("NASTECH_PREFILL_MESSAGES_FILE", "") or str(config_data.get("prefill_messages_file") or "")
...
def _load_webui_prefill_context(
    ...
    Supports the same bounded JSON-file shape used by NasTech Agent.  WebUI also
    ...
    file_raw = os.getenv("NASTECH_PREFILL_MESSAGES_FILE", "") or str(cfg.get("prefill_messages_file") or "")
...
def _webui_delivery_context_prompt(config_data: Optional[dict] = None) -> str:
    ...
    display_nastech_home = None
    try:
        from nastech_constants import get_nastech_home, display_nastech_home as _dh
        display_nastech_home = _dh
    except Exception:
        get_nastech_home = None  # type: ignore[assignment]

    connected = ["local (files on this machine)"]
    try:
        if get_nastech_home is not None:
            state_path = get_nastech_home() / "gateway_state.json"
    ...
    try:
        home_display = display_nastech_home() if display_nastech_home else "~/.nastech"
    except Exception:
        home_display = "~/.nastech"
...
def _cancelled_turn_hint(agent_name: str | None = None) -> str:
    name = str(agent_name or _preferred_agent_display_name()).strip() or 'NasTech'
...
def _classify_provider_error(err_str: str, exc=None, *, silent_failure: bool = False) -> dict:
    ...
    Keep this string-based until NasTech-Agent exposes stable structured
    ...
    _is_not_found = (
        # model_not_found hints mention Settings / `nastech model` below.
        '404' in err_str
        or 'not found' in _err_lower
        or 'does not exist' in _err_lower
        or 'model not found' in _err_lower
        or 'model_not_found' in _err_lower  # hint below points to Settings / `nastech model`
    ...
    if _is_quota:
        return {
            ...
            'hint': 'Your provider account is out of credits or usage. Top up, wait for the plan window to reset, or switch providers via `nastech model`.',
        }
    if _is_rate_limit:
        return {
    ...
    if _is_auth:
        return {
            ...
            'type': 'auth_mismatch',
            'hint': 'The selected model may not be supported by your configured provider or your API key is invalid. Run `nastech model` in your terminal to update credentials, then restart the WebUI.',
        }
    if _is_not_found:
        return {
            'label': 'Model not found',
            'type': 'model_not_found',
            'hint': 'The selected model was not found by the provider. Check the model ID in Settings or run `nastech model` to verify it exists for your provider.',
        }
    ...
    if silent_failure:
        return {
            ...
            'type': 'no_response',
            'hint': 'The provider returned no content and no error. This often means a usage/rate limit was hit silently. Check provider status, switch providers via `nastech model`, or try again in a moment.',
        }
...
def _aiagent_import_error_detail() -> str:
    ...
    The bare ImportError ("AIAgent not available -- check that NasTech-Agent is
    ...
    var if set, the sys.path entries that mention 'nastech', and the most-common
...
def _build_agent_thread_env(profile_runtime_env: dict | None, workspace: str, session_id: str, profile_home: str) -> dict:
    ...
    **kwargs alongside an explicit TERMINAL_CWD raises TypeError before the
    ...
    env.update({
        ...
        'NASTECH_EXEC_ASK': '1',
        'NASTECH_SESSION_KEY': session_id,
        'NASTECH_SESSION_ID': session_id,
        'NASTECH_SESSION_PLATFORM': 'webui',
        ...
        # NASTECH_SESSION_CHAT_ID to populate pending_watchers for WebUI
        # sessions so notify_on_complete completions enqueue and the agent
        # can be woken. NASTECH_SESSION_ID/PLATFORM come from upstream #2279.
        'NASTECH_SESSION_CHAT_ID': str(session_id),
...
# os.environ['NASTECH_SESSION_KEY'] (turn-start, line ~3263) and released the
...
def _set_turn_session_identity(session_id: str):
    ...
    a race:
      ...
      * ``gateway.session_context._SESSION_KEY`` — read by direct
        ``get_session_env("NASTECH_SESSION_KEY")`` consumers (e.g. the sudo
    ...
    flipping ``NASTECH_SESSION_PLATFORM`` from its env fallback (``'webui'``,
...
def _build_native_multimodal_message(workspace_ctx: str, msg_text: str, attachments, workspace: str, *, cfg: dict = None):
    ...
    bytes to NasTech as OpenAI-style image_url data URLs so vision-capable main
    ...
    # land in ~/.nastech/webui/attachments/<sid>/ (outside workspace_root by
...
def _fallback_title_from_exchange(user_text: str, assistant_text: str) -> Optional[str]:
    ...
    if topic_name:
        if not _contains_latin(topic_name):
            ...
            if any(k in combined for k in ('nastech', 'codex', 'ai')):
...
def _merge_display_messages_after_agent_result(previous_display, previous_context, result_messages, msg_text):
    ...
    If NasTech Agent returns a normal append-only history, append that delta to
...
def _attempt_credential_self_heal(
    ...
    Steps:
    1. Re-read ``~/.nastech/auth.json`` to pick up fresh credentials that
       may have been written by a concurrent ``nastech model`` CLI invocation.
    ...
    try:
        ...
        try:
            from api.profiles import (
                ...
                get_nastech_home_for_profile,
                get_profile_runtime_env,
            )
            _profile_home_path = get_nastech_home_for_profile(getattr(s, 'profile', None))
            _profile_home = str(_profile_home_path)
            _profile_runtime_env = get_profile_runtime_env(_profile_home_path)
        except ImportError:
            _profile_home = os.environ.get('NASTECH_HOME', '')
        ...
        # this session's NASTECH_SESSION_KEY to its WebUI session_id so the
        ...
        with _ENV_LOCK:
            ...
            old_exec_ask = os.environ.get('NASTECH_EXEC_ASK')
            old_session_key = os.environ.get('NASTECH_SESSION_KEY')
            old_session_id = os.environ.get('NASTECH_SESSION_ID')
            old_session_platform = os.environ.get('NASTECH_SESSION_PLATFORM')
            old_session_chat_id = os.environ.get('NASTECH_SESSION_CHAT_ID')
            old_nastech_home = os.environ.get('NASTECH_HOME')
            os.environ.update(_profile_runtime_env)
            os.environ['TERMINAL_CWD'] = str(s.workspace)
            os.environ['NASTECH_EXEC_ASK'] = '1'
            os.environ['NASTECH_SESSION_KEY'] = session_id
            os.environ['NASTECH_SESSION_ID'] = session_id
            os.environ['NASTECH_SESSION_PLATFORM'] = 'webui'
            # process_complete wiring (ours-original, Option B): see
            # _build_agent_thread_env above.
            os.environ['NASTECH_SESSION_CHAT_ID'] = str(session_id)
            if _profile_home:
                os.environ['NASTECH_HOME'] = _profile_home
                # Patch module-level caches to match the active profile.
                # _set_nastech_home() does this for process-wide switches
        ...
        # MUST run AFTER the NASTECH_HOME mutation above — `discover_mcp_tools()`
        # reads `~/.nastech/config.yaml` via `get_nastech_home()`, which uses
        # `os.environ['NASTECH_HOME']`.  Calling it before the mutation always
        ...
        # `_servers` by `(profile_home, name)` upstream in NasTech-Agent; that
        ...
        def _clarify_callback_impl(question, choices, sid, cancel_evt, put_event):
            """Bridge NasTech clarify prompts to the WebUI."""
        ...
        try:
            ...
            def on_tool(*cb_args, **cb_kwargs):
                ...
                # Modern NasTech Agent builds can call both tool_progress_callback
                ...
                if event_type == 'tool.completed':
                    ...
                    # `result` kwarg from modern NasTech builds; fall
            ...
            # Resolve API key via NasTech runtime provider (matches gateway behaviour).
            ...
            # nastech_cli.runtime_provider directly. Fall back to config.yaml
            ...
            # provider recovery). Match NasTech CLI/gateway semantics:
            ...
            # degrades gracefully when run against an older NasTech-Agent build.
            ...
            # after the operator raises NasTech' global turn budget.
            ...
            try:
                ...
                if _raw_max_iterations is None and isinstance(_cfg, dict):
                    # Back-compat for older NasTech config files that used a
            ...
            _agent_kwargs = dict(
                ...
                # Identify browser-originated sessions as WebUI so NasTech Agent
            ...
            # Params added in newer NasTech-Agent — skip if not supported
            ...
            if ephemeral:
                ...
                _sig_blob = _json.dumps([
                    ...
                    # at construction time, sourced from NASTECH_HOME. Same-session
            ...
            # (matches NasTech-Agent CLI behavior — passes via ephemeral_system_prompt)
            ...
            with _agent_lock:
                ...
                if _agent_sid and _agent_sid != session_id:
                    ...
                    # get_nastech_home_for_profile(getattr(s, 'profile', None))
                    # which falls back to the default profile's NASTECH_HOME.
                    ...
                    if _is_quota:
                        ...
                        if not _assistant_added:
                            ...
                            _err_hint = (
                                ...
                                'your API key is invalid. Run `nastech model` in your terminal to '
                ...
                if (not getattr(s, 'context_length', 0)) or _skip_cc_cl:
                    try:
                        ...
                        # Older NasTech-Agent builds whose get_model_context_length
                        ...
                        # Older NasTech-Agent builds may not expose this helper.
                ...
                if not ephemeral:
                    ...
                    # every completed turn.  This mirrors NasTech CLI where
            ...
            if not usage.get('context_length'):
                try:
                    ...
                    try:
                        ...
                        # Older NasTech-Agent builds: fall back to legacy 2-arg form.
                        _fb_cl = _get_cl(
                            getattr(agent, 'model', resolved_model or '') or '',
                            _cfg_base_url,
                        )
            ...
            # /goal parity: after a successful assistant turn, run the NasTech
            ...
            with _ENV_LOCK:
                ...
                if old_exec_ask is None: os.environ.pop('NASTECH_EXEC_ASK', None)
                else: os.environ['NASTECH_EXEC_ASK'] = old_exec_ask
                if old_session_key is None: os.environ.pop('NASTECH_SESSION_KEY', None)
                else: os.environ['NASTECH_SESSION_KEY'] = old_session_key
                if old_session_id is None: os.environ.pop('NASTECH_SESSION_ID', None)
                else: os.environ['NASTECH_SESSION_ID'] = old_session_id
                if old_session_platform is None: os.environ.pop('NASTECH_SESSION_PLATFORM', None)
                else: os.environ['NASTECH_SESSION_PLATFORM'] = old_session_platform
                if old_session_chat_id is None: os.environ.pop('NASTECH_SESSION_CHAT_ID', None)
                else: os.environ['NASTECH_SESSION_CHAT_ID'] = old_session_chat_id
                if old_nastech_home is None: os.environ.pop('NASTECH_HOME', None)
                else: os.environ['NASTECH_HOME'] = old_nastech_home
        ...
        # The user hint still points to Settings / `nastech model` from _classify_provider_error().
        if _exc_is_quota:
            ...
            _exc_label, _exc_type, _exc_hint = (
                ...
                'Run `nastech model` in your terminal to switch providers, then restart the WebUI.',
            )
...
def _handle_chat_steer(handler, body: dict) -> bool:
    ...
    if not hasattr(agent, "steer"):
        # Older NasTech-Agent that pre-dates the steer() method
