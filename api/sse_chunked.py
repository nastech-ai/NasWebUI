...
This is opt-in via the ``NASWEBUI_SSE_CHUNKED`` environment variable so
...
def chunked_sse_enabled() -> bool:
    """True when ``NASWEBUI_SSE_CHUNKED`` opts into chunked SSE framing."""
    return os.getenv("NASWEBUI_SSE_CHUNKED", "").strip().lower() in _TRUTHY
...
def end_sse_headers(handler):
    ...
    When ``NASWEBUI_SSE_CHUNKED`` is set, send ``Transfer-Encoding: chunked``
