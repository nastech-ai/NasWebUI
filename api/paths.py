...
def _nastech_home_has_webui_state(base: Path) -> bool:
    ...
    ``%USERPROFILE%\\.nastech`` location so we don't strand the user's existing
    sessions/pins/settings when the default moved to ``%LOCALAPPDATA%.nastech``
    (#2905).
    ...
    defaulted to ``%LOCALAPPDATA%.nastech`` on Windows since before #2897, so a
...
def _platform_default_nastech_home() -> Path:
    """Return the platform-aware default NasTech home when NASTECH_HOME is unset.

    Native Windows NasTech Agent installs default to %LOCALAPPDATA%.nastech,
    while POSIX installs use ~/.nastech.
    ...
    legacy ``%USERPROFILE%\\.nastech`` ONLY when it is populated AND the new
    ...
    if os.name == "nt":
        ...
        if local_app_data:
            new_home = Path(local_app_data) / "nastech"
            ...
            if (
                ...
                and not _nastech_home_has_webui_state(new_home)
                and _nastech_home_has_webui_state(legacy_home)
            ):
                return legacy_home
            return new_home
    return HOME / ".nastech"
