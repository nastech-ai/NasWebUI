...
def _safe_write(handler, body: bytes) -> None:
    ...
    try:
        ...
        import logging
        logging.getLogger("nastech.webui").debug(
...
def _build_redact_fn():
    """Return a redactor backed by NasTech-Agent plus local fallback patterns."""
    # Fallback mirrors the agent's known credential prefixes so WebUI API
    # responses remain a hard redaction boundary even without NasTech-Agent.
    # Keep this active even when NasTech-Agent is importable so API responses do
    ...
    def _combined_redact(text: str) -> str:
        ...
        # NASTECH_REDACT_SECRETS opt-in. The local fallback then handles the
        # common short-prefix shapes the agent omits (ghp_, sk-, hf_, AKIA).
        try:
            agent_redacted = redact_sensitive_text(text, force=True)
        except TypeError:
            # Older NasTech-Agent builds that predate the force kwarg.
...
PROFILE_COOKIE_NAME = 'nastech_profile'
