...
def _read_agent_source_version(agent_dir: Path) -> str | None:
    """Read NasTech Agent's package version from a copied source tree."""
    init_file = agent_dir / 'nastech_cli' / '__init__.py'
    try:
...
def _gateway_health_base_url() -> str:
    """Return the configured/default NasTech Agent gateway base URL."""
    raw = (
        os.environ.get('GATEWAY_HEALTH_URL')
        or os.environ.get('NASTECH_GATEWAY_HEALTH_URL')
        or 'http://NasTech-Agent:8642'
    ).strip()
...
def _detect_agent_version() -> str:
    """Detect the running NasTech Agent version for UI display."""
    ...
    if agent_dir is not None:
        ...
        if agent_dir.exists():
            ...
            # still lives in nastech_cli/__init__.py, so prefer that before giving
...
def _fallback_update_bullets(details: list[dict]) -> list[str]:
    bullets = []
    for item in details:
        label = item.get('label') or item.get('name') or 'NasTech'
...
def _worth_knowing_bullets(details: list[dict]) -> list[str]:
    ...
    if items:
        return items
    targets = [
        f"{item.get('label') or item.get('name') or 'NasTech'} ({item.get('behind') or 0} update{'s' if (item.get('behind') or 0) != 1 else ''})"
        for item in details
        if item.get('behind')
    ]
    if len(targets) > 1:
...
def _update_summary_prompt(details: list[dict]) -> tuple[str, str]:
    system = (
        "You write human-readable release summaries for NasTech users. "
...
def _apply_update_inner(target):
    ...
    stashed = False
    if status_out:
        _, ok = _run_git(['stash', 'push', '-m', 'nastech-update-autostash'], path)
        if not ok:
