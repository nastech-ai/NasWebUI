...
def _get_profile_home(profile) -> Path:
    """Resolve the nastech agent home directory for the given profile.

    Prefers the profile-specific helper from api.profiles; falls back to the
    NASTECH_HOME environment variable or ~/.nastech, expanding ~ correctly.
    """
    try:
        from api.profiles import get_nastech_home_for_profile
        return Path(get_nastech_home_for_profile(profile))
    except ImportError:
        return Path(os.environ.get('NASTECH_HOME') or '~/.nastech').expanduser()
...
def _profile_default_model_state(profile=None):
    ...
    try:
        from api.profiles import get_nastech_home_for_profile
        config_path = Path(get_nastech_home_for_profile(profile)) / "config.yaml"
...
def _active_state_db_path() -> Path:
    """Return state.db for the active NasTech profile, degrading to NASTECH_HOME."""
    try:
        from api.profiles import get_active_nastech_home
        nastech_home = Path(get_active_nastech_home()).expanduser().resolve()
    except Exception:
        nastech_home = Path(os.getenv('NASTECH_HOME', str(HOME / '.nastech'))).expanduser().resolve()
    return nastech_home / 'state.db'
...
def _resolve_cli_sessions_context():
    # Use the active WebUI profile's NASTECH_HOME to find state.db.
    # The active profile is determined by what the user has selected in the UI
    # (stored in the server's runtime config). This means:
    #   - default profile  -> ~/.nastech/state.db
    #   - named profile X  -> ~/.nastech/profiles/X/state.db
    # We resolve the active profile's home directory rather than just using
    # NASTECH_HOME (which is the server's launch profile, not necessarily the
    ...
    return nastech_home, db_path, cli_profile, cache_key


def _load_cli_sessions_uncached(nastech_home: Path, db_path: Path, _cli_profile) -> list:
    cli_sessions = []
    try:
    ...
    for row in read_importable_agent_session_rows(
        ...
        if not _title and _source == 'cron' and sid.startswith('cron_'):
            ...
            if len(parts) >= 3:
                _job_id = parts[1]
                try:
                    _jobs_path = nastech_home / 'cron' / 'jobs.json'
...
def get_cli_sessions() -> list:
    ...
    """
    nastech_home, db_path, cli_profile, cache_key = _resolve_cli_sessions_context()
    ...
    if ttl > 0:
        with _CLI_SESSIONS_CACHE_LOCK:
            ...
            try:
                sessions = _load_cli_sessions_uncached(nastech_home, db_path, cli_profile)
    ...
    try:
        return _load_cli_sessions_uncached(nastech_home, db_path, cli_profile)
    except Exception as _cli_err:
        logger.warning(
            "get_cli_sessions() failed — check state.db schema or path (%s): %s",
            db_path, _cli_err,
...
def get_state_db_session_messages(sid, *, stitch_continuations: bool = False, profile=None) -> list:
    """Read messages for a NasTech session from state.db.
    ...
    through another NasTech surface such as the Gateway API Server.  When
