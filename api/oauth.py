...
tokens stay server-side and are persisted to the active NasTech profile's
...
AUTH_JSON_PATH = Path.home() / ".nastech" / "auth.json"
...
def _get_active_nastech_home() -> Path:
    """Return the active NasTech profile home directory, falling back to ~/.nastech when profile resolution fails."""
    try:
        from api.profiles import get_active_nastech_home

        return Path(get_active_nastech_home())
    except Exception as exc:
        # Per Opus advisor on stage-296: log the silent fallback so a corrupt
        # profile state ending up writing tokens to ~/.nastech (instead of the
        # active profile) is observable in logs rather than failing silently.
        logger.warning(
            "Falling back to ~/.nastech for OAuth credential storage: "
...
def _persist_codex_credentials(nastech_home: Path, token_data: dict[str, Any]) -> Path:
    ...
    auth_path = Path(nastech_home) / "auth.json"
...
def _save_codex_credentials(token_data):
    ...
    return _persist_codex_credentials(_get_active_nastech_home(), token_data)
...
def _clear_anthropic_env_values(nastech_home: Path) -> None:
...
def _link_anthropic_credentials(nastech_home: Path) -> None:
    """Link NasTech to use Claude Code's credential store.

    Clears ANTHROPIC_TOKEN and ANTHROPIC_API_KEY from the NasTech .env so
    ...
    """
    _clear_anthropic_env_values(nastech_home)
...
def _run_anthropic_credential_worker(flow_id: str) -> None:
    ...
    while True:
        ...
        try:
            ...
            nastech_home = Path(flow["nastech_home"])
            _link_anthropic_credentials(nastech_home)
            with _OAUTH_FLOWS_LOCK:
                current = _OAUTH_FLOWS.get(flow_id)
                if not current or current.get("status") != "pending":
                    ...
                    cancelled = False
            if cancelled:
                _remove_anthropic_link_marker(nastech_home)
            return
        except Exception as exc:
...
def _remove_anthropic_link_marker(nastech_home: Path) -> None:
    ...
    while True:
        ...
        try:
            ...
            with _OAUTH_FLOWS_LOCK:
                ...
                if not current or current.get("status") != "pending":
                    return
            _persist_codex_credentials(Path(live["nastech_home"]), tokens)
...
def _start_anthropic_flow(nastech_home: Path) -> dict[str, Any]:
...
def start_onboarding_oauth_flow(body: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if provider in _ANTHROPIC_PROVIDER_ALIASES:
        return _start_anthropic_flow(_get_active_nastech_home())

    # Codex flow
    nastech_home = _get_active_nastech_home()
    try:
