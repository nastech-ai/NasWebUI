...
# extension — the same technique used throughout the NasTech codebase.
...
_PROVIDER_ENV_VAR: dict[str, str] = {
    ...
    # by default and the runtime in nastech_cli/runtime_provider.py only consumes
    ...
    # runtime reads (nastech_cli/auth.py:182, api_key_env_vars=("LM_API_KEY",)).
...
# through the NasTech CLI, not via API keys.  The WebUI cannot set these.
...
def _get_nastech_home() -> Path:
    """Return the active NasTech home directory."""
    try:
...
def _provider_has_shadowed_codex_oauth_value(provider_id: str) -> bool:
    ...
    if env_var:
        env_path = _get_nastech_home() / ".env"
...
def _write_env_file(env_path: Path, updates: dict[str, str | None]) -> None:
    ...
    with _ENV_LOCK:
        ...
        # ``~/.nastech/.env`` is also written by ``nastech_cli.config.save_env_value``
...
def _provider_has_key(provider_id: str) -> bool:
    ...
    Checks (in order):
    1. ``~/.nastech/.env`` for the known env var
...
def _account_usage_subprocess_env(home: Path, provider: str, api_key: str | None) -> dict[str, str]:
    env = dict(os.environ)
    env["NASTECH_HOME"] = str(Path(home))
...
def get_provider_quota(provider_id: str | None = None, *, refresh: bool = False) -> dict[str, Any]:
    ...
    providers reuse NasTech Agent's /usage account-limits abstraction so WebUI
...
def _cost_snapshots_dir() -> Path:
    ...
    Uses the NasTech home directory (profile-aware) so snapshots are
    isolated per profile, matching the existing STATE_DIR convention.
    """
    return _get_nastech_home() / _COST_SNAPSHOTS_DIR_NAME
...
def _append_cost_snapshot(provider: str, usage: int | float | None, limit: int | float | None) -> list[dict[str, Any]]:
    ...
    # deployments that share one NasTech home/state directory.
...
def get_providers() -> dict[str, Any]:
    ...
    for pid in sorted(known_ids):
        ...
        if not has_key and is_plugin_model_provider(pid):
            try:
                from nastech_cli.auth import get_auth_status as _gas_plugin
        ...
        if is_oauth:
            key_source = "oauth"
            # Check if actually authenticated via nastech_cli.
            ...
            # returns logged_in=False (e.g. token not in the nastech credential pool,
            ...
            try:
                ...
                # key just because the nastech_cli auth module is unavailable.
                logger.debug("nastech_cli auth check failed for %s", pid, exc_info=True)
        ...
        # The model picker already prefers nastech_cli + Codex local cache for
        ...
        # case below — when nastech_cli is unavailable or its lookup raises,
        ...
        if pid == "nous":
            try:
                ...
                logger.debug("Failed to load Nous Portal models from nastech_cli")
        ...
        if pid == "lmstudio":
            try:
                ...
                logger.debug("Failed to load LM Studio models from nastech_cli")
...
def set_provider_key(provider_id: str, api_key: str | None) -> dict[str, Any]:
    ...
    Writes the key to ``~/.nastech/.env`` using the standard env var name.
    ...
    if _provider_is_oauth(provider_id):
        return {
            ...
            "error": f"'{_PROVIDER_DISPLAY.get(provider_id, provider_id)}' uses OAuth authentication. "
                     f"Use `nastech model` in the terminal to configure it.",
        }
...
def remove_provider_key(provider_id: str) -> dict[str, Any]:
    ...
    Removes the key from ``~/.nastech/.env`` (via ``set_provider_key``)
