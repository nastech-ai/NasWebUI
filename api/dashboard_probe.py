"""Safe server-side probe for the official NasTech Agent dashboard.

The official `nastech dashboard` binds to 127.0.0.1:9119 by default and exposes
...
def _looks_like_official_dashboard(payload: object) -> bool:
    ...
    if not isinstance(version, str) or not version.strip():
        return False
    # Verified against current NasTech Agent `nastech_cli.web_server.get_status()`:
    # /api/status returns version plus these NasTech-specific fields. Requiring at
    # least one avoids treating any generic {version: ...} local service as the
    # official dashboard.
    return any(key in payload for key in ("release_date", "nastech_home", "config_path", "gateway_running"))
...
def probe_official_dashboard(
    ...
    """Best-effort check that `nastech dashboard` is running on host:port."""
    try:
        ...
        logger.debug("official NasTech dashboard probe failed", exc_info=True)
        return {"running": False}
