...
_SUPPORTED_PROVIDER_SETUPS = {
    ...
    "lmstudio": {
        "label": "LM Studio",
        # Canonical env var matches the agent CLI runtime (nastech_cli/auth.py:182,
...
_UNSUPPORTED_PROVIDER_NOTE = (
    ...
    "when your NasTech config selects the corresponding provider."
)


def _get_active_nastech_home() -> Path:
    try:
...
def _provider_api_key_present(
    ...
    # xai, etc.), ask the nastech_cli auth registry — it knows every provider's env
...
def _provider_oauth_authenticated(provider: str, nastech_home: "Path") -> bool:
    ...
    requested NasTech home. Known OAuth providers may store auth either in the
    legacy providers[provider_id] singleton state or in credential_pool entries
    used by current NasTech runtime auth resolution.
    """
    provider = (provider or "").strip().lower()
...
def _status_from_runtime(cfg: dict, imports_ok: bool) -> dict:
    ...
    env_values = _load_env_file(_get_active_nastech_home() / ".env")
    ...
    if provider_configured:
        ...
        if provider in _SUPPORTED_PROVIDER_SETUPS:
            ...
            if meta.get("key_optional"):
                ...
                if not provider_ready and meta.get("oauth_provider"):
                    provider_ready = _provider_oauth_authenticated(
                        str(meta.get("oauth_provider")), _get_active_nastech_home()
                    )
        else:
            ...
            provider_ready = (
                ...
                or _provider_oauth_authenticated(provider, _get_active_nastech_home())
            )

    chat_ready = bool(_NASTECH_FOUND and imports_ok and provider_ready)

    if not _NASTECH_FOUND or not imports_ok:
        state = "agent_unavailable"
        note = (
            "NasTech is not fully importable from the Web UI yet. Finish bootstrap or fix the "
            "agent install before provider setup will work."
        )
    elif chat_ready:
        state = "ready"
        provider_name = _PROVIDER_DISPLAY.get(
            provider, provider.title() if provider else "NasTech"
        )
        note = f"NasTech is minimally configured and ready to chat via {provider_name}."
    elif provider_configured:
        state = "provider_incomplete"
        if provider == "custom" and not base_url:
            note = (
                "NasTech has a saved provider/model selection but still needs the "
            ...
            note = (
                ...
                "Run 'nastech auth' or 'nastech model' in a terminal to complete "
        ...
        note = "NasTech is installed, but you still need to choose a provider and save working credentials."
    ...
    return {
        ...
        "env_path": str(_get_active_nastech_home() / ".env"),
    }
...
def _build_setup_catalog(cfg: dict) -> dict:
    ...
    # of a key input when the user has already authenticated via 'nastech auth'.
    current_is_oauth = (
        current_provider not in _SUPPORTED_PROVIDER_SETUPS and bool(current_provider)
    ) or _provider_oauth_authenticated(current_provider, _get_active_nastech_home())
...
def get_onboarding_status() -> dict:
    ...
    cfg = get_config()
    imports_ok, missing, errors = verify_nastech_imports()
    runtime = _status_from_runtime(cfg, imports_ok)
    workspaces = load_workspaces()
    last_workspace = get_last_workspace()
    available_models = get_available_models()

    # NASMUSICUI_SKIP_ONBOARDING=1 lets hosting providers (e.g. Agent37) ship
    ...
    skip_env = os.environ.get("NASMUSICUI_SKIP_ONBOARDING", "").strip()
    skip_requested = skip_env in {"1", "true", "yes"}
    auto_completed = skip_requested  # unconditional: operator says skip, we skip

    # Auto-complete for existing NasTech users: if config.yaml already exists
    # AND the provider is configured (or the system is chat_ready), treat onboarding
    # as done.  These users configured NasTech via the CLI before the Web UI existed;
    ...
    # a git branch switch in the NasTech-Agent repo).  Without this, a CLI-configured
...
def apply_onboarding_setup(body: dict) -> dict:
    ...
    if Path(config_path).exists() and not body.get("confirm_overwrite"):
        return {
            "error": "config_exists",
            "message": (
                "NasTech is already configured (config.yaml exists). "
    ...
    env_path = _get_active_nastech_home() / ".env"
    ...
    if not api_key and not _provider_api_key_present(provider, cfg, env_values):
        ...
        oauth_ready = bool(provider_meta.get("oauth_provider")) and _provider_oauth_authenticated(
            str(provider_meta.get("oauth_provider")), _get_active_nastech_home()
        )
    ...
    # Reload the nastech_cli provider/config cache so the next streaming call
    # picks up the new key without requiring a server restart.
    try:
        from api.profiles import _reload_dotenv
        _reload_dotenv(_get_active_nastech_home())
    except Exception:
    ...
    try:
        # nastech_cli may cache config at import time; ask it to reload if possible.
        from nastech_cli.config import reload as _cli_reload
        _cli_reload()
    except Exception:
        logger.debug("Failed to reload nastech_cli config")
