"""Expose NasTech-Agent's COMMAND_REGISTRY to the webui frontend.

This module is the single integration point with nastech_cli.commands.
If NasTech-Agent is unavailable the endpoint degrades to an empty list
...
def list_commands(_registry=None) -> list[dict[str, Any]]:
    ...
    Returns empty list if nastech_cli is not installed (graceful
    degradation -- the frontend has its own fallback minimum set).

    Args:
        _registry: Optional injected registry for testing. When None
            (production), imports COMMAND_REGISTRY from nastech_cli.
    """
    if _registry is None:
        try:
            from nastech_cli.commands import COMMAND_REGISTRY as _registry
        except ImportError:
            logger.warning("nastech_cli.commands not importable -- /api/commands returns []")
            return []
...
def _run_codex_runtime_command(arg_string: str) -> str:
    """Execute NasTech' shared Codex runtime switch for the active profile."""
    try:
        from nastech_cli.codex_runtime_switch import apply, parse_args
...
def execute_plugin_command(command: str) -> str:
    ...
    transport errors, matching NasTech' existing slash-command UX.
    """
