"""Tests for fix: onboarding wizard must not fire when NasTech is already configured.

Issue #420 — existing NasTech users (config.yaml present + chat_ready) were
...
class TestOnboardingGate:
    def test_config_exists_and_chat_ready_returns_completed_true(self, tmp_path):
        ...
        assert result["completed"] is True, (
            "Wizard fired for existing NasTech user! "
...
class TestApplyOnboardingSetupGuard:
    ...
    def test_setup_allowed_with_confirm_overwrite(self, tmp_path):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_home:
                ...
                # Also redirect _get_active_nastech_home so .env writes go to the temp dir,
                # never to the real ~/.nastech/.env.
                with mock.patch.object(mod, "_get_active_nastech_home", return_value=tmp_home_path):
    ...
    def test_setup_allowed_when_no_config_exists(self, tmp_path):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_home:
                tmp_home_path = pathlib.Path(tmp_home)
                # Redirect both config path and nastech home into temp dirs so the
                # test never touches the real ~/.nastech/.env.
                with (
                    mock.patch.object(mod, "_get_config_path", return_value=fake_config_path),
                    mock.patch.object(mod, "_get_active_nastech_home", return_value=tmp_home_path),
                ):
                    result = mod.apply_onboarding_setup(
                        {
                            "provider": "openrouter",
...
def _server_nastech_home() -> pathlib.Path:
...
class TestOnboardingGateIntegration:
    ...
    def test_existing_config_and_chat_ready_skips_wizard(self):
        ...
        try:
            ...
            (nastech_home / "config.yaml").unlink(missing_ok=True)
            (nastech_home / ".env").unlink(missing_ok=True)
            _http_post("/api/settings", {"onboarding_completed": False})
    @_needs_yaml
