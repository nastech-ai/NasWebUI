...
def test_visible_progress_token_reasoning_and_interim_are_deduped(cleanup_test_sessions):
    """Progress text can arrive through three NasTech callbacks; WebUI must show it once.
    ...
    setattr(fake_nastech_cli, "runtime_provider", fake_runtime_module)
    fake_nastech_state = types.ModuleType("nastech_state")
    setattr(fake_nastech_state, "SessionDB", mock.Mock(return_value=None))
    injected = {
