...
The legacy path is the default when NASWEBUI_GATEWAY_USE_RUNS_API is not set.
...
def test_legacy_loop_checks_nastech_approval_request_event():
    """Legacy SSE loop must handle `nastech.approval.request` events."""
    loop = _extract_legacy_sse_loop()
    assert '"nastech.approval.request"' in loop, (
        "Legacy SSE loop must check for nastech.approval.request event name"
    )
...
def test_legacy_loop_resets_sse_event_after_approval():
    ...
    loop = _extract_legacy_sse_loop()
    approval_idx = loop.find('"nastech.approval.request"')
...
def test_legacy_sse_loop_relays_approval_event():
    ...
    try:
        with patch.dict("os.environ", {"NASWEBUI_CHAT_BACKEND": "gateway"}):
