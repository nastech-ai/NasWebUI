...
def test_project_context_prefers_nastech_md_and_reports_shadowed_agents(tmp_path):
    workspace = tmp_path / "priority"
    workspace.mkdir()
    (workspace / "NASTECH.md").write_text("# NasWebUI Rules\n\nNasWebUI wins.", encoding="utf-8")
    (workspace / "AGENTS.md").write_text("# Agent Rules\n\nAgents lose.", encoding="utf-8")

    data = project_context_for(workspace)

    assert "NasWebUI wins." in data["content"]
    ...
    assert data["shadowed"][0]["shadowed_by"] == "NASTECH.md"


def test_project_context_walks_nastech_md_to_git_root_but_not_agents_md(tmp_path):
    root = tmp_path / "repo"
    child = root / "src" / "pkg"
    child.mkdir(parents=True)
    (root / ".git").mkdir()
    (root / ".nastech.md").write_text("# Root NasWebUI\n\nRoot project rules.", encoding="utf-8")
...
def test_project_context_content_is_redacted_in_memory_response(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(api.profiles, "get_active_nastech_home", lambda: home)
