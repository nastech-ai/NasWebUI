...
unframed SSE is buffered until-close, so NASWEBUI_SSE_CHUNKED opts into
...
def test_default_off_is_plain_end_headers(monkeypatch):
    monkeypatch.delenv("NASWEBUI_SSE_CHUNKED", raising=False)
...
def test_flag_on_emits_chunked_frames_that_decode_back(monkeypatch):
    monkeypatch.setenv("NASWEBUI_SSE_CHUNKED", "1")
...
def test_truthy_values(monkeypatch):
    for val in ("1", "true", "TRUE", "yes", "On"):
        monkeypatch.setenv("NASWEBUI_SSE_CHUNKED", val)
        assert chunked_sse_enabled() is True
