...
v0.51.134 (PR #2897) moved the Windows default NasTech home from
``%USERPROFILE%\.nastech`` to ``%LOCALAPPDATA%.nastech`` to match the agent.
...
The fix makes ``_platform_default_nastech_home()`` prefer the populated legacy
``%USERPROFILE%\.nastech`` ONLY when the new ``%LOCALAPPDATA%.nastech`` location
...
def windows_env(monkeypatch, tmp_path):
    ...
    needs before calling ``config._platform_default_nastech_home()``.
    """
...
def test_upgrade_fingerprint_prefers_populated_legacy_home(windows_env):
    ...
    result = config._platform_default_nastech_home()

    assert result == legacy_home, (
        "Windows upgrade must not strand WebUI state: when %LOCALAPPDATA%/nastech "
        "is empty but %USERPROFILE%/.nastech holds the user's sessions/pins, the "
...
def test_legacy_dir_present_but_empty_does_not_divert(windows_env):
    ...
    fallback — otherwise a stray empty %USERPROFILE%/.nastech would shadow a
    fresh install."""
    legacy_home, new_home = windows_env
...
def test_does_nothing_on_posix(monkeypatch, tmp_path):
    """On POSIX (os.name != 'nt') the resolver always returns ~/.nastech,
...
def test_no_files_are_moved_by_resolution(windows_env):
    ...
    config._platform_default_nastech_home()
...
class TestNasTechHomeHasWebuiState:
    """Unit coverage for the marker-detection helper."""

    def test_empty_or_missing_dir_is_not_state(self, tmp_path):
        assert paths._nastech_home_has_webui_state(tmp_path / "nope") is False
        empty = tmp_path / "empty"
        empty.mkdir()
        assert paths._nastech_home_has_webui_state(empty) is False

    def test_webui_sessions_marker_counts(self, tmp_path):
        (tmp_path / "webui" / "sessions").mkdir(parents=True)
        assert paths._nastech_home_has_webui_state(tmp_path) is True
    ...
    def test_agent_only_artifacts_do_not_count(self, tmp_path):
        ...
        assert paths._nastech_home_has_webui_state(tmp_path) is False


def test_profiles_base_home_uses_shared_path_helper(monkeypatch, tmp_path):
    """profiles._resolve_base_nastech_home() must share config's path helper so
    ...
    monkeypatch.delenv("NASTECH_HOME", raising=False)
    monkeypatch.delenv("NASTECH_BASE_HOME", raising=False)
    monkeypatch.setattr(paths, "_platform_default_nastech_home", lambda: sentinel)

    assert profiles._resolve_base_nastech_home() == sentinel
