...
PR #805 extends the fix to ALL endpoints: profile switches set a nastech_profile
...
class TestProfileCookieHelpers:
    ...
    def test_build_profile_cookie_sets_value(self):
        ...
        assert 'nastech_profile=alice' in s
    ...
    def test_build_profile_cookie_default_persists(self):
        ...
        assert 'nastech_profile=default' in s
    ...
    def test_get_profile_cookie_extracts_valid_name(self):
        from api.helpers import get_profile_cookie
        handler = MagicMock()
        handler.headers.get = lambda k, d='': 'nastech_profile=alice' if k == 'Cookie' else d
        assert get_profile_cookie(handler) == 'alice'

    def test_get_profile_cookie_accepts_default(self):
        from api.helpers import get_profile_cookie
        handler = MagicMock()
        handler.headers.get = lambda k, d='': 'nastech_profile=default' if k == 'Cookie' else d
    ...
    def test_get_profile_cookie_rejects_injection(self):
        ...
        for bad in ('../etc', 'a/b', 'name;DROP', 'WithCaps', 'has space', '.hidden'):
            handler = MagicMock()
            handler.headers.get = lambda k, d='', v=bad: f'nastech_profile={v}' if k == 'Cookie' else d
    ...
    def test_profile_cookie_name_defaults_to_nastech_profile(self, monkeypatch):
        from api.helpers import build_profile_cookie
    ...
    def test_profile_cookie_name_can_be_isolated_per_webui_instance(self, monkeypatch):
        ...
        monkeypatch.setenv('WEBUI_PROFILE_COOKIE_NAME', 'nastech_profile_social')

        s = build_profile_cookie('writer')
        assert 'nastech_profile_social=writer' in s
        assert 'nastech_profile=writer' not in s

        handler = MagicMock()
        handler.headers.get = lambda k, d='': (
            'nastech_profile=wrong; nastech_profile_social=writer' if k == 'Cookie' else d
        )
    ...
    def test_configured_profile_cookie_ignores_default_cookie_name(self, monkeypatch):
        ...
        monkeypatch.setenv('WEBUI_PROFILE_COOKIE_NAME', 'nastech_profile_main')

        handler = MagicMock()
        handler.headers.get = lambda k, d='': 'nastech_profile=social_profile' if k == 'Cookie' else d
        assert get_profile_cookie(handler) is None
...
def test_switch_profile_process_wide_false_does_not_mutate_global():
    ...
    original_env_home = os.environ.get('NASTECH_HOME')

    # We need a profile that exists to get past the validation path.
    # Use 'default' — switch_profile accepts it without requiring nastech_cli.
    try:
        ...
        # NASTECH_HOME env must not change
        assert os.environ.get('NASTECH_HOME') == original_env_home, (
            "process_wide=False must not mutate os.environ['NASTECH_HOME']"
        )
