...
Note (cross-tool): NasTech-Agent's ``openai-codex`` provider config
...
def test_openai_api_key_env_var_path_detects_openai_codex(monkeypatch):
    ...
    The primary OAuth detection path (nastech_cli.auth) handles Codex for
    users who ran `nastech auth login openai-codex`. This test covers the
    fallback path for environments where nastech_cli is not available or
