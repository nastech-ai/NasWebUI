...
# (the real package lives in NasTech-Agent, which is not installed here).
...
class TestGenerateTitleRawViaAuxTimeout(unittest.TestCase):
    ...
    def test_configured_api_key_is_passed_to_aux_client(self):
        ...
        NasTech Agent does not fall back to auxiliary.title_generation.api_key
...
class TestBackgroundTitleProfileRouting(unittest.TestCase):
            ...
            with patch.dict(os.environ, {'NASTECH_HOME': 'default-home'}, clear=False):
                with self.assertLogs('api.profiles', level='DEBUG') as logs:
                    with profiles.profile_env_for_background_worker(session, 'background title'):
                        captured['NASTECH_HOME'] = os.environ.get('NASTECH_HOME')

        message_found = any(
            'Failed to resolve profile env for background title profile work' in record.getMessage()
            for record in logs.records
        )
        self.assertEqual(captured['NASTECH_HOME'], 'default-home')
    ...
    def test_skill_home_snapshot_removes_modules_imported_during_context(self):
        ...
        try:
            ...
            setattr(imported_during_context, 'NASTECH_HOME', 'profile-home')
                ...
                with patch('api.profiles.get_profile_runtime_env', return_value=runtime_env):
                    with patch.dict(os.environ, {'NASTECH_HOME': default_home, 'OPENROUTER_API_KEY': 'default-openrouter-key'}, clear=False):
                        ...
                        with profiles.profile_env_for_background_worker(session, 'background title'):
                            ...
                            captured['thread_home'] = getattr(_thread_ctx, 'env', {}).get('NASTECH_HOME')
