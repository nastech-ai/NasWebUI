...
# Do NOT touch real ~/.nastech, real credentials, or real profile directories.
#
# Test strategy:
#   - Mock _DEFAULT_NASTECH_HOME to a tmp_path so _resolve_base_nastech_home()
#     picks up the isolated root.
#   - Inject a mock 'nastech_cli.profiles' module directly into sys.modules so
#     that the `from nastech_cli.profiles import seed_profile_skills` inside
#     create_profile_api resolves to the mock (not the real module).
#   - Stub nastech_cli.profiles.create_profile to create the profile dir.
#   - Stub nastech_cli.profiles.seed_profile_skills to record calls.
...
# Import the module under test directly (isolated from any real NASTECH_HOME env).
...
@pytest.fixture
def fake_nastech_home(tmp_path, monkeypatch):
    # Point _DEFAULT_NASTECH_HOME at an isolated temp directory so that
    # profile-path resolution does not touch the real ~/.nastech.
    fake_home = tmp_path / '.nastech'
    fake_home.mkdir(parents=True)
    monkeypatch.setenv('NASTECH_BASE_HOME', str(fake_home))
    monkeypatch.setattr(profiles_mod, '_DEFAULT_NASTECH_HOME', fake_home)
    return fake_home


def _install_nastech_cli_profiles_mock(create_impl, seed_impl):
    # Inject a mock 'nastech_cli.profiles' module directly into sys.modules.
    # This is the only way to intercept `from nastech_cli.profiles import X`
    # inside create_profile_api — patch.dict(sys.modules, ...) only modifies
    # existing keys and cannot add new ones.
    mock = ModuleType('nastech_cli.profiles')
    mock.create_profile = create_impl
    mock.seed_profile_skills = seed_impl
    sys.modules['nastech_cli'] = ModuleType('nastech_cli')
    sys.modules['nastech_cli.profiles'] = mock
    return mock


def _remove_nastech_cli():
    for key in list(sys.modules):
        if key == 'nastech_cli' or key.startswith('nastech_cli.'):
            del sys.modules[key]


# Module references saved at import time so we can restore the real nastech_cli
# after each test that overwrites sys.modules['nastech_cli.profiles'].  This
# prevents the `FallbackDoesNotCrash` tests from finding a deleted entry and
# incorrectly skipping.
_real_nastech_cli = sys.modules.get('nastech_cli')
_real_nastech_cli_profiles = sys.modules.get('nastech_cli.profiles')


def _restore_real_nastech_cli():
    if _real_nastech_cli is not None:
        sys.modules['nastech_cli'] = _real_nastech_cli
    if _real_nastech_cli_profiles is not None:
        sys.modules['nastech_cli.profiles'] = _real_nastech_cli_profiles


# ── Tests ──────────────────────────────────────────────────────────────────────

class TestNoCloneSeedsSkills:
    def test_seed_called_once_with_resolved_path(self, fake_nastech_home):
        calls = []

        def fake_create(name, **kw):
            _make_profile_dir(_isolated_profiles_root(fake_nastech_home), name)

        def fake_seed(profile_path, quiet=None):
            calls.append({'profile_path': profile_path, 'quiet': quiet})

        _remove_nastech_cli()
        _install_nastech_cli_profiles_mock(fake_create, fake_seed)
        ...
        # Path must be the resolved profile directory under the fake nastech home.
        expected_path = _isolated_profiles_root(fake_nastech_home) / 'testprofile'
...
class TestCloneSkipsSeeding:
    def test_seed_not_called_when_clone_from_is_set(self, fake_nastech_home):
        calls = []
...
class TestSeedFailureIsBestEffort:
    def test_seed_raising_logs_warning_and_still_returns_profile(self, fake_nastech_home, caplog):
...
class TestNasTechCliUnavailableFallbackDoesNotCrash:
    def test_fallback_create_still_produces_profile_dict(self, fake_nastech_home):
        # Simulate nastech_cli being present but create_profile raising ImportError
        ...
        #
        # We cannot permanently delete nastech_cli.profiles from sys.modules (it
        ...
        real_mod = sys.modules.get('nastech_cli.profiles')
        if real_mod is None:
            # nastech_cli.profiles was already cleaned up by a prior test in this
            # process — skip rather than failing with a confusing assertion.
            pytest.skip('nastech_cli.profiles not in sys.modules (cleaned up by prior test)')

        orig_create = real_mod.create_profile
        real_mod.create_profile = MagicMock(side_effect=ImportError('nastech_cli profiles unavailable'))
        try:
            with patch.object(profiles_mod, 'list_profiles_api', return_value=[]):
        ...
        expected_path = _isolated_profiles_root(fake_nastech_home) / 'isolatedprofile'
        assert Path(result['path']) == expected_path

    def test_seed_unavailable_logs_debug_without_crashing(self, fake_nastech_home, caplog):
        import logging as std_logging
        ...
        real_mod = sys.modules.get('nastech_cli.profiles')
        real_nastech_cli = sys.modules.get('nastech_cli')
        if real_mod is None or real_nastech_cli is None:
            pytest.skip('nastech_cli.profiles not in sys.modules (cleaned up by prior test)')

        # We need nastech_cli.profiles.seed_profile_skills to not exist so that
        # `from nastech_cli.profiles import seed_profile_skills` raises ImportError.
        ...
        fake_nastech_cli = ModuleType('nastech_cli')
        sys.modules['nastech_cli'] = fake_nastech_cli
        sys.modules['nastech_cli.profiles'] = mock
        ...
        try:
            ...
            _remove_nastech_cli()
            sys.modules['nastech_cli'] = real_nastech_cli
