...
Previously it did not load REPO_ROOT/.env, so NASWEBUI_HOST, NASWEBUI_PORT
...
class TestLoadRepoDotenv:
    ...
    def test_sets_env_var_from_dotenv(self, tmp_path):
        ...
        self._run(tmp_path, "NASWEBUI_HOST=0.0.0.0\n")
        assert os.environ.get("NASWEBUI_HOST") == "0.0.0.0"

    def test_sets_port_from_dotenv(self, tmp_path):
        """NASWEBUI_PORT is loaded as a string (caller does int() conversion)."""
        self._run(tmp_path, "NASWEBUI_PORT=18787\n")
        assert os.environ.get("NASWEBUI_PORT") == "18787"

    def test_ignores_comment_lines(self, tmp_path):
        """Lines starting with # are not loaded."""
        os.environ.pop("NASWEBUI_HOST", None)
        self._run(tmp_path, "# NASWEBUI_HOST=should-be-ignored\n")
        assert os.environ.get("NASWEBUI_HOST") is None

    def test_ignores_blank_lines(self, tmp_path):
        """Blank lines are silently skipped without error."""
        self._run(tmp_path, "\n\nNASWEBUI_PORT=9000\n\n")
        assert os.environ.get("NASWEBUI_PORT") == "9000"
    ...
    def test_noop_when_dotenv_unreadable(self, tmp_path, capsys):
        ...
        env_path.write_text("NASWEBUI_PORT=9999\n")
        orig = bs.REPO_ROOT
        try:
            bs.REPO_ROOT = tmp_path
    ...
    def test_overwrites_existing_env_var(self, tmp_path):
        ...
        os.environ["NASWEBUI_HOST"] = "127.0.0.1"
        self._run(tmp_path, "NASWEBUI_HOST=0.0.0.0\n")
        assert os.environ.get("NASWEBUI_HOST") == "0.0.0.0"
    ...
    def test_preserve_existing_env_keeps_ctl_overrides(self, tmp_path):
        ...
        os.environ["NASWEBUI_PRESERVE_ENV"] = "1"
        os.environ["NASTECH_HOME"] = "/runtime/naswebuiOne"
        os.environ["NASWEBUI_PASSWORD"] = ""
        self._run(
            tmp_path,
            "NASTECH_HOME=/repo/default\nNASWEBUI_PASSWORD=repo-password\n",
        )
        assert os.environ.get("NASTECH_HOME") == "/runtime/naswebuiOne"
        assert os.environ.get("NASWEBUI_PASSWORD") == ""
    ...
    def test_does_not_set_empty_values(self, tmp_path):
        ...
        os.environ.pop("NASWEBUI_EMPTY_KEY", None)
        self._run(tmp_path, 'NASWEBUI_EMPTY_KEY=""\n')
        ...
        val = os.environ.get("NASWEBUI_EMPTY_KEY")
    ...
    def test_export_prefix_stripped(self, tmp_path):
        ...
        self._run(tmp_path, "export NASWEBUI_HOST=0.0.0.0\n")
        assert os.environ.get("NASWEBUI_HOST") == "0.0.0.0", (
