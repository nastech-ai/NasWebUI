...
Previously it did not load REPO_ROOT/.env, so NASMUSICUI_HOST, NASMUSICUI_PORT
...
class TestLoadRepoDotenv:
    ...
    def test_sets_env_var_from_dotenv(self, tmp_path):
        ...
        self._run(tmp_path, "NASMUSICUI_HOST=0.0.0.0\n")
        assert os.environ.get("NASMUSICUI_HOST") == "0.0.0.0"

    def test_sets_port_from_dotenv(self, tmp_path):
        """NASMUSICUI_PORT is loaded as a string (caller does int() conversion)."""
        self._run(tmp_path, "NASMUSICUI_PORT=18787\n")
        assert os.environ.get("NASMUSICUI_PORT") == "18787"

    def test_ignores_comment_lines(self, tmp_path):
        """Lines starting with # are not loaded."""
        os.environ.pop("NASMUSICUI_HOST", None)
        self._run(tmp_path, "# NASMUSICUI_HOST=should-be-ignored\n")
        assert os.environ.get("NASMUSICUI_HOST") is None

    def test_ignores_blank_lines(self, tmp_path):
        """Blank lines are silently skipped without error."""
        self._run(tmp_path, "\n\nNASMUSICUI_PORT=9000\n\n")
        assert os.environ.get("NASMUSICUI_PORT") == "9000"
    ...
    def test_noop_when_dotenv_unreadable(self, tmp_path, capsys):
        ...
        env_path.write_text("NASMUSICUI_PORT=9999\n")
        orig = bs.REPO_ROOT
        try:
            bs.REPO_ROOT = tmp_path
    ...
    def test_overwrites_existing_env_var(self, tmp_path):
        ...
        os.environ["NASMUSICUI_HOST"] = "127.0.0.1"
        self._run(tmp_path, "NASMUSICUI_HOST=0.0.0.0\n")
        assert os.environ.get("NASMUSICUI_HOST") == "0.0.0.0"
    ...
    def test_does_not_set_empty_values(self, tmp_path):
        ...
        os.environ.pop("NASMUSICUI_EMPTY_KEY", None)
        self._run(tmp_path, 'NASMUSICUI_EMPTY_KEY=""\n')
        ...
        val = os.environ.get("NASMUSICUI_EMPTY_KEY")
    ...
    def test_multiple_keys_all_loaded(self, tmp_path):
        ...
        content = "NASMUSICUI_HOST=0.0.0.0\nNASMUSICUI_PORT=18787\n"
    ...
    def test_export_prefix_stripped(self, tmp_path):
        ...
        self._run(tmp_path, "export NASMUSICUI_HOST=0.0.0.0\n")
        assert os.environ.get("NASMUSICUI_HOST") == "0.0.0.0", (
...
class TestBootstrapStructure:
    ...
    def test_dotenv_loaded_before_default_host_port(self):
        ...
        assert load_pos < host_pos, (
            ...
            "so that NASMUSICUI_HOST from .env is picked up"
        )
        assert load_pos < port_pos, (
            "_load_repo_dotenv() must be called before DEFAULT_PORT assignment "
            "so that NASMUSICUI_PORT from .env is picked up"
        )
