...
class TestSessionDBInjection(unittest.TestCase):
    ...
    def test_nastech_state_import_present(self):
        """SessionDB must be imported from nastech_state inside _run_agent_streaming."""
        self.assertIn(
    ...
    def test_sessiondb_init_in_try_except(self):
        ...
        pattern = (
            ...
            r"[\s\S]*?try:\s*\n[\s\S]*?from nastech_state import SessionDB[\s\S]*?return SessionDB\(db_path=state_db_path\)[\s\S]*?except Exception as _db_err:"
        )
        self.assertRegex(
            helper_src,
            pattern,
    ...
    def test_session_db_initialized_before_agent_construction(self):
        ...
        db_pos = STREAMING_PY.find("from nastech_state import SessionDB")
...
class TestRuntimeRouteInjection(unittest.TestCase):
    ...
    def test_runtime_provider_keys_are_forwarded_to_agent(self):
        ...
        so the WebUI degrades gracefully against older NasTech-Agent builds.
        """
        for snippet in (
    ...
    def test_runtime_route_is_forwarded_from_resolver_into_agent_init(self):
        ...
        fake_nastech_cli.runtime_provider = fake_runtime_module
        fake_nastech_state = types.ModuleType("nastech_state")
        fake_nastech_state.SessionDB = mock.Mock(return_value=fake_session_db)
        ...
        with mock.patch.object(streaming, "get_session", return_value=fake_session), \
             ...
             mock.patch.dict(
                 sys.modules,
                 {
                     "nastech_cli": fake_nastech_cli,
                     "nastech_cli.runtime_provider": fake_runtime_module,
                     "nastech_state": fake_nastech_state,
                 },
             ):
    ...
    def test_runtime_provider_forwards_interim_assistant_callback(self):
        ...
        fake_queue = queue.Queue()
        fake_rt_module = types.ModuleType("nastech_cli.runtime_provider")
        ...
        fake_nastech_cli = types.ModuleType("nastech_cli")
        fake_nastech_cli.runtime_provider = fake_rt_module
        fake_nastech_state = types.ModuleType("nastech_state")
        fake_nastech_state.SessionDB = mock.Mock(return_value=object())
        ...
        with mock.patch.object(streaming, "get_session", return_value=fake_session), \
             ...
             mock.patch.dict(sys.modules, {
                 "nastech_cli": fake_nastech_cli,
                 "nastech_cli.runtime_provider": fake_rt_module,
                 "nastech_state": fake_nastech_state,
             }):
            streaming.STREAMS[fake_stream_id] = fake_queue
            streaming._run_agent_streaming(
        ...
        self.assertIn("Match the normal NasTech messaging style", captured["agent"].ephemeral_system_prompt)
        self.assertIn(
...
class TestSessionDBAST(unittest.TestCase):
    ...
    def test_sessiondb_try_not_inside_env_lock(self):
        ...
        # a Try node that also contains 'from nastech_state import SessionDB'
        for node in ast.walk(self.tree):
            ...
            for stmt in node.body:
                if isinstance(stmt, ast.Try):
                    # Check if this try imports nastech_state
...
class TestCredentialPoolBackwardCompat(unittest.TestCase):
    ...
    when running against an older NasTech-Agent that lacks them (issue #772)."""
    ...
    def test_older_agent_without_credential_pool_does_not_crash(self):
        ...
        class OlderAgent:
            """Simulates a NasTech-Agent build that predates credential_pool."""
            def __init__(self, model=None, provider=None, base_url=None, api_key=None,
                         platform=None, quiet_mode=False, enabled_toolsets=None,
                         fallback_model=None, session_id=None, session_db=None,
                         stream_delta_callback=None, reasoning_callback=None,
        ...
        fake_nastech_cli.runtime_provider = fake_rt_module
        fake_nastech_state = types.ModuleType("nastech_state")
        fake_nastech_state.SessionDB = mock.Mock(return_value=None)
