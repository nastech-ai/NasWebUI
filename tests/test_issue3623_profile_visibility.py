...
def _install_fake_nastech_profiles(monkeypatch, rows):
    nastech_cli = types.ModuleType("nastech_cli")
    profiles_mod = types.ModuleType("nastech_cli.profiles")
    profiles_mod.list_profiles = lambda: rows
    monkeypatch.setitem(sys.modules, "nastech_cli", nastech_cli)
    monkeypatch.setitem(sys.modules, "nastech_cli.profiles", profiles_mod)
...
def test_profile_yaml_visible_false_is_exposed_as_hidden(monkeypatch, tmp_path):
    ...
    _install_fake_nastech_profiles(monkeypatch, rows)
