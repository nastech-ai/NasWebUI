...
NASTECH_HOME = pathlib.Path(os.getenv('NASTECH_HOME', str(HOME / '.nastech')))
...
# ~/.nastech/config.yaml.  Override it before any product modules are imported so
# tests that read/write config.yaml stay inside the isolated test home.
os.environ['NASTECH_CONFIG_PATH'] = str(TEST_STATE_DIR / 'config.yaml')
...
def _restore_profile_home_globals():
    """Restore NASTECH_HOME / NASTECH_BASE_HOME after every test.

    Several tests call ``api.profiles.switch_profile()`` (or set NASTECH_HOME
    directly) which mutates ``os.environ['NASTECH_HOME']`` IN PLACE — not via
    ...
    NASTECH_HOME points at a deleted tmpdir and breaks any later test whose
    ...
    """
    saved_home = os.environ.get('NASTECH_HOME')
    saved_base = os.environ.get('NASTECH_BASE_HOME')
    ...
    # _DEFAULT_NASTECH_HOME stale. Fixing it at setup time guarantees each test
    # starts from a base root that matches the current (restored) env.
    _rederive_default_nastech_home()
    yield
    for key, val in (('NASTECH_HOME', saved_home), ('NASTECH_BASE_HOME', saved_base)):
        if val is None:
...
def _rederive_default_nastech_home():
    """Recompute api.profiles._DEFAULT_NASTECH_HOME from the current env.

    api.profiles caches the base home at import time. A test that re-imports
    api.profiles under a temporary NASTECH_BASE_HOME (e.g. test_profile_env_isolation)
    corrupts that global to a now-deleted tmpdir, making get_nastech_home_for_profile
    resolve later tests' profiles under the dead path. Re-deriving keeps it honest.
    """
    prof_mod = sys.modules.get('api.profiles')
    if prof_mod is not None and hasattr(prof_mod, '_resolve_base_nastech_home'):
        try:
            prof_mod._DEFAULT_NASTECH_HOME = prof_mod._resolve_base_nastech_home()
        except Exception:
            pass
...
NASTECH_AGENT = _discover_agent_dir()
VENV_PYTHON  = _discover_python(NASTECH_AGENT)

# Work dir: agent dir if found, else repo root
WORKDIR = str(NASTECH_AGENT) if NASTECH_AGENT else str(REPO_ROOT)

# ── Agent availability detection ─────────────────────────────────────────────
# Tests that require NasTech-Agent modules (cron, skills, approval, chat/stream)
# are skipped when the agent isn't installed, instead of failing with 500 errors.
AGENT_AVAILABLE = NASTECH_AGENT is not None

def _check_agent_modules():
    """Verify NasTech-Agent Python modules are actually importable."""
    if not NASTECH_AGENT:
        return False
    try:
        import importlib
...
# pytest marker: skip tests that need NasTech-Agent when it's not present
requires_agent = pytest.mark.skipif(
    not AGENT_AVAILABLE,
    reason="NasTech-Agent not found (skipping agent-dependent test)"
)
requires_agent_modules = pytest.mark.skipif(
    not AGENT_MODULES_AVAILABLE,
    reason="NasTech-Agent Python modules not importable (cron, skills_tool)"
)

def pytest_configure(config):
    config.addinivalue_line("markers", "requires_agent: skip when NasTech-Agent dir is not found")
    config.addinivalue_line("markers", "requires_agent_modules: skip when NasTech-Agent Python modules are not importable")


# ── Disable AWS IMDS probing for the pytest session ────────────────────────
# Background: when NasTech-Agent's bedrock_adapter / botocore credential chain
...
# the guard the nastech_cli/doctor.py command already uses in its parallel-probe
# block).
#
...
# raises OSError("nastech test network isolation").  Tests that deliberately
...
def pytest_collection_modifyitems(config, items):
    ...
    test names to known categories that depend on NasTech-Agent modules.
    ...
    # Exact list of tests known to fail without NasTech-Agent.
    ...
    skip_marker = pytest.mark.skip(reason="requires NasTech-Agent (not installed)")
    skipped = 0
    ...
    if skipped:
        print(f"\nWARNING: NasTech-Agent not found; {skipped} agent-dependent tests will be skipped\n")
...
def test_server():
    ...
    real_skills  = NASTECH_HOME / 'skills'
    ...
    env.update({
        ...
        # ~/.nastech can redirect _get_active_nastech_home() out of the sandbox,
    ...
    for _attempt in range(1, boot_attempts + 1):
        ...
        # 45s (up from 20s): server.py imports the full NasTech-Agent, which is
