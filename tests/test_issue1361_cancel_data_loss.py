...
def test_cancel_copy_falls_back_to_naswebui_for_blank_bot_name(monkeypatch):
    ...
    assert streaming._cancelled_turn_hint() == (
        'The run was cancelled by the user before NasTech finished. '
        'No provider failure occurred.'
    )
    ...
    def test_cancel_stream_does_not_duplicate_existing_worker_cancel_marker(self):
        ...
        _make_session(
            ...
            messages=[
                ...
                {'role': 'assistant', 'content': '**Task cancelled:** Task cancelled.\n\n*The run was cancelled by the user before NasTech finished. No provider failure occurred.*', '_error': True, 'timestamp': 101},
            ],
        )
        _setup_cancel_state(sid, stream_id)
