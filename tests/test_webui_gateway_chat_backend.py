...
def test_gateway_chat_worker_translates_sse_and_persists_session(tmp_path, monkeypatch):
    ...
    class FakeResponse:
        ...
        def __iter__(self):
            yield b'event: nastech.tool.progress\n'
    ...
    assert captured["headers"]["X-nastech-session-id"] == s.session_id
    assert captured["headers"]["X-nastech-session-key"] == f"webui:{s.session_id}"
