...
# AND a model.base_url set, nastech_cli reports the 'custom' pseudo-provider as
...
def _available_models_with_full_cfg(provider, default, base_url):
    ...
    Clears model-override env vars (NASTECH_MODEL, OPENAI_MODEL, LLM_MODEL)
    during the call so the real nastech profile environment doesn't leak into
    ...
    _model_env_keys = ('NASTECH_MODEL', 'OPENAI_MODEL', 'LLM_MODEL')
...
def test_no_phantom_custom_group_when_active_provider_is_set(monkeypatch):
    ...
    # Force nastech_cli to report both the real provider and the phantom
    # 'custom' as authenticated, simulating what list_available_providers()
    # returns when base_url is configured.
    fake_mod = types.ModuleType('nastech_cli.models')
    ...
    monkeypatch.setitem(sys.modules, 'nastech_cli.models', fake_mod)
...
def test_custom_endpoint_uses_model_config_api_key_for_model_discovery(monkeypatch):
    ...
    monkeypatch.delenv('NASTECH_API_KEY', raising=False)
    monkeypatch.delenv('NASTECH_OPENAI_API_KEY', raising=False)
