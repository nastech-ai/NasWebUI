...
def test_reasoning_probe_falls_back_without_nastech_cli(
    tmp_path,
    monkeypatch,
    lmstudio_probe_server,
):
    blocked_modules = [
        ...
        if name == "nastech_cli" or name.startswith("nastech_cli.")
    ]
    ...
    class _Blocker:
        ...
        def load_module(self, name):
            raise ImportError(f"nastech_cli blocked for test: {name}")
...
def test_credentialed_probe_never_calls_nastech_cli(
    tmp_path,
    monkeypatch,
    lmstudio_probe_server,
):
    """When a credential is configured, the probe must use the built-in
    no-redirect fallback and NEVER the bundled nastech_cli probe (which follows
...
""",
    ...
    monkeypatch.setitem(sys.modules, "nastech_cli", type("NasTechCli", (), {})())
    monkeypatch.setitem(sys.modules, "nastech_cli.models", _CliModule)
    ...
    # … and the redirect-following nastech_cli probe was never invoked.
    assert cli_calls == [], "credentialed probe must bypass nastech_cli"
...
def test_keyless_probe_logs_signature_mismatch_before_fallback(
    ...
    """Keyless probes still prefer nastech_cli; an incompatible CLI signature
...
def test_credentialed_probe_does_not_follow_redirects_end_to_end(
    ...
    nastech_cli importable: when the CONFIGURED LM Studio endpoint returns a 302,
    the credentialed probe must not forward the key to the redirect target.
    This is the production path (nastech_cli present) — credentialed probes route
    ...
    # Sanity: this test only proves the redirect guard if nastech_cli is
