...
ROOT CAUSE (RCA t_f62ff1e8, verified line-by-line):
  ...
  ``os.environ['NASMUSICUI_SESSION_KEY']`` (streaming.py turn-start), and the env
...
def test_concurrent_turns_capture_their_own_session_under_env_race():
    ...
    streaming = importlib.import_module("api.streaming")
    pytest.importorskip("tools.approval", reason="NasTech-Agent not installed")
    ...
    # The two turn() threads below stamp os.environ["NASMUSICUI_SESSION_KEY"]
    ...
    _prev_env = os.environ.get("NASMUSICUI_SESSION_KEY", _prev_env_sentinel)
    ...
    def turn(my_sid: str, label: str) -> None:
        ...
        with bind(my_sid):
            os.environ["NASMUSICUI_SESSION_KEY"] = my_sid
            barrier.wait()
            if label == "B":
    ...
    try:
        ...
        # Restore NASMUSICUI_SESSION_KEY to its pre-test value (or unset if it was
        ...
        if _prev_env is _prev_env_sentinel:
            os.environ.pop("NASMUSICUI_SESSION_KEY", None)
        else:
            assert isinstance(_prev_env, str)
            os.environ["NASMUSICUI_SESSION_KEY"] = _prev_env
...
def test_turn_identity_binder_restores_previous_value():
    ...
    registration that reads NASMUSICUI_SESSION_PLATFORM still works)."""
    streaming = importlib.import_module("api.streaming")
    pytest.importorskip("tools.approval", reason="NasTech-Agent not installed")
    from tools.approval import get_current_session_key
    from gateway import session_context as sc
