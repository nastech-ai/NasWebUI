...
def test_streaming_sessiondb_uses_session_profile_state_db(tmp_path, monkeypatch):
    """WebUI streaming must not rely on nastech_state.DEFAULT_DB_PATH.

    ``nastech_state.DEFAULT_DB_PATH`` is frozen at import time. A non-default
    ...
    profile_home = tmp_path / "nastech-home" / "profiles" / "zhangtingban"
    ...
    fake_nastech_state.SessionDB = fake_session_db
    ...
    monkeypatch.setattr(profiles, "get_nastech_home_for_profile", lambda _profile: profile_home)
