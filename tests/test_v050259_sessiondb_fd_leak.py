...
def test_session_db_close_is_idempotent():
    ...
    Skipped when nastech_state is not on the import path (e.g. on the GH
    ...
    if importlib.util.find_spec("nastech_state") is None:
        pytest.skip("nastech_state not on import path (CI-only — agent repo not present)")
