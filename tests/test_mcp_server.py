...
def _cleanup_state_dir(state_dir: Path):
    ...
    if saved.get("captured"):
        ...
        # Restore NASTECH_BASE_HOME / NASTECH_HOME if we changed them
...
def _reimport_mcp():
    ...
    Also normalizes NASTECH_BASE_HOME / NASTECH_HOME to point at a
    ...
    isolated_home = state_dir.parent / "nastech-home"
    ...
    if not _SAVED_CONSTANTS.get("captured"):
        _SAVED_CONSTANTS["env"] = {
            "NASTECH_BASE_HOME": os.environ.get("NASTECH_BASE_HOME", _MISSING_ENV),
            "NASTECH_HOME": os.environ.get("NASTECH_HOME", _MISSING_ENV),
        }

    os.environ["NASTECH_BASE_HOME"] = str(isolated_home)
    os.environ["NASTECH_HOME"] = str(isolated_home)
