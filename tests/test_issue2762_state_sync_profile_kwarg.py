...
def two_profile_homes(tmp_path, monkeypatch):
    ...
    ``nastech_state.SessionDB`` itself (so the schema matches what the
    ...
    pytest.importorskip("nastech_state")
...
def _read_session(db_path: Path, session_id: str):
    ...
    try:
        # Real state.db schema (see api/state_sync.py + nastech_cli StateDB):
...
def test_get_state_db_honors_explicit_profile_kwarg(two_profile_homes):
    ...
    # Some installs ship without the nastech_state package; the function
    # returns None gracefully and there's nothing to assert.
    try:
        import nastech_state  # noqa: F401
    except ImportError:
        pytest.skip("nastech_state package not available in this test env")
...
def test_unknown_explicit_profile_returns_none_not_falls_back(two_profile_homes):
    ...
    NASTECH_HOME and write to the wrong DB. That fallback would
...
def test_invalid_profile_name_refused_not_falls_back(two_profile_homes, bad_name):
    ...
    ``_DEFAULT_NASTECH_HOME`` for any name failing ``_PROFILE_ID_RE``
