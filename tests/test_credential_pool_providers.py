...
def _install_fake_nastech_cli(monkeypatch, *, with_load_pool: bool = False, pool_data: dict | None = None):
    """Stub nastech_cli modules so tests are deterministic and offline.

    When *with_load_pool* is True, also stubs nastech_cli.credential_pool with a
    ...
    # not the live ~/.nastech/auth.json via the real venv module.
...
def _call_get_available_models(monkeypatch, tmp_path, auth_payload, *, with_load_pool: bool = False):
    """Call get_available_models() with auth.json pinned to a temp NasTech home."""
...
def test_ollama_cloud_empty_catalog_skips_group(monkeypatch, tmp_path):
    """When nastech_cli returns no models for ollama-cloud, the group is omitted.
    ...
    import sys as _sys
    _sys.modules["nastech_cli.models"].provider_model_ids = lambda pid: []
