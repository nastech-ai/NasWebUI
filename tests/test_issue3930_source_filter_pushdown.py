...
def test_get_cli_sessions_source_filter_uses_distinct_cache_key(monkeypatch, tmp_path):
    nastech_home = tmp_path / "naswebui"
    nastech_home.mkdir()
    monkeypatch.setattr(profiles, "get_active_nastech_home", lambda: str(nastech_home))
    ...
    def fake_loader(_nastech_home, _db_path, _cli_profile, source_filter=None):
...
def test_get_cli_sessions_all_profiles_pushes_source_filter_to_every_context(monkeypatch, tmp_path):
    ...
    contexts = [
        (nastech_home, nastech_home / "state.db", "default"),
        (nastech_home / "p2", nastech_home / "p2" / "state.db", "haku"),
    ]
    monkeypatch.setattr(models, "_all_profiles_cli_contexts", lambda: (contexts, "ctx-key"))

    seen = []

    def fake_loader(_nastech_home, _db_path, _cli_profile, source_filter=None, **kwargs):
        seen.append(source_filter)
