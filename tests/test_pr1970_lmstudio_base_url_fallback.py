...
def test_lmstudio_fallback_works_when_nastech_cli_unavailable(tmp_path, monkeypatch):
    """The lmstudio branch must populate models from the urlopen fallback even
    when `from nastech_cli.models import provider_model_ids` raises ImportError.
    ...
    a CI-vs-local divergence where local environments with nastech_cli installed
    worked, and CI (clean editable install) failed with empty model groups.

    Caught in CI on stage-337; fix splits the nastech_cli try from the urlopen
    ...
    # Block nastech_cli import the way a CI runner without the package would.
    blocked_modules = [name for name in list(sys.modules) if name == "nastech_cli" or name.startswith("nastech_cli.")]
...
""",
        ...
        # Fallback must succeed despite nastech_cli being unimportable.
        assert "lmstudio" in groups, (
            f"lmstudio group missing when nastech_cli unavailable; groups={list(groups)}"
        )
