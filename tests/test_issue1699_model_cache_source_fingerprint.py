...
runs `nastech setup` in a terminal and the NasTech auth store switches the active
...
def _write_auth_store(nastech_home, provider_id: str) -> None:
...
def _configure_isolated_sources(tmp_path, monkeypatch, provider_id: str) -> None:
    ...
    monkeypatch.setenv("NASTECH_CONFIG_PATH", str(config_path))
    ...
    # Keep the test hermetic without requiring NasTech-Agent to be installed in
    # CI: inject the tiny nastech_cli surface get_available_models() imports.
    ...
    _write_auth_store(nastech_home, provider_id)
...
def test_disk_models_cache_invalidates_when_auth_store_active_provider_changes(
    ...
    # External terminal `nastech setup` changes auth.json, not WebUI's in-process cache.
    nastech_home = config._models_cache_path.parent.parent / "nastech-home"
    _write_auth_store(nastech_home, "opencode-go")
    _reset_memory_cache()
