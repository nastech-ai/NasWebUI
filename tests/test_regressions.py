...
def test_real_jobs_json_not_polluted_by_tests(cleanup_test_sessions):
    """R6: Test runs must not write to the real ~/.nastech/cron/jobs.json.
    When NASTECH_HOME isolation was missing, every test run added test-job-* entries.
    """
    real_jobs_path = pathlib.Path.home() / ".nastech" / "cron" / "jobs.json"
...
def test_streaming_bridge_accepts_current_tool_progress_callback_signature(cleanup_test_sessions):
    """R17: api/streaming.py must accept the current NasTech agent callback contract.
...
def test_skills_slash_command_defined():
    ...
    array is now sourced from NasTech-Agent's ``COMMAND_REGISTRY`` at boot
...
def test_provider_oauth_authenticated_accepts_credential_pool_entries(
    ...
    NasTech runtime resolves Codex credentials from credential_pool; onboarding
