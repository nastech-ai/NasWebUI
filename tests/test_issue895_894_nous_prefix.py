"""
Regression tests for #895 (set_nastech_default_model strips @nous: prefix + blocks on live fetch)
...
from api.config import resolve_model_provider, set_nastech_default_model
...
# ── #895: set_nastech_default_model persists @provider: prefix ──────────────
...
class TestSetDefaultModelPreservesAtPrefix:
    ...
    def test_at_nous_prefix_strips_to_bare_for_cli_compatibility(self, tmp_path, monkeypatch):
        """set_nastech_default_model must persist the RESOLVED bare/slash form, not the
        `@provider:` prefix. The `@provider:` syntax is a WebUI-internal routing hint;
        the NasTech-Agent CLI reads `config.yaml -> model.default` directly and passes
        ...
        Storing `@nous:anthropic/...` would break any user who runs `nastech` in the
        ...
        result = set_nastech_default_model("@nous:anthropic/claude-opus-4.6")
        ...
        assert saved["model"]["default"] == "anthropic/claude-opus-4.6", (
            f"Config must persist the resolved bare form so the NasTech-Agent CLI "
    ...
    def test_save_does_not_return_full_model_catalog(self, tmp_path, monkeypatch):
        """set_nastech_default_model must return a lightweight ack, not call get_available_models (#895)."""
        config_file = tmp_path / "config.yaml"
        config_file.write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        ...
        result = set_nastech_default_model("openai/gpt-5.4-mini")
        # Must be a simple dict with ok+model, NOT the full catalog (which has "groups")
        assert result.get("ok") is True
        assert "groups" not in result, (
            "set_nastech_default_model must not return the full model catalog — "
