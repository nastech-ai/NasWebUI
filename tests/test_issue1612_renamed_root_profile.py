"""Tests for issue #1612: renamed root profile must resolve to ~/.nastech,
not ~/.nastech/profiles/<name>.

A renamed root/default NasTech profile (`is_default=True` on the agent side
but with a display name like `kinni`) was being treated as a named profile
directory under `~/.nastech/profiles/kinni`, which doesn't exist. Every
...
def test_is_root_profile_renamed_root_via_list_profiles_api(monkeypatch):
    ...
    monkeypatch.setattr(p, 'list_profiles_api', lambda: [
        {'name': 'kinni', 'is_default': True, 'path': str(p._DEFAULT_NASTECH_HOME)},
...
def test_is_root_profile_caches_results(monkeypatch):
    ...
    def fake_list():
        calls['n'] += 1
        return [{'name': 'kinni', 'is_default': True, 'path': '/tmp/.nastech'}]
...
def test_is_root_profile_invalidation_drops_stale(monkeypatch):
    ...
    seq = [
        [{'name': 'kinni', 'is_default': True, 'path': '/tmp/.nastech'}],
        [{'name': 'noblepro', 'is_default': True, 'path': '/tmp/.nastech'}],
    ]
...
def test_is_root_profile_handles_list_profiles_failure(monkeypatch):
    ...
    def boom():
        raise RuntimeError("nastech_cli explosion")
...
def test_switch_profile_resolution_renamed_root_picks_default_home(tmp_path, monkeypatch):
    ...
    _DEFAULT_NASTECH_HOME, not raise 'Profile <name> does not exist.'
...
def test_switch_profile_sticky_marker_renamed_root(tmp_path, monkeypatch):
    ...
    _is_root_profile('default') → resolves to _DEFAULT_NASTECH_HOME, which
