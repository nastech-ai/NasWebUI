...
``nastech_cli.models.provider_model_ids("nous")``, prefix every id with ``@nous:``
to match the existing routing convention, fall back to the curated static
list when ``nastech_cli`` is unavailable.
"""
...
# by nastech_cli.models.provider_model_ids("nous") (see #1538 issue body).
...
def _install_fake_nastech_cli(monkeypatch, *, nous_ids=None, raise_on_lookup=False):
    """Install fake ``nastech_cli`` modules so detection sees Nous as authenticated
    and ``provider_model_ids("nous")`` returns the desired catalog.

    Mirrors :func:`tests.test_issue1420_lmstudio_provider_env_var._install_fake_nastech_cli`
    but specialised for Nous detection (Nous is OAuth so the env-var path
    is not used — we drive detection via ``nastech_cli.auth.list_auth_providers``).
    """
    fake_pkg = types.ModuleType("nastech_cli")
    fake_pkg.__path__ = []

    fake_models = types.ModuleType("nastech_cli.models")
    fake_models.list_available_providers = lambda: []
    if raise_on_lookup:
        def _raise(_pid):
            raise RuntimeError("simulated nastech_cli failure")
    ...
    fake_auth = types.ModuleType("nastech_cli.auth")
...
def _scrub_provider_env(monkeypatch):
    ...
    via the fake nastech_cli stubs (not unrelated keys leaked from the runner)."""
    for var in (
        ...
        try:
            ...
            for required in (
                ...
                assert required in ids, (
                    ...
                    f"the nastech_cli dispatch is broken or the @nous: prefix is "
                    f"missing."
                )
        finally:
            restore()
...
class TestNousStaticFallback:
    """When ``nastech_cli`` is not importable or its lookup raises, we fall back
    ...
    def test_static_fallback_when_nastech_cli_raises(self, monkeypatch, tmp_path):
        ...
        try:
            ...
            nous_groups = [g for g in data.get("groups", []) if g.get("provider_id") == "nous"]
            assert nous_groups, (
                "Nous group must still appear when nastech_cli fails — the "
...
class TestStaticListPreservedAsFallback:
    ...
    def test_static_list_present(self):
        ...
        assert _PROVIDER_MODELS.get("nous"), (
            ...
            "a fallback for environments where nastech_cli is unavailable."
        )
