...
must NOT engage from the NASTECH_HOME *shape* alone.

The bug: `_is_isolated_profile_mode()` inferred "isolated mode" purely from a
`~/.nastech/profiles/<name>` shaped NASTECH_HOME. But the NasTech Agent launcher exports
...
The fix (#4586): isolated mode requires an EXPLICIT opt-in — NASWEBUI_ISOLATED_PROFILE —
as the primary gate (default OFF). The profile-shaped NASTECH_HOME is only a secondary
...
def _clear_cache_and_force_flag_off(monkeypatch):
    ...
    We explicitly clear NASWEBUI_ISOLATED_PROFILE so the suite's ambient env can't
    accidentally enable isolation and mask the regression.
    """
    monkeypatch.delenv("NASWEBUI_ISOLATED_PROFILE", raising=False)
...
def named_profile_home():
    """A normal single-user layout: base ~/.nastech with several profiles, active = `webui`.

    The active profile's home is `~/.nastech/profiles/webui` — exactly the
    `*/profiles/<name>` shape the NasTech Agent launcher exports for a named profile, and
...
class TestIssue4586NamedProfileIsNotIsolated:
    ...
    def test_shape_alone_does_not_enable_isolated_mode(self, named_profile_home):
        ...
        with mock.patch.dict(os.environ, {"NASTECH_HOME": str(active)}, clear=False):
            with mock.patch("api.profiles._INITIAL_NASTECH_HOME", str(active)):
                ...
                assert _is_isolated_profile_mode() is False, (
                    ...
                    "NASWEBUI_ISOLATED_PROFILE opt-in (#4586)"
                )
    ...
    def test_profiles_tab_is_not_clamped_to_single_profile(self, named_profile_home):
        ...
        unlike exercising the full nastech_cli-backed discovery machinery (which depends on
    ...
    def test_switching_to_another_profile_is_allowed(self, named_profile_home):
        ...
        with mock.patch.dict(os.environ, {"NASTECH_HOME": str(active)}, clear=False):
            with mock.patch("api.profiles._INITIAL_NASTECH_HOME", str(active)):
                with mock.patch("api.profiles._DEFAULT_NASTECH_HOME", base):
                    with mock.patch("api.profiles._resolve_base_nastech_home", return_value=base):
...
class TestIssue4586ExplicitOptInStillWorks:
    ...
    def test_flag_plus_shape_enables_isolated_mode(self, named_profile_home, flag):
        active = named_profile_home["active"]
        with mock.patch.dict(os.environ, {"NASTECH_HOME": str(active),
                                          "NASWEBUI_ISOLATED_PROFILE": flag}, clear=False):
            with mock.patch("api.profiles._INITIAL_NASTECH_HOME", str(active)):
    ...
    def test_flag_without_profile_shape_stays_off(self, named_profile_home):
        """Secondary requirement: a stray flag without a profile-shaped NASTECH_HOME = OFF."""
        base = named_profile_home["base"]  # base ~/.nastech, NOT a */profiles/<name> path
        with mock.patch.dict(os.environ, {"NASTECH_HOME": str(base),
                                          "NASWEBUI_ISOLATED_PROFILE": "1"}, clear=False):
            with mock.patch("api.profiles._INITIAL_NASTECH_HOME", str(base)):
                assert _is_isolated_profile_mode() is False, (
                    "flag set but NASTECH_HOME is the base home → isolation must stay off"
                )
...
class TestIssue4589ProfileEnvCannotDisableIsolation:
    ...
    contained user could put NASWEBUI_ISOLATED_PROFILE=0 in their profile .env
    ...
    def test_profile_env_cannot_clear_isolated_flag(self, named_profile_home):
        ...
        assert "NASWEBUI_ISOLATED_PROFILE" in _PROTECTED_ENV_KEYS
        ...
        with mock.patch.dict(os.environ, {"NASTECH_HOME": str(active),
            ...
            with mock.patch("api.profiles._INITIAL_NASTECH_HOME", str(active)):
                ...
                # The contained user plants NASWEBUI_ISOLATED_PROFILE=0 in their .env.
                (active / ".env").write_text(
                    "NASWEBUI_ISOLATED_PROFILE=0\nSOME_OTHER_KEY=ok\n", encoding="utf-8"
                )
                try:
                    _reload_dotenv(active)
                    # The protected flag must NOT have been overridden → still isolated.
                    assert os.environ.get("NASWEBUI_ISOLATED_PROFILE") == "1", (
    ...
    def test_runtime_env_path_also_strips_protected_flag(self, named_profile_home):
        ...
        assert "NASWEBUI_ISOLATED_PROFILE" in _BLOCKED_RUNTIME_ENV_KEYS
        ...
        assert "NASWEBUI_ISOLATED_PROFILE" not in env, (
