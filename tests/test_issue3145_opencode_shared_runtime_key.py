...
def test_shared_opencode_api_key_resolves_for_runtime(monkeypatch, tmp_path, provider_id):
    ...
    nastech_home.mkdir()
    (nastech_home / ".env").write_text("OPENCODE_API_KEY=shared-opencode-key\n", encoding="utf-8")

    monkeypatch.setattr(providers, "_get_nastech_home", lambda: nastech_home)
...
def test_shared_opencode_api_key_resolves_from_process_env(monkeypatch, tmp_path, provider_id):
    ...
    monkeypatch.setattr(providers, "_get_nastech_home", lambda: tmp_path / "missing-home")
    monkeypatch.delenv("OPENCODE_ZEN_API_KEY", raising=False)
    monkeypatch.delenv("OPENCODE_GO_API_KEY", raising=False)
