...
class TestCronCreatedEventListener:
    """A global `nastech:cron_created` listener must be registered so
    future chat paths can trigger the cron list refresh."""

    def test_listener_registered_at_module_scope(self):
        js = _read("static/panels.js")
        assert re.search(
            r"addEventListener\(\s*['\"]nastech:cron_created['\"]",
            js,
        ), (
            "panels.js must register a window-level 'nastech:cron_created' event listener"
        )

    def test_listener_triggers_load_crons(self):
        js = _read("static/panels.js")
        m = re.search(
            r"addEventListener\(\s*['\"]nastech:cron_created['\"].*?\}\s*\)",
            js,
            re.DOTALL,
        )
        assert m, "nastech:cron_created listener body not found"
        body = m.group(0)
        assert 'loadCrons' in body, (
            "nastech:cron_created listener must call loadCrons() to refresh the list"
        )
