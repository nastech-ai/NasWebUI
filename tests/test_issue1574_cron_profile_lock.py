...
def _install_fake_cron(monkeypatch, run_job, events):
    ...
    cron_jobs.NASTECH_DIR = Path("/tmp/nastech")
    cron_jobs.CRON_DIR = cron_jobs.NASTECH_DIR / "cron"
    ...
    cron_scheduler._nastech_home = Path("/tmp/nastech")
    cron_scheduler._LOCK_DIR = cron_scheduler._nastech_home / "cron"
...
def _write_spawn_fake_agent(root: Path, *, run_job_body: str):
    ...
    (cron_dir / "scheduler.py").write_text(
        "from pathlib import Path\n"
        "_nastech_home = Path('/tmp/nastech')\n"
        "_LOCK_DIR = _nastech_home / 'cron'\n"
...
def _activate_spawn_fake_agent(fake_agent_root: Path):
    ...
    parts = [
        ...
        if p and ("NasTech-Agent" not in p or p == fake_path)
    ]
    os.environ["PYTHONPATH"] = os.pathsep.join([fake_path, *[p for p in parts if p != fake_path]])
    sys.path[:] = [
        p
        for p in sys.path
        if not p or "NasTech-Agent" not in p or p == fake_path
    ]
...
def _real_nastech_agent_editable_install_present() -> bool:
    ...
    resolve; on a maintainer's dev machine an editable install of NasTech-Agent
    ...
    try:
        ...
        _write_spawn_fake_agent(
            ...
            run_job_body=(
                ...
                "    return True, str(scheduler._nastech_home), 'final', None\n"
            ),
        )
        _activate_spawn_fake_agent(fake_agent_root)
        import api.routes as routes
...
def test_manual_cron_subprocess_drains_large_result_before_join(tmp_path):
    ...
    if _real_nastech_agent_editable_install_present():
