...
def test_handle_health_restart_success(monkeypatch):
    ...
    monkeypatch.setattr("api.routes.get_active_nastech_home", lambda: "/mock/nastech/home")

    # Mock shutil.which to find nastech CLI
    monkeypatch.setattr("shutil.which", lambda cmd: "/mock/bin/nastech" if cmd == "naswebui" else None)
    ...
    assert called_args == [["/mock/bin/nastech", "gateway", "restart"]]
    assert called_env.get("NASTECH_HOME") == "/mock/nastech/home"
