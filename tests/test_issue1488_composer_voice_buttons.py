...
class TestVoiceModePreferenceGate:
    ...
    def test_voice_mode_pref_is_localstorage_backed(self):
        """The pref reads from localStorage key 'nastech-voice-mode-button'."""
        src = _src("boot.js")
        assert "'nastech-voice-mode-button'" in src, (
            "boot.js must read/write the localStorage key 'nastech-voice-mode-button' "
    ...
    def test_panels_js_wires_voice_mode_pref(self):
        ...
        assert "'nastech-voice-mode-button'" in src, \
            "panels.js must persist the pref to localStorage key 'nastech-voice-mode-button'."
        assert "_applyVoiceModePref" in src, \
