"""Regression tests for #1240 — WebUI model catalog should delegate to NasTech CLI.

The WebUI picker should not freeze ordinary providers to its static
``_PROVIDER_MODELS`` snapshot when NasTech CLI can return a fresher provider
...
def _install_fake_nastech_cli(monkeypatch, *, provider_id: str, live_ids, raise_on_lookup: bool = False):
    """Install a nastech_cli stub that reports one authenticated provider."""
...
def _configure(monkeypatch, tmp_path, *, provider: str, default: str = ""):
    ...
    nastech_home.mkdir()
    monkeypatch.setattr(profiles, "get_active_nastech_home", lambda: nastech_home)
...
def test_generic_provider_uses_nastech_cli_catalog_before_static_snapshot(monkeypatch, tmp_path):
    ...
    this model was invisible even though NasTech CLI knew about it.
    """
    _scrub_provider_env(monkeypatch)
    calls = _install_fake_nastech_cli(
        monkeypatch,
...
def test_generic_provider_prefixes_live_ids_when_not_active_provider(monkeypatch, tmp_path):
    ...
    # Anthropic is authenticated via NasTech CLI, but OpenAI is the active
