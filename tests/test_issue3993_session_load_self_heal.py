...
def test_clear_stuck_session_helper_exists_and_is_wired():
    ...
    assert "localStorage.removeItem('naswebui-session')" in helper
...
const localStorage = {{ removeItem(k){{ if(k==='naswebui-session') removed=true; }} }};
...
def test_stale_load_guard_present_before_self_heal():
    ...
    assert block.index(guard) < block.index("localStorage.removeItem('naswebui-session')"), \
