...
Covers:
  ...
  4. _status_from_runtime() marks copilot/openai-codex as provider_ready when
     credentials exist
  5. _status_from_runtime() gives a helpful "nastech auth" note (not "API key")
...
def _make_auth_json_with_credential_pool(
    ...
    This reproduces setups where NasTech runtime resolves OAuth credentials from
...
class TestProviderOAuthAuthenticated:
    ...
    def _call(self, provider: str, nastech_home: pathlib.Path) -> bool:
        # Import fresh so we don't get a stale module reference
        from api.onboarding import _provider_oauth_authenticated
        return _provider_oauth_authenticated(provider, nastech_home)
...
class TestStatusFromRuntimeOAuth:
    ...
    def _call(self, provider: str, model: str, nastech_home: pathlib.Path) -> dict:
        from api.onboarding import _status_from_runtime
        import api.onboarding as _ob
        orig_home = _ob._get_active_nastech_home
        orig_found = _ob._NASTECH_FOUND
        _ob._get_active_nastech_home = lambda: nastech_home
        # Simulate NasTech-Agent being available so we reach the provider logic
        ...
        try:
            ...
            _ob._get_active_nastech_home = orig_home
    ...
    def test_copilot_not_ready_without_credentials(self, tmp_path):
        ...
        We mock nastech_cli.auth to be unavailable so the function falls through
        to the auth.json path.  With no auth.json the result must be False.
        """
        import unittest.mock

        # Prevent the nastech_cli fast path from finding real credentials
    ...
    def test_oauth_incomplete_note_mentions_nasmusicui_auth(self, tmp_path):
        ...
        assert "nastech auth" in note or "nastech model" in note, (
            f"Expected 'nastech auth' or 'nastech model' in note, got: {note!r}"
        )
...
class TestApplyOnboardingSetupUnsupportedProvider:
    ...
    def _call(self, provider: str) -> dict:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with unittest.mock.patch("api.onboarding._get_active_nastech_home",
