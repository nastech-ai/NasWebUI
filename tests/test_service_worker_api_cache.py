...
The WebUI can be served at /nastech/. In that deployment API requests look like
/nastech/api/sessions, not /api/sessions. The service worker must treat those as
...
def test_service_worker_excludes_subpath_mounted_api_routes_from_cache():
    assert "url.pathname.includes('/api/')" in SW_SRC, (
        ...
        "/nastech/api/sessions, not only root-mounted /api/*"
    )
...
def test_service_worker_excludes_subpath_mounted_health_routes_from_cache():
    assert "url.pathname.includes('/health')" in SW_SRC, (
        ...
        "/nastech/health, not only root-mounted /health"
    )
