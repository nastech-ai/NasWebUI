...
Python environment (the agent venv). They are skipped when NasTech-Agent is
...
def _server_nastech_home() -> pathlib.Path:
    """Get the nastech home path the test server is actually using.
    ...
    conftest is imported multiple times under different NASTECH_HOME environments
    (api.config resets NASTECH_HOME at module import time via init_profile_state).
    """
    ...
    # Fallback
    nastech_home = pathlib.Path.home() / ".nastech"
    return nastech_home / "webui-mvp-test"
...
def test_onboarding_setup_openrouter_writes_real_config_and_env():
    ...
    cfg_text = (_server_nastech_home() / "config.yaml").read_text(encoding="utf-8")
    env_text = (_server_nastech_home() / ".env").read_text(encoding="utf-8")
...
def test_onboarding_setup_detects_incomplete_saved_provider():
    ...
    (_server_nastech_home() / ".env").unlink(missing_ok=True)
...
def test_onboarding_complete_persists_flag():
    ...
    settings = json.loads(
        (_server_nastech_home() / "settings.json").read_text(encoding="utf-8")
    )
