"""Regression tests for #4413 — provider-model seeder from nastech_cli.
...
sys.path.insert(0, str(REPO.parent / ".nastech" / "nastech-agent"))
...
def _patch_core_pm(fake_pm: dict):
    """Patch ``nastech_cli.models._PROVIDER_MODELS`` with *fake_pm*."""
    # Build a fake nastech_cli.models module with just _PROVIDER_MODELS.
    fake_module = mock.MagicMock()
    fake_module._PROVIDER_MODELS = fake_pm
    return mock.patch.dict(sys.modules, {"nastech_cli.models": fake_module})
...
class TestSeederAddsMissingModels:
    ...
    def test_no_op_without_nastech_cli(self, restore_providers):
        """Seeder must be a no-op (not raise) when nastech_cli is unavailable."""
        with mock.patch.dict(sys.modules, {"nastech_cli.models": None}):
...
class TestExceptionNarrowing:
    """The seeder's nastech_cli import should only catch ImportError, not all
    ...
    def test_does_not_swallow_runtime_errors(self, restore_providers):
        ...
        with _patch_core_pm(fake_pm):
            with mock.patch(
                ...
                # catches ImportError for the optional nastech_cli import.
