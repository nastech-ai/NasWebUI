...
def test_switch_profile_returns_target_workspace_not_current(tmp_path, monkeypatch):
    ...
    # Patch _DEFAULT_NASTECH_HOME to our tmp dir
    orig_default = profiles._DEFAULT_NASTECH_HOME
    profiles._DEFAULT_NASTECH_HOME = default_home
    ...
    try:
        ...
        profiles._DEFAULT_NASTECH_HOME = orig_default
...
def test_regression_switch_profile_default_workspace_not_from_process_global(tmp_path, monkeypatch):
    ...
    orig_default = profiles._DEFAULT_NASTECH_HOME
    orig_active = profiles._active_profile
    profiles._DEFAULT_NASTECH_HOME = base
    profiles._active_profile = "default"
    profiles._tls.profile = None
...
def test_regression_switch_profile_returns_target_model():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        orig = profiles._DEFAULT_NASTECH_HOME
        ...
        try:
            ...
            profiles._DEFAULT_NASTECH_HOME = orig
...
def test_get_config_reloads_when_request_profile_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("NASTECH_CONFIG_PATH", raising=False)
    ...
    monkeypatch.setattr(
        config,
        "_get_config_path",
        lambda: profiles.get_active_nastech_home() / "config.yaml",
    )

    orig_default_home = profiles._DEFAULT_NASTECH_HOME
    orig_active = profiles._active_profile
    ...
    try:
        ...
        profiles._DEFAULT_NASTECH_HOME = orig_default_home
        profiles._active_profile = orig_active
        config._cfg_cache.clear()
