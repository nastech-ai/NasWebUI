...
1. **Endpoint disagreement (the bug):** The Settings → Providers card and the
   ...
   ``list_available_providers().authenticated``. On some nastech_cli versions
...
def _install_fake_nastech_cli(
    ...
    *,
    ...
    """Install fake ``nastech_cli`` modules with controllable Nous behavior.

    The two flags ``list_authenticated`` and ``auth_status_logged_in`` model
    the divergence between ``nastech_cli.models.list_available_providers()``
    and ``nastech_cli.auth.get_auth_status()`` that #1567 calls out as a
    real-world pattern on some nastech_cli versions.
    """
...
class TestNousDetectionSymmetry:
    ...
    def test_picker_omits_nous_when_both_auth_signals_false(self, monkeypatch, tmp_path):
        ...
        nastech_cli-unavailable case only."""
        _scrub_provider_env(monkeypatch)
        _install_fake_nastech_cli(
            monkeypatch,
...
class TestNousLiveFetchEmpty:
    """When authenticated but live-fetch returns [] (transient nastech_cli
    ...
    def test_nastech_cli_unavailable_falls_back_to_static_4(self, monkeypatch, tmp_path):
        """When nastech_cli is unavailable (raises) — distinct from returning [] —
        ...
        try:
            ...
            nous_groups = [g for g in data["groups"] if g["provider_id"] == "nous"]
            assert nous_groups, (
                "When nastech_cli raises, Nous group MUST still appear with "
