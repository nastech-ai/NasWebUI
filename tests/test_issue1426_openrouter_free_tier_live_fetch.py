...
OpenRouter group:
  ...
  (1) `nastech_cli.models.fetch_openrouter_models()` — the curated tool-supporting
...
def _isolate_openrouter_cache(monkeypatch):
    ...
    try:
        from nastech_cli import models as _hm
...
def test_openrouter_falls_back_to_static_when_live_fails(monkeypatch):
    """If both nastech_cli.fetch and the direct urlopen raise, the picker
    ...
    # Force nastech_cli to fail too
    import sys
    fake_module = type(sys)("nastech_cli.models")
    ...
    monkeypatch.setitem(sys.modules, "nastech_cli.models", fake_module)
