...
def _fake_agent(tmp_path):
    ...
    cli_dir = agent_dir / "nastech_cli"
    cli_dir.mkdir(parents=True)
    (cli_dir / "main.py").write_text("print('fake nastech cli')\n", encoding="utf-8")
    return agent_dir
...
def test_gateway_start_runs_profile_scoped_agent_cli_and_returns_status(monkeypatch, tmp_path):
    ...
    cmd, kwargs = calls[0]
    assert cmd[:2] == [sys.executable, str(agent_dir / "nastech_cli" / "main.py")]
...
def test_gateway_action_contention_returns_409_without_spawning(monkeypatch, tmp_path):
    ...
    NOT spawn an overlapping `nastech gateway` subprocess (server-side
    single-flight guard)."""
...
def test_gateway_lifecycle_missing_cli_returns_sanitized_error(monkeypatch, tmp_path):
    ...
    assert data["ok"] is False
    assert data["action"] == "start"
    assert data["error"] == "NasTech agent CLI entrypoint not found"
