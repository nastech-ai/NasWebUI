...
Users with older NasTech-Agent versions (missing 'source' column in state.db) got
...
class TestCliSessionsErrorSurface:
    ...
    def test_missing_source_column_suggests_upgrade(self):
        """Warning message must suggest upgrading NasTech-Agent."""
        assert "Upgrade NasTech-Agent" in combined_src or "upgrade NasTech-Agent" in combined_src.lower()
