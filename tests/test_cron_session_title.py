...
Session ID format produced by NasTech-Agent: cron_<job_id>_<YYYYMMDD>_<HHMMSS>
"""
import json
import sqlite3
...
def _write_jobs_json(nasmusicui_home, jobs):
    """Write cron/jobs.json with the given jobs list."""
    cron_dir = nasmusicui_home / "cron"
...
def fake_nasmusicui_home(tmp_path, monkeypatch):
    """Point get_cli_sessions() at a temporary NASTECH_HOME and disable
...
def test_cron_session_uses_job_name_when_title_missing(fake_nasmusicui_home):
    """A cron session with no title should display the friendly job name."""
    _write_jobs_json(fake_nasmusicui_home, [
        {"id": "cd65df6fc1a8", "name": "wiki-auto-ingest"},
    ])
    _make_state_db(fake_nasmusicui_home / "state.db", [
...
def test_cron_session_falls_back_when_jobs_json_missing(fake_nasmusicui_home):
...
def test_cron_session_falls_back_when_job_id_not_in_jobs_json(fake_nasmusicui_home):
...
def test_explicit_title_is_preserved(fake_nasmusicui_home):
...
def test_non_cron_sessions_unaffected(fake_nasmusicui_home):
    ...
    # filtering.
    conn = sqlite3.connect(str(fake_nasmusicui_home / "state.db"))
    conn.execute(
        "INSERT INTO messages (session_id, timestamp) VALUES (?, ?)",
