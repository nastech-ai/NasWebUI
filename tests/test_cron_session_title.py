...
def _write_jobs_json(naswebui_home, jobs):
    """Write cron/jobs.json with the given jobs list."""
    cron_dir = naswebui_home / "cron"
...
def test_cron_session_uses_job_name_when_title_missing(fake_naswebui_home):
    """A cron session with no title should display the friendly job name."""
    _write_jobs_json(fake_naswebui_home, [
        {"id": "cd65df6fc1a8", "name": "wiki-auto-ingest"},
    ])
    _make_state_db(fake_naswebui_home / "state.db", [
...
def test_cron_session_falls_back_when_jobs_json_missing(fake_naswebui_home):
...
def test_cron_session_falls_back_when_job_id_not_in_jobs_json(fake_naswebui_home):
...
def test_explicit_title_is_preserved(fake_naswebui_home):
...
def test_non_cron_sessions_unaffected(fake_naswebui_home):
    ...
    # filtering.
    conn = sqlite3.connect(str(fake_naswebui_home / "state.db"))
    conn.execute(
        "INSERT INTO messages (session_id, timestamp) VALUES (?, ?)",
