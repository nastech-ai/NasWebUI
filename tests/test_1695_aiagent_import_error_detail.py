...
Patrick-81 reported a symlinked NasTech-Agent install that produced a bare
"AIAgent not available -- check that NasTech-Agent is on sys.path" error with
...
class TestAIAgentImportErrorDetail:
    ...
    def test_preserves_original_message_for_log_scrapers(self):
        ...
        assert first == "AIAgent not available -- check that NasTech-Agent is on sys.path", (
    ...
    def test_lists_sys_path_entries_when_relevant(self, monkeypatch):
        """If sys.path contains entries mentioning nastech/agent, the diagnostic
        ...
        monkeypatch.syspath_prepend("/fake/NasTech-Agent")
        out = helper()
        assert "/fake/NasTech-Agent" in out

    def test_handles_no_relevant_sys_path_entries(self, monkeypatch):
        """If sys.path has NO nastech/agent-related entries, the diagnostic must
        say so explicitly — this is itself a strong diagnostic signal.
        """
        helper = _import_helper()
        # Replace sys.path with entries that mention neither nastech nor agent.
        ...
        out = helper()
        assert "no entries mention nastech or agent" in out, (
