...
specific NASTECH_HOME / NASWEBUI_STATE_DIR. We do this in a SUBPROCESS rather
...
def _platform_default_state_dir() -> Path:
    """What STATE_DIR resolves to with NASTECH_HOME + STATE_DIR both unset."""
    return _state_dir_for_env()


def test_config_state_dir_defaults_to_nastech_home_webui(tmp_path):
    nastech_home = tmp_path / ".nastech" / "profiles" / "isolated"
    nastech_home.mkdir(parents=True)

    state_dir = _state_dir_for_env(NASTECH_HOME=nastech_home)
    assert state_dir == (nastech_home / "webui").resolve()


def test_config_state_dir_unchanged_for_normal_install_nastech_home_unset():
    """Backward-compat: with NASTECH_HOME unset, STATE_DIR stays at the platform
    default `<~/.nastech>/webui` — a normal install's state must NOT relocate
    (the #4449/#4454 state-dir move only affects an explicitly-set NASTECH_HOME).

    Cross-check: the unset-NASTECH_HOME result must NOT equal the result of
    pointing NASTECH_HOME at an arbitrary other base — i.e. the default is
    ...
    # Pointing NASTECH_HOME elsewhere produces a DIFFERENT dir, proving the unset
    # case resolves to the platform default rather than echoing an injected base.
    elsewhere = _state_dir_for_env(NASTECH_HOME="/tmp/nastech-4449-elsewhere-base")
    assert elsewhere == Path("/tmp/nastech-4449-elsewhere-base/webui").resolve()
    assert default != elsewhere


def test_config_state_dir_explicit_override_takes_precedence(tmp_path):
    """NASWEBUI_STATE_DIR always wins over the NASTECH_HOME-derived default,
