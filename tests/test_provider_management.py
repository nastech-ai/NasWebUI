...
def _install_fake_nastech_cli(monkeypatch):
...
class TestGetProviders:
    ...
    def test_openai_codex_provider_card_prefers_live_catalog(self, monkeypatch, tmp_path):
        ...
        /api/models already uses nastech_cli/Codex cache discovery for Codex.  The
...
class TestSetProviderKey:
    ...
    def test_set_key_writes_to_env_file(self, monkeypatch, tmp_path):
        ...
        try:
            ...
            assert env_path.exists(), f".env not written to {env_path}; NASTECH_HOME={__import__('os').environ.get('NASTECH_HOME')!r}"
...
class TestIssue1410OllamaEnvVarBleed:
    ...
    nastech_cli/runtime_provider.py only consumes OLLAMA_API_KEY when the
