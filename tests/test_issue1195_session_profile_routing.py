...
def _make_nasmusicui_home(base: Path, profile_name: str | None = None) -> Path:
    """Create a temp NASTECH_HOME (with optional profile dir) and return it."""
    nasmusicui_home = base / ".nastech"
    nasmusicui_home.mkdir(parents=True, exist_ok=True)
    if profile_name:
        (nasmusicui_home / "profiles" / profile_name).mkdir(parents=True, exist_ok=True)
    return nasmusicui_home


# ── tests ────────────────────────────────────────────────────────────────────

class TestGetNasMusicUIHomeForProfile:
    """get_nasmusicui_home_for_profile() must return the profile path regardless of
    whether the directory already exists on disk (#1195)."""

    @pytest.fixture(autouse=True)
    def _patch_default_home(self, tmp_path):
        """Patch _DEFAULT_NASTECH_HOME to a temp directory for isolation."""
        from api.profiles import _DEFAULT_NASTECH_HOME as real_default

        fake_home = tmp_path / ".nastech"
        fake_home.mkdir(parents=True)
        with patch("api.profiles._DEFAULT_NASTECH_HOME", fake_home):
    ...
    def test_existing_profile_returns_profile_dir(self, _patch_default_home):
        ...
        from api.profiles import get_nasmusicui_home_for_profile

        # Create an existing profile directory
        profile_dir = fake_home / "profiles" / "ayan"
        profile_dir.mkdir(parents=True)

        result = get_nasmusicui_home_for_profile("ayan")
    ...
    def test_nonexistent_profile_still_returns_profile_path(self, _patch_default_home):
        ...
        result = get_nasmusicui_home_for_profile("newprofile")
    ...
    def test_none_returns_default(self, _patch_default_home):
        ...
        result = get_nasmusicui_home_for_profile(None)
    ...
    def test_empty_string_returns_default(self, _patch_default_home):
        ...
        result = get_nasmusicui_home_for_profile("")
        assert result == fake_home
    ...
    def test_default_string_returns_default(self, _patch_default_home):
        ...
        result = get_nasmusicui_home_for_profile("default")
        assert result == fake_home
