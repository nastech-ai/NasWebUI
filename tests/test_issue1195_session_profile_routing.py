...
def _make_naswebui_home(base: Path, profile_name: str | None = None) -> Path:
    """Create a temp NASTECH_HOME (with optional profile dir) and return it."""
    naswebui_home = base / ".nastech"
    naswebui_home.mkdir(parents=True, exist_ok=True)
    if profile_name:
        (naswebui_home / "profiles" / profile_name).mkdir(parents=True, exist_ok=True)
    return naswebui_home


# ── tests ────────────────────────────────────────────────────────────────────

class TestGetNasWebUIHomeForProfile:
    """get_naswebui_home_for_profile() must return the profile path regardless of
    ...
    def test_existing_profile_returns_profile_dir(self, _patch_default_home):
        ...
        from api.profiles import get_naswebui_home_for_profile

        # Create an existing profile directory
        profile_dir = fake_home / "profiles" / "ayan"
        profile_dir.mkdir(parents=True)

        result = get_naswebui_home_for_profile("ayan")
    ...
    def test_nonexistent_profile_still_returns_profile_path(self, _patch_default_home):
        ...
        result = get_naswebui_home_for_profile("newprofile")
    ...
    def test_none_returns_default(self, _patch_default_home):
        ...
        result = get_naswebui_home_for_profile(None)
    ...
    def test_empty_string_returns_default(self, _patch_default_home):
        ...
        result = get_naswebui_home_for_profile("")
        assert result == fake_home
    ...
    def test_default_string_returns_default(self, _patch_default_home):
        ...
        result = get_naswebui_home_for_profile("default")
        assert result == fake_home
