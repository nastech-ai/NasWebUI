...
class TestUpdateChecker:
    ...
    def test_check_repo_includes_compare_url_from_normalized_remote_and_merge_base(self, tmp_path, monkeypatch):
        ...
        def fake_run(args, cwd, timeout=10):
            ...
            if args[:2] == ['remote', 'get-url']:
                return 'git@github.com:NousResearch/NasTech-Agent.git', True
            return '', True

        monkeypatch.setattr(upd, '_run_git', fake_run)
        result = upd._check_repo(tmp_path, 'agent')

        assert result['repo_url'] == 'https://github.com/NousResearch/NasTech-Agent'
        assert result['current_sha'] == 'abcdef1'
        assert result['latest_sha'] == 'def5678'
        assert result['compare_url'] == 'https://github.com/NousResearch/NasTech-Agent/compare/abcdef1...def5678'
    ...
    def test_release_check_ignores_post_release_branch_commits(self, tmp_path, monkeypatch):
        ...
        def fake_run(args, cwd, timeout=10):
            ...
            if args[:2] == ['remote', 'get-url']:
                return 'https://github.com/NousResearch/NasTech-Agent.git', True
            if args[:2] == ['rev-parse', '--abbrev-ref']:
                return 'origin/main', True
            if args[:2] == ['rev-list', '--count']:
                return '16', True
    ...
    def test_detect_agent_version_reads_copied_source_tree(self, tmp_path, monkeypatch):
        ...
        package_dir = agent_dir / 'nastech_cli'
    ...
    def test_detect_agent_version_falls_back_to_gateway_health(self, monkeypatch):
        ...
        class FakeResponse:
            ...
            def read(self):
                return b'{"status":"ok","platform":"NasTech-Agent","version":"0.14.1"}'
        ...
        monkeypatch.setenv('GATEWAY_HEALTH_URL', 'http://NasTech-Agent:8642/health')
        monkeypatch.setattr(upd.urllib.request, 'urlopen', fake_urlopen)

        assert upd._detect_agent_version() == '0.14.1'
        assert seen == [('http://NasTech-Agent:8642/health', 0.75)]
...
class TestUpdateSummaryRouteModelSelection:
    ...
    def test_summary_route_auxiliary_model_uses_active_profile_env(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles, 'get_nastech_home_for_profile', lambda profile: profile_home)
        monkeypatch.setattr(
            profiles,
            'get_profile_runtime_env',
            lambda home: {'NASTECH_TEST_PROFILE_ENV': 'work-runtime'},
        )
        ...
        def fake_resolve_model_provider(model):
            ...
            captured['model_resolution_env'] = {
                ...
                'NASTECH_TEST_PROFILE_ENV': os.environ.get('NASTECH_TEST_PROFILE_ENV'),
                'THREAD_NASTECH_HOME': thread_env.get('NASTECH_HOME'),
                'THREAD_NASTECH_TEST_PROFILE_ENV': thread_env.get('NASTECH_TEST_PROFILE_ENV'),
            }
        ...
        fake_runtime_provider = types.ModuleType('nastech_cli.runtime_provider')
        ...
        fake_nastech_cli.__path__ = []
        fake_nastech_cli.runtime_provider = fake_runtime_provider
        monkeypatch.setitem(sys.modules, 'nastech_cli', fake_nastech_cli)
        monkeypatch.setitem(sys.modules, 'nastech_cli.runtime_provider', fake_runtime_provider)
        ...
        def fake_get_text_auxiliary_client(task, main_runtime=None):
            ...
            captured['aux_env'] = {
                ...
                'THREAD_NASTECH_HOME': thread_env.get('NASTECH_HOME'),
                'THREAD_NASTECH_TEST_PROFILE_ENV': thread_env.get('NASTECH_TEST_PROFILE_ENV'),
                'SKILL_MODULE_HOME': getattr(fake_skill_module, 'NASTECH_HOME'),
        ...
        monkeypatch.setenv('NASTECH_TEST_PROFILE_ENV', 'default-runtime')
        ...
        assert captured['model_resolution_env'] == {
            ...
            'NASTECH_TEST_PROFILE_ENV': 'work-runtime',
            'THREAD_NASTECH_HOME': str(profile_home),
            'THREAD_NASTECH_TEST_PROFILE_ENV': 'work-runtime',
        }
        ...
        assert getattr(fake_skill_module, 'NASTECH_HOME') == 'default-home'
        assert getattr(fake_skill_module, 'SKILLS_DIR') == 'default-home/skills'
        assert os.environ.get('NASTECH_HOME') == 'default-home'
        assert os.environ.get('NASTECH_TEST_PROFILE_ENV') == 'default-runtime'
...
class TestWhatsNewSummaryToggle:
    ...
    def test_update_banner_summary_flow_keeps_diff_links_after_summary(self):
        ...
        assert 'nastech-whats-new-generated-summaries' in src
    ...
    def test_update_summary_deduplicates_repeated_agent_summary_bullets(self):
        ...
        duplicate_menu_item = (
            'The `nastech tools` menus should open noticeably faster, especially when checking available tools or auth state.'
        )
