...
def _both_callsites():
    ...
    excluded because they exist precisely to support older NasTech-Agent
...
def test_legacy_signature_fallback_present():
    """Older NasTech-Agent builds may not yet have config_context_length on
    ...
    assert "except TypeError:" in STREAMING_PY, (
        "Both callsites must catch TypeError to support older NasTech-Agent "
...
def test_routes_session_load_fallback_passes_config_overrides():
    ...
    # Legacy fallback for older NasTech-Agent builds that pre-date the kwargs.
    assert "except TypeError:" in helper, (
        "session-load fallback must catch TypeError to support older "
        "NasTech-Agent builds without the new kwargs."
    )
