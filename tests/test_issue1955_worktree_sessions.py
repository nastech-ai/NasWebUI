...
def test_worktree_metadata_round_trips_through_session_file(_isolate_sessions):
    s = Session(
        ...
        workspace=str(_isolate_sessions.parent / "repo" / ".worktrees" / "nastech-1234"),
        worktree_path=str(_isolate_sessions.parent / "repo" / ".worktrees" / "nastech-1234"),
        worktree_branch="nastech/nastech-1234",
    ...
    assert raw["worktree_path"].endswith(".worktrees/nastech-1234")
    assert raw["worktree_branch"] == "nastech/nastech-1234"
...
def test_new_session_with_worktree_info_persists_immediately(_isolate_sessions):
    ...
    s = new_session(
        ...
        worktree_info={
            "path": str(worktree),
            "branch": "nastech/nastech-abcd1234",
            "repo_root": str(repo),
            "created_at": 456.0,
        },
    )
    ...
    assert s.worktree_branch == "nastech/nastech-abcd1234"
...
def test_empty_worktree_session_remains_visible_in_sidebar(_isolate_sessions):
    repo = _isolate_sessions.parent / "repo"
    worktree = repo / ".worktrees" / "nastech-visible"
    worktree.mkdir(parents=True)
    ...
    s = new_session(
        ...
        worktree_info={
            "path": str(worktree),
            "branch": "nastech/nastech-visible",
            "repo_root": str(repo),
            "created_at": 789.0,
        },
    )
...
def test_create_worktree_for_workspace_calls_agent_setup_with_repo_root(tmp_path, monkeypatch):
    ...
    def fake_setup(repo_root):
        ...
        return {
            "path": str(repo / ".worktrees" / "nastech-test"),
            "branch": "nastech/nastech-test",
            "repo_root": str(repo),
        }
    ...
    assert info["path"].endswith(".worktrees/nastech-test")
    assert info["branch"] == "nastech/nastech-test"
...
def test_session_new_route_creates_worktree_backed_session(tmp_path, monkeypatch):
    ...
    repo = tmp_path / "repo"
    worktree = repo / ".worktrees" / "nastech-route"
    repo.mkdir()
    worktree.mkdir(parents=True)
    ...
    assert session["worktree_branch"] == "nastech/nastech-route"
