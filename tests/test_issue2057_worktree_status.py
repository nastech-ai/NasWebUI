...
def git_worktree(tmp_path):
    ...
    worktree = tmp_path / "nastech-status"
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.email", "test@example.com")
    _git(repo, "config", "user.name", "NasTech Test")
    ...
    _git(repo, "worktree", "add", "-b", "nastech/status", str(worktree), "main")
    _git(worktree, "push", "-u", "origin", "nastech/status")
...
def _session_for_worktree(repo, worktree, **kwargs):
    return Session(
        ...
        worktree_branch="nastech/status",
