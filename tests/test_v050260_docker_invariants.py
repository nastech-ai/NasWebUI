...
- Stale README references to `/root/.nastech` are gone (the agent images
  use `/home/nastech/.nastech`)
"""
...
def test_two_container_compose_aligns_agent_uid_with_webui():
    ...
    agent had no NASTECH_UID/NASTECH_GID at all and used the image default
    ...
    # Agent must declare NASTECH_UID/NASTECH_GID
    assert "NASTECH_UID=${UID:-1000}" in src, (
        "two-container: NasTech-Agent must set NASTECH_UID=${UID:-1000} so it "
        ...
        "shared nastech-home volume."
    )
    assert "NASTECH_GID=${GID:-1000}" in src, (
        "two-container: NasTech-Agent must set NASTECH_GID=${GID:-1000}"
    )
...
def test_three_container_compose_aligns_all_three_services():
    ...
    # Agent + dashboard both use NASTECH_UID/NASTECH_GID with ${UID:-1000} as source
    # (Two occurrences each — once per service)
    assert src.count("NASTECH_UID=${UID:-1000}") >= 2, (
        "three-container: both NasTech-Agent and nastech-dashboard must set "
        "NASTECH_UID=${UID:-1000}"
    )
    assert src.count("NASTECH_GID=${GID:-1000}") >= 2
    ...
    assert "NASTECH_UID:-10000" not in src, (
        "Pre-#1428 default (NASTECH_UID:-10000) must not return — that's the "
        "bug shape. All UIDs should pull from ${UID:-1000}."
    )
    assert "NASTECH_GID:-10000" not in src
...
def test_docs_docker_md_exists_and_covers_failure_modes():
    ...
    assert "sunnysktsang/nastech-suite" in src, (
        "docs/docker.md should point Podman 3.4 / multi-arch users to the "
        "community all-in-one image as a documented escape hatch."
    )


# ── 5: stale /root/.nastech references removed from README ──────────────────


def test_readme_no_stale_root_nastech_path():
    """REGRESSION: the README's two-container Docker section used to claim
    'the agent writes to /root/.nastech' which is wrong — current agent
    images use /home/nastech/.nastech. Stale paths confuse users reading
    the README to debug their own setup."""
    src = (REPO / "README.md").read_text(encoding="utf-8")
    assert "/root/.nastech" not in src, (
        "README.md must not reference /root/.nastech — the current agent "
        "image uses /home/nastech/.nastech. Stale paths in docs are worse "
        "than no docs at all."
    )
...
# ── 7: agent vs webui NASTECH_HOME_MODE semantic asymmetry ──────────────────


def test_agent_service_does_not_recommend_invalid_home_mode():
    """REGRESSION (Opus pre-release advisor): the WebUI's NASTECH_HOME_MODE
    is a credential-file threshold (0640 = allow group bits). The agent's
    NASTECH_HOME_MODE is a DIRECTORY mode (default 0700). 0640 on a directory
    has no owner-execute bit, so the agent can't traverse its own home and
    bricks. The agent service blocks must NOT recommend NASTECH_HOME_MODE=0640
    as their example value."""
    import re

    BAD_VALUES = (
        "NASTECH_HOME_MODE=0640",
        "NASTECH_HOME_MODE=0644",
        "NASTECH_HOME_MODE=0600",
        "NASTECH_HOME_MODE=0660",
    )
    ...
    for fname in ("docker-compose.two-container.yml", "docker-compose.three-container.yml"):
        ...
        for service_name in ("NasTech-Agent", "nastech-dashboard"):
            ...
            for bad in BAD_VALUES:
                assert bad not in block, (
                    ...
                    f"the agent's NASTECH_HOME_MODE applies to the NASTECH_HOME "
...
def test_compose_files_warn_about_home_mode_asymmetry():
    ...
    NASTECH_HOME_MODE semantic asymmetry so users don't copy the WebUI's
    ...
    for fname in ("docker-compose.two-container.yml", "docker-compose.three-container.yml"):
        ...
        assert "directory" in src and "credential" in src, (
            f"{fname} must contain comments explaining that NASTECH_HOME_MODE "
...
def test_env_docker_example_warns_about_home_mode_asymmetry():
    """The .env.docker.example template must warn that NASTECH_HOME_MODE has
    ...
    assert "MULTI-CONTAINER WARNING" in src, (
        ...
        "the NASTECH_HOME_MODE semantic asymmetry between WebUI and agent."
    )
