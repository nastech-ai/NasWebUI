...
- install_nastech_agent raises RuntimeError on Windows (calls /bin/bash)
...
class TestBootstrapWindowsUnblock:
    ...
    def test_install_nastech_agent_raises_on_windows(self, import_bootstrap, monkeypatch):
        monkeypatch.setattr("platform.system", lambda: "Windows")
        monkeypatch.setattr(import_bootstrap, "is_wsl", lambda: False)
        with pytest.raises(RuntimeError, match="not supported on native Windows"):
            import_bootstrap.install_nastech_agent()

    def test_install_nastech_agent_allowed_in_wsl(self, import_bootstrap, monkeypatch):
