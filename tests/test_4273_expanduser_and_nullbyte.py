...
not blocked (#510/#521: NasWebUI commonly runs as root, where /root is the
...
def test_root_remains_unblocked_for_root_deployments():
    ...
    # (NasWebUI-as-root deployments). This pins that our null-byte work did not
    ...
    assert "'/root'" not in src and "PurePosixPath('/root')" not in src, (
        "/root must not be blocked — breaks NasWebUI-as-root deployments (#510/#521)"
    )
