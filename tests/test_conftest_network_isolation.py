...
def test_rfc1918_private_ipv4_is_allowed():
    ...
    assert _conftest._naswebui_addr_is_local("10.0.0.5") is True
    assert _conftest._naswebui_addr_is_local("172.16.5.1") is True
    assert _conftest._naswebui_addr_is_local("172.31.255.254") is True
    assert _conftest._naswebui_addr_is_local("192.168.1.22") is True
...
def test_link_local_is_allowed():
    ...
    import tests.conftest as _conftest
    assert _conftest._naswebui_addr_is_local("169.254.169.254") is True
...
def test_reserved_tlds_are_allowed():
    ...
    import tests.conftest as _conftest
    assert _conftest._naswebui_addr_is_local("example.com") is True
    assert _conftest._naswebui_addr_is_local("my-mac.tailnet.example") is True
    assert _conftest._naswebui_addr_is_local("anything.invalid") is True
    assert _conftest._naswebui_addr_is_local("test-host.test") is True
    assert _conftest._naswebui_addr_is_local("printer.local") is True
    assert _conftest._naswebui_addr_is_local("localhost") is True


def test_public_ipv4_is_blocked():
    """Public IPs must NOT be treated as local."""
    import tests.conftest as _conftest
    assert _conftest._naswebui_addr_is_local("8.8.8.8") is False
    assert _conftest._naswebui_addr_is_local("1.1.1.1") is False
    assert _conftest._naswebui_addr_is_local("203.0.113.0") is True  # TEST-NET-3
    assert _conftest._naswebui_addr_is_local("204.0.113.0") is False  # outside
...
def test_allow_outbound_network_fixture_unswaps_the_wrappers(allow_outbound_network):
    ...
    assert "_naswebui_blocked_create_connection" not in getattr(
        socket.create_connection, "__qualname__", ""
    ), "allow_outbound_network fixture did not restore the real create_connection"
    assert "_naswebui_blocked_socket_connect" not in getattr(
...
def test_block_is_active_outside_the_fixture():
    ...
    under multiple roots (which produces two distinct function objects with
    the same __qualname__ but different `is` identity)."""
    assert "_naswebui_blocked_create_connection" in getattr(
        socket.create_connection, "__qualname__", ""
    ), "default state should have the blocked wrapper installed on socket.create_connection"
    assert "_naswebui_blocked_socket_connect" in getattr(
        socket.socket.connect, "__qualname__", ""
