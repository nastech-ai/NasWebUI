...
before the `NASTECH_HOME` env mutation that stamps the per-session profile.
Result: `_load_mcp_config()` always read the default profile's
`~/.nastech/config.yaml`, never the non-default profile's MCP servers.

The fix moves the call past the `_ENV_LOCK` env-mutation block so
`discover_mcp_tools()` runs with the correct `NASTECH_HOME` for the session's
profile.
...
def test_discover_mcp_tools_called_after_nasmusicui_home_mutation():
    ...
    """
    home_set_line = _line_of(r"os\.environ\['NASTECH_HOME'\]\s*=\s*_profile_home")
    ...
    assert discover_call_line > home_set_line, (
        ...
        f"NASTECH_HOME mutation at line {home_set_line} (issue #1968). "
...
def test_discover_mcp_tools_called_after_env_lock_release():
    ...
    NasTech-Agent), and holding the env lock across that would serialize all
