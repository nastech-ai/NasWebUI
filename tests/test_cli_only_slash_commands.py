"""Regression tests for WebUI handling of NasTech CLI-only slash commands."""
...
def test_cli_only_response_mentions_webui_and_cli_scope():
    ...
    assert "NasTech CLI-only command" in COMMANDS_JS
...
def test_browser_cli_only_response_explains_server_side_browser_tools():
    ...
    assert "`/browser` itself only works in `nastech chat`" in response
...
def test_cli_only_response_helper_uses_canonical_command_name():
    ...
    assert "`/browser` is a NasTech CLI-only command" in result["response"]
