...
Covers:
- NASWEBUI_SKIP_ONBOARDING=1 bypasses the wizard unconditionally (chat_ready not required)
- NASWEBUI_SKIP_ONBOARDING unset leaves default behaviour unchanged
...
class TestSkipOnboardingEnvVar(unittest.TestCase):
    ...
    def test_skip_env_1_and_chat_ready_marks_completed(self):
        """NASWEBUI_SKIP_ONBOARDING=1 + chat_ready=True → completed=True."""
        status = self._run_status(_READY_RUNTIME, {"NASWEBUI_SKIP_ONBOARDING": "1"})
    ...
    def test_skip_env_true_and_chat_ready_marks_completed(self):
        """NASWEBUI_SKIP_ONBOARDING=true also accepted."""
        status = self._run_status(_READY_RUNTIME, {"NASWEBUI_SKIP_ONBOARDING": "true"})
        self.assertTrue(status["completed"])

    def test_skip_env_yes_and_chat_ready_marks_completed(self):
        """NASWEBUI_SKIP_ONBOARDING=yes also accepted."""
        status = self._run_status(_READY_RUNTIME, {"NASWEBUI_SKIP_ONBOARDING": "yes"})
        self.assertTrue(status["completed"])

    def test_skip_env_1_works_even_when_not_chat_ready(self):
        """NASWEBUI_SKIP_ONBOARDING=1 skips unconditionally — chat_ready is NOT required."""
        status = self._run_status(_NOT_READY_RUNTIME, {"NASWEBUI_SKIP_ONBOARDING": "1"})
        self.assertTrue(status["completed"],
    ...
    def test_skip_env_unset_leaves_default_false(self):
        ...
        env = {k: v for k, v in os.environ.items() if k != "NASWEBUI_SKIP_ONBOARDING"}
...
class TestApplyOnboardingSkipGuard(unittest.TestCase):
    ...
    def test_apply_setup_blocked_when_skip_env_set(self):
        ...
        with patch.dict(os.environ, {"NASWEBUI_SKIP_ONBOARDING": "1"}, clear=False), \
