...
def test_settings_default_bot_name():
    """GET /api/settings should return bot_name defaulting to 'NasTech'."""
    d, status = get("/api/settings")
    assert status == 200
    assert "bot_name" in d
    assert d["bot_name"] == "NasTech"
...
def test_settings_empty_bot_name_defaults_to_nasmusicui():
    ...
    try:
        ...
        assert status == 200
        assert d.get("bot_name") == "NasTech"
        d2, _ = get("/api/settings")
        assert d2.get("bot_name") == "NasTech"
    finally:
        post("/api/settings", {"bot_name": "NasTech"})
...
def test_login_page_shows_default_bot_name():
    """GET /login should contain 'NasTech' in title and h1 when default."""
    html, status = get_raw("/login")
    assert status == 200
    assert "<title>NasTech" in html
    assert "<h1>NasTech</h1>" in html
