...
def test_gateway_watcher_pins_explicit_profile_home(tmp_path, monkeypatch):
    ...
    watcher = gw.GatewayWatcher(nastech_home=profile_a, profile_name="a")
...
def test_restart_watcher_for_profile_replaces_singleton_with_profile_home(tmp_path, monkeypatch):
    ...
    class FakeWatcher:
        def __init__(self, *, profile_name="", nastech_home=None, state_db_path=None):
            self.profile_name = profile_name
            self.nastech_home = nastech_home
    ...
    old = FakeWatcher(profile_name="old", nastech_home=old_home)
    monkeypatch.setattr(gw, "_watchers", {str(old_home): old})
    monkeypatch.setattr(gw, "GatewayWatcher", FakeWatcher)
    monkeypatch.setattr(profiles, "get_nastech_home_for_profile", lambda name: tmp_path / name)
    ...
    assert watcher.nastech_home == tmp_path / "target"
...
def test_restart_watcher_for_profile_keeps_subscribed_other_profile(tmp_path, monkeypatch):
    ...
    default_watcher = FakeWatcher(profile_name="default", nastech_home=default_home)
    ...
    monkeypatch.setattr(gw, "GatewayWatcher", FakeWatcher)
    monkeypatch.setattr(profiles, "get_nastech_home_for_profile", lambda name: work_home)
...
def test_restart_watcher_for_profile_swaps_atomically(tmp_path, monkeypatch):
    ...
    class FakeWatcher:
        ...
        def start(self):
            self.started = True
            if len(seen) == 0:
                seen.append(gw.get_watcher(profile_name="target", nastech_home=target_home))
    ...
    existing = FakeWatcher(profile_name="target", nastech_home=target_home)
    existing.started = True
    monkeypatch.setattr(gw, "_watchers", {str(target_home): existing})
    monkeypatch.setattr(gw, "GatewayWatcher", FakeWatcher)
    monkeypatch.setattr(profiles, "get_nastech_home_for_profile", lambda name: target_home)
    ...
    assert watcher is created[-1]
    assert gw.get_watcher(profile_name="target", nastech_home=target_home) is watcher
...
def test_start_watcher_pins_active_profile_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(profiles, "get_nastech_home_for_profile", lambda name: profile_home)

    gw.start_watcher()

    assert len(created) == 1
    assert created[0].profile_name == "work"
    assert created[0].nastech_home == profile_home
...
def test_get_watcher_scopes_lookup_to_active_profile(tmp_path, monkeypatch):
    ...
    default_watcher = FakeWatcher(profile_name="default", nastech_home=default_home)
    work_watcher = FakeWatcher(profile_name="work", nastech_home=work_home)
    monkeypatch.setattr(
        gw,
        "_watchers",
        {
...
def test_subscribe_after_stop_gets_sentinel_immediately():
    ...
    watcher = gw.GatewayWatcher(nastech_home=None, profile_name="race")
