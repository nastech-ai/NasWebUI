...
class TestSessionIDValidation:
    ...
    def test_new_format_session_id_passes_validation(self):
        """New NasTech-Agent session IDs (YYYYMMDD_HHMMSS_xxxxxx) must pass validation."""
...
class TestSanitizeError:
    def test_unix_path_stripped(self):
        ...
        e = FileNotFoundError("/home/nastech/.nastech/sessions/abc123.json")
        result = _sanitize_error(e)
        assert "/home/nastech" not in result
        assert "<path>" in result

    def test_nested_unix_path_stripped(self):
        from api.helpers import _sanitize_error
        e = ValueError("cannot read /var/lib/nastech/data.db: permission denied")
        result = _sanitize_error(e)
        assert "/var/lib/nastech" not in result
        assert "<path>" in result
    ...
    def test_windows_path_stripped(self):
        from api.helpers import _sanitize_error
        e = FileNotFoundError("C:\\Users.nastech\\AppData\\sessions\\x.json not found")
        result = _sanitize_error(e)
        assert "C:\\Users.nastech" not in result
...
class TestSkillsPathTraversal:
    ...
    def test_valid_skill_accepted(self, webui_server):
        ...
        # 500 = skills module not available (NasTech-Agent not installed) — skip
        if status == 500:
            import pytest; pytest.skip("skills module requires NasTech-Agent")
