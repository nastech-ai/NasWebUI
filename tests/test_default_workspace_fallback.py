...
def test_resolve_default_workspace_falls_back_to_existing_home_work(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("NASWEBUI_DEFAULT_WORKSPACE", raising=False)
...
def test_env_var_workspace_takes_priority_over_passed_raw(monkeypatch, tmp_path):
    """NASWEBUI_DEFAULT_WORKSPACE env var overrides a None raw arg but not a valid one."""
    env_ws = tmp_path / "env_workspace"
    env_ws.mkdir()
    state_dir = tmp_path / "state"
    monkeypatch.setattr(config, "HOME", tmp_path)
    monkeypatch.setattr(config, "STATE_DIR", state_dir)
    monkeypatch.setenv("NASWEBUI_DEFAULT_WORKSPACE", str(env_ws))
...
def test_env_var_wins_over_settings_json_on_startup(monkeypatch, tmp_path):
    """NASWEBUI_DEFAULT_WORKSPACE must not be overridden by settings.json at startup.
    ...
    if not _os.getenv("NASWEBUI_DEFAULT_WORKSPACE"):
    ...
    assert current_ws == env_ws.resolve(), (
        ...
        "settings.json must not override NASWEBUI_DEFAULT_WORKSPACE."
    )
