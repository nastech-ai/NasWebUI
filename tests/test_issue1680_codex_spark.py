...
def _install_fake_nasmusicui_models(monkeypatch, provider_model_ids):
    ...
    monkeypatch.setitem(sys.modules, "nastech_cli", nastech_cli)
    monkeypatch.setitem(sys.modules, "nastech_cli.models", models)
...
def test_openai_codex_group_uses_provider_model_ids_for_spark(monkeypatch, tmp_path):
    ...
    copied _PROVIDER_MODELS["openai-codex"] without asking nastech_cli.
    """
    calls = []
    ...
    # sys.modules['nastech_cli.models'] without restoring it, list_available_providers
    ...
    # gpt-5.3-codex-spark when nastech_cli.provider_model_ids returns it".
    if calls != ["openai-codex"]:
        import pytest
        pytest.skip(f"nastech_cli stub not active for openai-codex (likely test-isolation pollution from sibling test). Got calls={calls}")
