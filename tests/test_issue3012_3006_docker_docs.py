...
def test_docker_docs_warn_sudo_changes_home_bind_mount():
    """#3006: sudo can render ${HOME}/.nastech as /root/.nastech."""
    assert "`sudo docker compose up -d` can make `${HOME}` expand to the root user's home" in README
    assert "Docker mounts the wrong `.nastech` directory instead of your real `~/.nastech`" in README
    assert "NASTECH_HOME=/home/you/.nastech" in README

    assert "sudo` often changes `$HOME` to `/root`" in DOCKER_MD
    assert "`${NASTECH_HOME:-${HOME}/.nastech}` becomes `/root/.nastech`" in DOCKER_MD
    assert "NASTECH_HOME=/home/youruser/.nastech" in DOCKER_MD
