...
class TestStreamingModelNotFoundDetection:
    ...
    def test_not_found_hint_mentions_settings(self):
        """The model_not_found hint must mention Settings or nastech model."""
        src = _read("api/streaming.py")
        idx = src.find("model_not_found")
        block = src[idx:idx + 500]
        assert "Settings" in block or "nastech model" in block, (
            "model_not_found hint must mention Settings or nastech model command"
        )
