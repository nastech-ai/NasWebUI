...
1. The `NasTech-Agent-src` named volume is mounted READ-ONLY on the WebUI
...
3. `docs/docker.md` documents the agent-image upgrade procedure (`docker volume
   rm NasTech-Agent-src`) — the root cause of #1416.
"""
...
# ── 1: NasTech-Agent-src must be read-only on the WebUI mount ────────────────
...
def test_agent_service_keeps_writable_agent_src_mount():
    ...
    for fn in ("docker-compose.two-container.yml", "docker-compose.three-container.yml"):
        ...
        # The agent's mount is `NasTech-Agent-src:/opt/nastech` (no :ro suffix).
        # Look for the line that has /opt/nastech without :ro.
        agent_lines = [
            line for line in src.splitlines()
            if "NasTech-Agent-src:/opt/nastech" in line
        ]
        assert agent_lines, f"{fn}: agent must mount NasTech-Agent-src at /opt/nastech"
        for line in agent_lines:
            assert not line.rstrip().endswith(":ro"), (
                f"{fn}: agent's NasTech-Agent-src mount must be writable "
                f"(it populates /opt/nastech on first run): {line!r}"
            )
...
def test_two_container_workspace_uses_home_env_var():
    ...
    src = (REPO / "docker-compose.two-container.yml").read_text(encoding="utf-8")
    assert "${NASTECH_WORKSPACE:-${HOME}/workspace}:/workspace" in src, (
    ...
    assert "${NASTECH_WORKSPACE:-~/workspace}" not in src, (
...
def test_docker_md_documents_agent_image_upgrade():
    """The `NasTech-Agent-src` named volume caches the agent source on first
    ...
    assert "NasTech-Agent-src" in docs
...
# which touches `nastech_agent.egg-info/` *inside the source tree* even under
...
def test_docker_init_stages_agent_source_for_writable_install():
    ...
    for line in install_lines:
        assert '"$_agent_src[all]"' not in line, (
            ...
            "directly — this fails with EROFS when the NasTech-Agent volume "
