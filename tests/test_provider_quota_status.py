...
def test_codex_account_usage_is_fetched_under_active_profile_home(monkeypatch, tmp_path):
    """Codex account limits must use the selected WebUI profile's NASTECH_HOME."""
    monkeypatch.setattr(profiles, "get_active_nastech_home", lambda: tmp_path)
    old_cfg, old_mtime = _with_config(model={"provider": "openai-codex"})

    import api.providers as providers
    seen = {}
    previous_home = os.environ.get("NASTECH_HOME")
    ...
    assert os.environ.get("NASTECH_HOME") == previous_home
...
def test_anthropic_oauth_usage_unavailable_reason_is_reported(monkeypatch, tmp_path):
    """NasTech Agent can report why account limits are not available."""
    monkeypatch.setattr(profiles, "get_active_nastech_home", lambda: tmp_path)
...
def test_account_usage_profile_fetches_can_overlap_for_different_homes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(providers, "_get_nastech_home", fake_home)
    monkeypatch.setattr(providers, "_agent_fetch_account_usage_for_home", fake_fetch)
...
def test_provider_quota_card_is_rendered_in_providers_panel():
    ...
    assert "nastech-provider-quota-pool-open" in panels
...
def test_provider_key_mutation_invalidates_warm_account_usage_workers(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(providers, "_get_nastech_home", lambda: tmp_path)
