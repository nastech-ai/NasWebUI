...
def test_cron_panel_loads_gateway_status_for_scheduling_guidance():
    ...
    assert "scheduled jobs require the NasTech gateway daemon" in panels
...
def test_docker_docs_explain_single_container_cron_gateway_boundary():
    ...
    assert "scheduled jobs require the NasTech gateway daemon" in docs
    ...
    assert "NASMUSICUI_GATEWAY_BASE_URL" in docs
