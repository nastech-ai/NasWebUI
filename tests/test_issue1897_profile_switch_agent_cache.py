...
def test_same_session_profile_switch_rebuilds_agent_under_new_soul_home(tmp_path, monkeypatch):
    ...
    NASTECH_HOME at construction time and keeps that value in a cached system
    ...
    default_home = tmp_path / "nastech-home"
    ...
    class SoulCachingAgent:
        ...
        def run_conversation(self, **kwargs):
            ...
            homes_seen_during_runs.append(os.environ.get("NASTECH_HOME"))
    ...
    fake_nastech_state.SessionDB = lambda: None
    ...
    monkeypatch.setattr(profiles, "get_nastech_home_for_profile", home_for_profile)
    ...
    monkeypatch.setitem(sys.modules, "nastech_cli.runtime_provider", fake_runtime_module)
    monkeypatch.setitem(sys.modules, "nastech_state", fake_nastech_state)
...
def test_cache_signature_includes_profile_home():
    ...
    assert "_profile_home" in block, (
        ...
        "previous profile's NASTECH_HOME, leaking the old SOUL.md into new turns."
    )
