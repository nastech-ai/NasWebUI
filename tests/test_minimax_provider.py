...
def _force_env_fallback(monkeypatch):
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name in ("nastech_cli.models", "nastech_cli.auth"):
...
def test_minimax_fallback_provider_label():
    ...
    for NasTech-Agent routing to api.minimax.io. OpenRouter free-tier variants
...
def test_minimax_cn_provider_models_match_nastech_agent_catalog():
...
def test_minimax_api_key_in_env_scan_tuple():
    ...
    MiniMax provider in the dropdown without editing ~/.nastech/.env."""
...
def test_minimax_cn_key_can_be_managed_from_provider_settings():
    """Provider settings should use the NasTech Agent env var for minimax-cn."""
