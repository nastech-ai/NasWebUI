"""Tests for GET /api/commands -- exposes NasTech-Agent COMMAND_REGISTRY."""
import json
import urllib.error
...
def _install_fake_codex_runtime_switch(monkeypatch):
    import sys
    nastech_cli_pkg = sys.modules.get("nastech_cli") or ModuleType("nastech_cli")
    nastech_cli_pkg.__path__ = []
    codex_runtime_switch = ModuleType("nastech_cli.codex_runtime_switch")
    calls = []
    ...
    monkeypatch.setitem(sys.modules, "nastech_cli", nastech_cli_pkg)
    monkeypatch.setitem(sys.modules, "nastech_cli.codex_runtime_switch", codex_runtime_switch)
    return calls
...
def test_list_commands_returns_empty_for_empty_registry():
    ...
    nastech_cli is missing (the empty-or-missing case)."""
    from api.commands import list_commands
    assert list_commands(_registry=[]) == []


def test_list_commands_degrades_when_agent_missing(monkeypatch):
    """If nastech_cli.commands is not importable, list_commands() returns []
    ...
    import sys
    monkeypatch.setitem(sys.modules, 'nastech_cli.commands', None)
