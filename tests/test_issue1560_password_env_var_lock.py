"""Tests for issue #1560 — Settings password silently no-ops when NASMUSICUI_PASSWORD env var is set.

Root cause: NASMUSICUI_PASSWORD takes precedence in api.auth.get_password_hash(),
...
def test_get_settings_surfaces_password_env_var_flag():
    ...
    assert 'NASMUSICUI_PASSWORD' in block, \
        'GET /api/settings must read NASMUSICUI_PASSWORD env var'
...
def test_post_settings_refuses_set_password_when_env_var_shadowed():
    ...
    assert 'NASMUSICUI_PASSWORD env var is set' in src, \
...
def test_post_settings_refuses_clear_password_when_env_var_shadowed():
    ...
    guard_idx = src.index('NASMUSICUI_PASSWORD env var is set')
...
def test_i18n_locked_string_mentions_env_var_name_in_all_locales():
    """Each locale's banner must literally mention NASMUSICUI_PASSWORD so users can find it."""
    src = _read('static/i18n.js')
    blocks = _split_locales(src)
    for loc in LOCALES:
        ...
        assert 'NASMUSICUI_PASSWORD' in line, \
            f"{loc}: banner must literally name NASMUSICUI_PASSWORD"
...
def test_get_settings_returns_password_env_var_false_when_unset(monkeypatch):
    """When NASMUSICUI_PASSWORD is not set in the test process,
    ...
    # NASMUSICUI_PASSWORD while exercising the locked-password path.
    ...
    assert not os.getenv('NASMUSICUI_PASSWORD', '').strip(), \
        'this test requires NASMUSICUI_PASSWORD to be unset'
