"""Tests for issue #1560 — Settings password silently no-ops when NASWEBUI_PASSWORD env var is set.

Root cause: NASWEBUI_PASSWORD takes precedence in api.auth.get_password_hash(),
...
def test_get_settings_surfaces_password_env_var_flag():
    ...
    assert 'NASWEBUI_PASSWORD' in block, \
        'GET /api/settings must read NASWEBUI_PASSWORD env var'
...
def test_post_settings_refuses_set_password_when_env_var_shadowed():
    ...
    assert 'NASWEBUI_PASSWORD env var is set' in src, \
...
def test_post_settings_refuses_clear_password_when_env_var_shadowed():
    ...
    guard_idx = src.index('NASWEBUI_PASSWORD env var is set')
...
def test_i18n_locked_string_mentions_env_var_name_in_all_locales():
    """Each locale's banner must literally mention NASWEBUI_PASSWORD so users can find it."""
    src = _read('static/i18n.js')
    blocks = _split_locales(src)
    for loc in LOCALES:
        ...
        assert 'NASWEBUI_PASSWORD' in line, \
            f"{loc}: banner must literally name NASWEBUI_PASSWORD"
...
def test_get_settings_returns_password_env_var_false_when_unset(monkeypatch):
    """When NASWEBUI_PASSWORD is not set in the test process,
    ...
    # NASWEBUI_PASSWORD while exercising the locked-password path.
    ...
    assert not os.getenv('NASWEBUI_PASSWORD', '').strip(), \
        'this test requires NASWEBUI_PASSWORD to be unset'
