...
class FakeKanbanDB:
    ...
    def worker_log_path(self, task_id):
        ...
        return Path(f"/tmp/nastech-kanban/{task_id}.log")
...
def _load_bridge(monkeypatch):
    ...
    fake_nastech_cli.kanban_db = fake_kanban
    monkeypatch.setitem(sys.modules, "nastech_cli", fake_nastech_cli)
    monkeypatch.setitem(sys.modules, "nastech_cli.kanban_db", fake_kanban)
...
def test_board_pointer_drift_falls_back_to_default(monkeypatch):
    bridge = _load_bridge(monkeypatch)
    fake_kanban = sys.modules["nastech_cli.kanban_db"]
    fake_kanban.boards = {
...
def test_handle_kanban_get_returns_503_when_nastech_cli_missing(monkeypatch):
    """If nastech_cli is unavailable (webui-only deploy), the bridge must
    ...
    """
    bridge = _load_bridge(monkeypatch)
    # Force _kb() to raise ImportError as if nastech_cli was uninstalled
    monkeypatch.setattr(
        bridge, "_kb",
        lambda: (_ for _ in ()).throw(ImportError("No module named 'nastech_cli'")),
    )
...
def test_handle_kanban_post_returns_503_when_nastech_cli_missing(monkeypatch):
    """Same fallback contract for POST verb."""
    bridge = _load_bridge(monkeypatch)
    monkeypatch.setattr(
        bridge, "_kb",
        lambda: (_ for _ in ()).throw(ImportError("nastech_cli missing")),
    )
    captured = {}
...
def test_handle_kanban_patch_returns_503_when_nastech_cli_missing(monkeypatch):
