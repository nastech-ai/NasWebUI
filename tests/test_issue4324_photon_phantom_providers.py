...
These tests mock at the WebUI boundary (stub ``nastech_cli`` + a fake
...
def _install_fake_nastech_cli(monkeypatch, *, with_load_pool: bool = False, pool_data: dict | None = None):
    """Stub nastech_cli so detection is deterministic and offline.
...
def _call_get_available_models(monkeypatch, tmp_path, auth_payload, *,
    ...
    for var in ("OPENAI_API_KEY", "NASTECH_API_KEY", "NASTECH_OPENAI_API_KEY",
