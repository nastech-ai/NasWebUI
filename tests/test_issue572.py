...
Covers:
  1. _provider_api_key_present returns True for minimax-cn when
     MINIMAX_CN_API_KEY is in env (via nastech_cli.auth.get_auth_status)
  ...
  4. The nastech_cli import failure path is safe (falls back gracefully)
"""
...
def _inject_nastech_cli_auth(get_auth_status_return):
    """Inject a minimal nastech_cli.auth stub into sys.modules.

    CI doesn't install nastech_cli (it's a separate package).  Tests that
    exercise the nastech_cli fallback path must inject the module themselves
    rather than relying on mock.patch('nastech_cli.auth.get_auth_status')
    which fails with ModuleNotFoundError when the module isn't installed.
    """
    mock_auth = types.ModuleType("nastech_cli.auth")
    mock_auth.get_auth_status = mock.MagicMock(return_value=get_auth_status_return)
    mock_nastech_cli = types.ModuleType("nastech_cli")

    return mock.patch.dict(sys.modules, {
        "nastech_cli": mock_nastech_cli,
        "nastech_cli.auth": mock_auth,
    })
...
class TestProviderApiKeyPresentFallback:
    ...
    def test_unsupported_provider_logged_out_returns_false(self):
        ...
        with mock.patch("api.onboarding._SUPPORTED_PROVIDER_SETUPS", {
            ...
            with _inject_nastech_cli_auth({"logged_in": False}):
                result = _call_provider_api_key_present("deepseek")
                assert result is False

    def test_nastech_cli_import_failure_is_safe(self):
        """If nastech_cli is unavailable, falls back silently to False."""
        import builtins
        real_import = builtins.__import__

        def _block_nastech_cli(name, *args, **kwargs):
            if name.startswith("nastech_cli"):
                raise ImportError("nastech_cli not available")
            return real_import(name, *args, **kwargs)

        with mock.patch("api.onboarding._SUPPORTED_PROVIDER_SETUPS", {
            "openrouter": {}, "anthropic": {}, "openai": {}, "custom": {}
        }):
            with mock.patch("builtins.__import__", side_effect=_block_nastech_cli):
                result = _call_provider_api_key_present("minimax-cn")
...
class TestOnboardingStatusUnsupportedProvider:
    ...
    def _make_status(self, chat_ready: bool, provider: str = "minimax-cn"):
        ...
        with (
            ...
            mock.patch.object(mod, "verify_nastech_imports", return_value=(True, [], {})),
